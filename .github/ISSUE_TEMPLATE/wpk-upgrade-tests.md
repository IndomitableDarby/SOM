...
title: 'Release [SOM VERSION] - Release Candidate [RC VERSION] - WPK upgrade tests'
labels: 'WPK, core'
assignees: ''
...
Som versions to test (Upgrade to the current agent version):
### Linux
...
- [ ] The som home backup is restored correctly (no traces of the installation, but only the `.tar.gz` backup and the logs).
...
- [ ] Som service runs som-control (`systemctl cat som-agent.service`)
- [ ] Som service runs ossec-control (`systemctl cat som-agent.service`)
- [ ] The service was enabled (`systemctl is-enabled som-agent.service`)
- [ ] Init file runs som-control (`cat /etc/rc.d/init.d/som-agent`)
- [ ] Init file runs ossec-control (`cat /etc/rc.d/init.d/som-agent`)
- [ ] Som as service is enabled `chkconfig --list` 
- [ ] Som starts and connects when the backup is restored (`cat /var/ossec/var/run/ossec-agentd.state`)
- [ ] Som starts and connects automatically when the system is rebooted.
- [ ] Restore SELinux policies (`semodule -l | grep -i som`) (DISABLED)
...
## Windows:
### UPGRADE FAIL
- [ ] Som-Agent folder tree:  No files are lost after the rollback. The logs of the failed upgrade (`ossec.log`) are kept.
- [ ] After the rollback the agent connects to the manager
- [ ] After reboot, the Som-Agent starts and connects to the manager.
- [ ] The correct Som-Agent version is shown in the list of Windows' `programs and features`.
- [ ] A new version of Som-Agent can be manually installed via MSI after the rollback process.

### UPGRADE OK

- [ ] Message `Upgrade finished successfully.` is shown in `upgrade.log` file.
- [ ] Som service is started and the agent is connected to the manager.

...
$uninstall "som-$target" | tee /packages/status.log

if grep -i " removed.*som-$target" $installed_log | tee -a /packages/status.log; then
    echo "Som $target was uninstalled successfully."
    exit 0
else
    echo "Failed to uninstall Som $target."
    exit 1
fi

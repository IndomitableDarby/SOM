...
This integration enrichs any alert generated by Som via the Maltiverse API,
...
And restart Som Manager:

    /etc/init.d/som-manager restart
...
#    5 - Invalid URL
#    6 - Unable to connect to Som
#    7 - Unexpected error sending message to Som
...
def process_args(args: list):
    ...
    # Som Manager in case of positive match
...
def send_event(msg: str, agent: dict | None = None):
    """Send an event to the Som Manager.
    ...
    try:
        ...
        if e.errno == 111:
            print('ERROR: Som is not running.')
            sys.exit(6)
        elif e.errno == 90:
            print('ERROR: Message too long to send to Som.')
            sys.exit(7)

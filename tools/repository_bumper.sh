...
update_file_sources() {
    ...
    # Update som-*.sh scripts
    for script in \
        "$DIR_SRC/init/som-server.sh" \
        "$DIR_SRC/init/som-client.sh" \
        "$DIR_SRC/init/som-local.sh"
    do
        if [[ -n "$new_version" ]]; then
    ...
    # Update som-installer.nsi
    local nsi_file="$DIR_SRC/win32/som-installer.nsi"
    if [[ -n "$new_version" ]]; then
    ...
    # Update som-installer.wxs
    if [[ -n "$new_version" ]]; then
        local wxs_file="$DIR_SRC/win32/som-installer.wxs"
        local current_wxs_version
        current_wxs_version=$(grep -E '<Product ' "$wxs_file" \
            | sed -E 's/.*Version="([^"]+)".*/\1/')

        if [[ "$current_wxs_version" != "$new_version" ]]; then
            sed -i -E "s|(<Product Id=\"\*\" Name=\"Som Agent\" Language=\"1033\" Version=\")[^\"]+(\" Manufacturer=)|\1${new_version}\2|" "$wxs_file"
...
update_file_framework() {
    ...
    local init_file="$DIR_FRAMEWORK/som/__init__.py"
    local cluster_file="$DIR_FRAMEWORK/som/core/cluster/__init__.py"
...
update_file_packages() {
    ...
    for spec_file in $(find "$DIR_PACKAGE" -type f -name "*.spec"); do
        ...
        if [[ -n "$existing_line" ]]; then
            sed -i -E \
                "s|^\* .+ - ${final_version}$|* ${spec_date} support <info@som.com> - ${final_version}|" \
                "$spec_file"
            log_action "Updated changelog date for version ${final_version} in: $spec_file"
        else
            sed -i -E \
                "/^%changelog\s*$/a * ${spec_date} support <info@som.com> - ${final_version}\n- More info: https://documentation.som.com/current/release-notes/release-${final_version//./-}.html" \
                "$spec_file"
...
${INSTALL_TYPE} (${final_version}-RELEASE) stable; urgency=low
 ...
 -- Som, Inc <info@som.com>  ${formatted_date}
...
EOF
        ...
        if grep -qE "$version_pattern_grep" "$changelog_file"; then
            ...
            BEGIN { inside_match = 0 }
            {
                ...
                if (inside_match && $0 ~ /^ -- Som, Inc <info@som.com>  /) {
                    print " -- Som, Inc <info@som.com>  " new_date
                    inside_match = 0
                    next
                }
                print
    ...
    for copyright_file in $(find "$DIR_PACKAGE" -type f -name "copyright"); do
        sed -i -E \
            "s|(^    Som, Inc <info@som.com> on )[^$]+(\$)|\1${formatted_date}\2|" \
            "$copyright_file"

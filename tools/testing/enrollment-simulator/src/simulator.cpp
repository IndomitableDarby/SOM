...
SomAuthSimulator::SomAuthSimulator(const std::string& host_addr, int port_num,
...
SomAuthSimulator::~SomAuthSimulator() {
    if (ssl_ctx) {
        SSL_CTX_free(ssl_ctx);
    }
}

bool SomAuthSimulator::resolve_hostname() {
...
std::string SomAuthSimulator::generate_random_name(int length) {
    std::string name;
...
std::string SomAuthSimulator::generate_agent_name(bool is_new) {
...
AgentConfig SomAuthSimulator::create_agent_config(double new_ratio, double incorrect_pass_ratio,
...
std::string SomAuthSimulator::build_request(const AgentConfig& config) {
    std::stringstream ss;
...
bool SomAuthSimulator::parse_response(const std::string& response) {
    return response.find("OSSEC K:") == 0;
}

RegistrationResult SomAuthSimulator::register_agent(const AgentConfig& config, const DelayRange& connect_delay,
...
void SomAuthSimulator::worker(int num_registrations, double new_ratio, double incorrect_pass_ratio,
                               double modern_version_ratio, double group_ratio,
...
void SomAuthSimulator::run_simulation(int num_threads, int total_registrations, double new_ratio,
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        threads.emplace_back(&SomAuthSimulator::worker, this,

...
A performance testing tool for the Som authentication daemon (`som-authd`). This simulator generates concurrent SSL connections to test agent enrollment scenarios under various load conditions.

## Overview

The enrollment simulator creates multiple threads that simultaneously attempt to register agents with the Som authentication daemon. It supports various agent configurations and provides detailed performance statistics including response times, success rates, and throughput metrics.

## Features

- **Multi-threaded simulation** with configurable thread count
- **SSL/TLS connections** to som-authd
...
|--------|-------------|---------|
| `--host HOST` | Target Som server hostname or IP | localhost |
| `--port PORT` | Target port for som-authd | 1515 |
...
- Ensure som-authd is running and accepting SSL connections
...
### High Error Rates
- Check som-authd logs for error details
...
This tool is part of the Som project and follows the same licensing terms.

...
This directory contains a helper script and a Dockerfile for running [Coverity static analysis](https://scan.coverity.com/) on the Som codebase. It allows executing the analysis both locally and in CI environments using the same containerized setup.

## Files

- `coverity.sh`: main helper script for downloading the image, running the analysis, and uploading results.
- `Dockerfile`: defines the image used to compile Som with Coverity analysis enabled.
...
* `--build`
  Compile the project using the Coverity Docker image and generate the output in `som.tgz`.
* `--upload`
  Upload `som.tgz` to Coverity Scan.
  **Requires** `TOKEN` to be set.
  Fails if the tarball does not exist.
* `--clean`
  Remove generated files (`cov-int/` directory and `som.tgz` tarball).
* `--jobs N`
...
# Just upload the build to the ossec-som project
COVERITY_TOKEN=your_token PROJECT=ossec-som ./coverity.sh --upload
...
You can run this same scan from GitHub Actions, in the [4_codeanalysis_coverity](https://github.com/som/som/actions/workflows/4_codeanalysis_coverity.yml) workflow.

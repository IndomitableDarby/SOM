...
EMAIL="${EMAIL:-devel@som.com}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(realpath "$SCRIPT_DIR/../../..")"
COVERITY_DIR="$SCRIPT_DIR"
IMAGE="ghcr.io/som/coverity-scan:latest"
COV_DIR="$ROOT_DIR/cov-int"
TARBALL="$ROOT_DIR/som.tgz"
...
Options:
  ...
  --build         Run Coverity analysis and generate som.tgz
  --upload        Upload som.tgz to Coverity
  --clean         Remove generated files (cov-int/ and som.tgz)
...
Environment variables:
  PROJECT         Coverity project name (default: "som")
  COVERITY_TOKEN  Coverity token (required for --build-image or --upload)
  EMAIL           Email associated with Coverity account (default: devel@som.com)
EOF
}
...
if $DO_BUILD_IMAGE; then
    ...
    wget -q https://scan.coverity.com/download/linux64 \
        --post-data "token=${COVERITY_TOKEN}&project=som%2F${PROJECT}" \
        -O "$TOOL_TGZ"
...
if $DO_UPLOAD; then
    ...
    response=$(curl -s -w "%{http_code}" \
        ...
        "https://scan.coverity.com/builds?project=som%2F${PROJECT}")

#!/usr/bin/env python3
# MITM tool for Som queue socket
# June 21, 2022
...
def connect(addr=ADDR, blen=BLEN):
    """Connect to the Som queue socket as a client."""
...
def listen(addr=ADDR, blen=BLEN):
    """Bind and listen on the Som queue socket."""
    try:
        unlink(addr)
...
def main():
    parser = argparse.ArgumentParser(
        description="Intercept communication between Som daemons and the main queue socket.",

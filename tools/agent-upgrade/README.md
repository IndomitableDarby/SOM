...
```
openssl req -new -nodes -newkey rsa:2048 -keyout wpkcert.key -out wpkcert.csr -subj '/C=US/ST=CA/O=Som'
```

- `/C=US` is the country.
- `/ST=CA` is the state.
- `/O=Som` is the organization's name.
...
```
curl -Lo som-3.0.zip https://github.com/som/som/archive/3.0.zip
unzip som-3.0.zip
```
...
```
make -C som-3.0/src TARGET=agent
```
...
In this particular case, the Som Project's root directory contains the proper `upgrade.sh` file.
...
1. Overwrite the shipped root CA with your certificate. This will prevent your agent from upgrading using WPK packages from Som.
...
Get the WPK package into the Som manager and run:
...
> som_agent_v3.0.0-beta7_centos_7_x86_64.wpk

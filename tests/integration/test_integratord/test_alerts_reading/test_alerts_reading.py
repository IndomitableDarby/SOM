...
brief: Integratord manages Som integrations with other applications such as Yara or Slack, by feeding
...
references:
    - https://documentation.som.com/current/user-manual/manager/manual-integration.html#slack
    - https://documentation.som.com/current/user-manual/reference/daemons/som-integratord.html
...
from som_testing.constants.daemons import ANALYSISD_DAEMON, SOM_DB_DAEMON, INTEGRATOR_DAEMON
from som_testing.constants.paths.logs import ALERTS_JSON_PATH, SOM_LOG_PATH
from som_testing.modules.analysisd.configuration import ANALYSISD_DEBUG
from som_testing.modules.integratord.configuration import INTEGRATORD_DEBUG
from som_testing.modules.integratord.patterns import INTEGRATORD_THIRD_PARTY_RESPONSE, INTEGRATORD_INODE_CHANGED, \
...
from som_testing.utils.commands import run_local_command_returning_output
from som_testing.utils.configuration import get_test_cases_data, load_configuration_template
from som_testing.utils.file import copy, remove_file
...
daemons_handler_configuration = {'daemons': [INTEGRATOR_DAEMON, SOM_DB_DAEMON, ANALYSISD_DAEMON]}
...
def test_integratord_change_json_inode(test_configuration, test_metadata, set_som_configuration, truncate_monitored_files,
                                       configure_local_internal_options, daemons_handler, wait_for_integratord_start):
    '''
    description: Check that som-integratord detects a change in the inode of the alerts.json and continues reading
                 alerts.
    ...
    test_phases:
        ...
        - test:
            ...
            - Replace the `alerts.json` file while som-integratord is reading it.
            - Wait for the inode change to be detected by som-integratord.
            - Check if som-integratord detects that the file's inode has changed.
            - Insert an alert in the `alerts.json` file.
    ...
    expected_output:
        - r'.+som-integratord.*DEBUG: jqueue_next.*Alert file inode changed.*'
        - r'.+som-integratord.*Processing alert.*'
        - r'.+som-integratord.*<Response [200]>'
    '''
    som_monitor = FileMonitor(SOM_LOG_PATH)
    ...
    # Start monitor
    som_monitor.start(callback=generate_callback(INTEGRATORD_THIRD_PARTY_RESPONSE), timeout=session_parameters.default_timeout)

    # Check that expected log appears
    assert som_monitor.callback_result
    ...
    # Start monitor
    som_monitor.start(callback=generate_callback(INTEGRATORD_INODE_CHANGED), timeout=session_parameters.default_timeout)
...
def test_integratord_read_valid_alerts(test_configuration, test_metadata, set_som_configuration, truncate_monitored_files,
                                       configure_local_internal_options, daemons_handler, wait_for_integratord_start):
    '''
    ...
    expected_output:
        - r'.+som-integratord.*alert_id.*\"integration\": \"slack\".*'
    '''
...
def test_integratord_read_invalid_alerts(test_configuration, test_metadata, set_som_configuration, truncate_monitored_files,
                                         configure_local_internal_options, daemons_handler, wait_for_integratord_start):
    '''
    ...
    test_phases:
        ...
        - test:
            ...
            - Check if som-integratord process the alert and report an error.
        - teardown:
            - Truncate Som's logs.
            - Restore initial configuration, both `ossec.conf` and `local_internal_options.conf`.
    ...
    expected_output:
        - r'.+som-integratord.*WARNING: Invalid JSON alert read.*'
        - r'.+som-integratord.*WARNING: Overlong JSON alert read.*'
    ...
    # Start monitor
    som_monitor.start(callback=generate_callback(callback), timeout=session_parameters.default_timeout)

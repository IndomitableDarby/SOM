...
brief: The 'som-analysisd' daemon receives the log messages and compares them to the rules.
       ...
       Specifically, these tests will check if the 'som-analysisd' daemon generates alerts
...
from som_testing.modules.analysisd import patterns, utils
...
def test_mitre_check_alert(test_configuration, truncate_monitored_files, configure_local_rules, daemons_handler):
    ...
    tags:
        ...
        - wdb_socket
    '''
    som_alert_monitor = file_monitor.FileMonitor(ALERTS_JSON_PATH)

    # Wait until Mitre's event is detected
    if test_configuration not in invalid_configurations:
        som_alert_monitor.start(timeout=30, callback=callbacks.generate_callback(patterns.ANALYSISD_ALERT_STARTED))
        utils.validate_mitre_event(json.loads(som_alert_monitor.callback_result))
    else:

...
brief: The 'som-analysisd' daemon receives the log messages and compares them to the rules.
       ...
       Specifically, these tests will check if the 'som-analysisd' daemon handles correctly
...
from som_testing.constants.daemons import ANALYSISD_DAEMON
from som_testing.constants.paths.logs import SOM_LOG_PATH
from som_testing.constants.paths.sockets import ANALYSISD_QUEUE_SOCKET_PATH
...
def test_error_messages(test_metadata, configure_local_internal_options, configure_sockets_environment_module,
    ...
    '''
    description: Check if when the 'som-analysisd' daemon socket receives a message with an invalid event,
                 it generates the corresponding error that sends to the 'som-db' daemon socket.
    ...
    # Start monitor
    monitor_log = file_monitor.FileMonitor(SOM_LOG_PATH)

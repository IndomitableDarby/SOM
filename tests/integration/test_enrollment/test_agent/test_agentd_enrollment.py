...
brief: This module verifies the correct behavior of Som Agentd during the enrollment under different configurations.
components:
    - agentd
...
from som_testing.utils.services import get_version
...
cases_path = Path(TEST_CASES_PATH, 'cases_som_enrollment.yaml')
config_path = Path(CONFIGS_PATH, 'config_som_enrollment.yaml')
...
def test_agentd_enrollment(test_configuration, test_metadata, set_som_configuration, daemons_handler_module, shutdown_agentd,
    ...
    input_description:
        Different test cases are contained in an external YAML file (som_enrollment_tests.yaml) which includes the

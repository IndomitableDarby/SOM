...
references:
    - https://documentation.som.com/current/user-manual/api/configuration.html#logs
...
from som_testing.constants.api import CONFIGURATION_TYPES, SOM_API_USER, LOGIN_ROUTE
from som_testing.constants.daemons import API_DAEMONS_REQUIREMENTS
from som_testing.constants.paths.logs import SOM_API_JSON_LOG_FILE_PATH, SOM_API_LOG_FILE_PATH
from som_testing.modules.api.patterns import API_TIMEOUT_ERROR_MSG, API_LOGIN_REQUEST_MSG
...
def test_logs_formats(test_configuration, test_metadata, add_configuration, truncate_monitored_files, daemons_handler):
    ...
    if 'json' in current_formats:
        json_file_monitor = file_monitor.FileMonitor(SOM_API_JSON_LOG_FILE_PATH)
        if current_level == 'error':
    ...
    if 'plain' in current_formats:
        plain_file_monitor = file_monitor.FileMonitor(SOM_API_LOG_FILE_PATH)
        if current_level == 'error':

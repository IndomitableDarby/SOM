...
brief: These tests will check that the settings related to the API host address and listening port
       are working correctly. The Som API is an open source 'RESTful' API that allows for interaction
...
references:
    ...
    - https://documentation.som.com/current/user-manual/api/configuration.html#api-configuration-options
...
from som_testing.modules.api.utils import get_base_url, login
...
def test_host_port(test_configuration, test_metadata, add_configuration, truncate_monitored_files, daemons_handler,
    ...
    test_phases:
        ...
        - test:
            - Check that expected host and port appear in the Som API log

...
brief: These tests will check if the response_postprocessing middleware of the API handled by the 'som-apid' daemon is
       working properly. The Som API is an open source 'RESTful' API that allows the interaction with the Som
...
from som_testing.constants.api import SOM_API_PROTOCOL
from som_testing.modules.api.utils import login, get_base_url, set_authorization_header
...
def test_response_postprocessing(test_configuration, test_metadata, truncate_monitored_files, daemons_handler,
    ...
    if use_login_token:
        ...
        session.mount(f"{SOM_API_PROTOCOL}://", adapter)

...
brief: These tests will check if the stats can be obtained from the API and if they follow the expected schema.
       The Som API is an open source 'RESTful' API that allows for interaction with the Som manager from
...
references:
    - https://documentation.som.com/current/user-manual/api/reference.html (Get Som daemon stats)
    - https://documentation.som.com/current/user-manual/api/reference.html (Get Som daemon stats from an agent)
...
from som_testing import DATA_PATH
from som_testing.constants.api import DAEMONS_STATS_ROUTE
from som_testing.modules.api.utils import get_base_url, login, validate_statistics
...
@pytest.mark.parametrize('test_configuration,test_metadata', zip(test1_configuration, test1_metadata),
                         ids=test1_cases_ids)
def test_manager_statistics_format(test_configuration, test_metadata, load_som_basic_configuration,
                                   set_som_configuration, daemons_handler):
    """

...
brief: The Som 'ms-graph' module is capable of communicating with Microsoft Graph & parsing its various
...
from som_testing.modules.modulesd.configuration import MODULESD_DEBUG
from som_testing.modules.modulesd import patterns
from som_testing.tools.monitors.file_monitor import FileMonitor
from som_testing.utils.configuration import get_test_cases_data
from som_testing.utils.configuration import load_configuration_template
...
def test_future_events_yes(test_configuration, test_metadata, set_som_configuration, configure_local_internal_options,
    ...
    parameters:
        ...
        - daemons_handler:
            type: fixture
            brief: Manages daemons to reset Som.
    ...
    expected_output:
        - r'.*som-modulesd:ms-graph.*Bookmark updated'
        - r'.*som-modulesd:ms-graph.*seconds to run first scan'
    '''

    som_log_monitor = FileMonitor(SOM_LOG_PATH)

    som_log_monitor.start(callback=callbacks.generate_callback(r".*som-modulesd:ms-graph.*Bookmark updated"))

    if(som_log_monitor.callback_result != None):
        control_service('stop')
        truncate_file(SOM_LOG_PATH)
        control_service('start')
        som_log_monitor.start(callback=callbacks.generate_callback(r".*som-modulesd:ms-graph.*seconds to run first scan"))
        assert (som_log_monitor.callback_result != None), f'Error, `first scan` not found in log'
    else:
...
def test_future_events_no(test_configuration, test_metadata, set_som_configuration, configure_local_internal_options,
                 truncate_monitored_files, daemons_handler, wait_for_msgraph_start, proxy_setup):
    '''
    ...
    expected_output:
        - r'.*som-modulesd:ms-graph.*Bookmark updated'
        - r'.*som-modulesd:ms-graph.*seconds to run next scan'
    '''
    ...
    if(som_log_monitor.callback_result != None):
        ...
        som_log_monitor.start(callback=callbacks.generate_callback(r".*som-modulesd:ms-graph.*seconds to run next scan"))
        assert (som_log_monitor.callback_result != None), f'Error, `next scan` not found in log'

        som_log_monitor.start(callback=callbacks.generate_callback(r".*som-modulesd:ms-graph.*seconds to run first scan"), timeout=10)
        assert (som_log_monitor.callback_result == None), f'Error, `first scan` not found in log'
    else:
        assert (False), f'Error `Bookmark updated` not found in log'
...
def test_curl_max_size(test_configuration, test_metadata, set_som_configuration, configure_local_internal_options,
                 truncate_monitored_files, daemons_handler, wait_for_msgraph_start, proxy_setup):
    '''
    ...
    expected_output:
        - r'.*som-modulesd:ms-graph.*Reached maximum CURL size'
    '''

    som_log_monitor = FileMonitor(SOM_LOG_PATH)

    som_log_monitor.start(callback=callbacks.generate_callback(r".*som-modulesd:ms-graph.*Reached maximum CURL size"))
    assert (som_log_monitor.callback_result != None), f'Error, `maximum CURL size` not found in log'


@pytest.mark.parametrize('test_configuration, test_metadata', zip(t4_configurations, t4_configuration_metadata), ids=t4_case_ids)
def test_valid_resource(test_configuration, test_metadata, set_som_configuration, configure_local_internal_options,
                 truncate_monitored_files, daemons_handler, wait_for_msgraph_start, proxy_setup):
    '''
    ...
    expected_output:
        - r'.*som-modulesd:ms-graph.*microsoft.graph.security.alert'
        - r'.*som-modulesd:ms-graph.*microsoft.graph.security.incident'
    '''

    som_log_monitor = FileMonitor(SOM_LOG_PATH)

    som_log_monitor.start(callback=callbacks.generate_callback(r".*som-modulesd:ms-graph.*microsoft.graph.security.alert"))
    assert (som_log_monitor.callback_result != None), f'Error, `security.alert` not found in log'

    som_log_monitor.start(callback=callbacks.generate_callback(r".*som-modulesd:ms-graph.*microsoft.graph.security.incident"))
    assert (som_log_monitor.callback_result != None), f'Error, `security.incident` not found in log'


@pytest.mark.parametrize('test_configuration, test_metadata', zip(t5_configurations, t5_configuration_metadata), ids=t5_case_ids)
def test_invalid_resource(test_configuration, test_metadata, set_som_configuration, configure_local_internal_options,
                 truncate_monitored_files, daemons_handler, wait_for_msgraph_start, proxy_setup):
    '''
    ...
    expected_output:
        - r'.*som-modulesd:ms-graph.*Received unsuccessful status
            code when attempting to get relationship \'invalid\'
    '''

    som_log_monitor = FileMonitor(SOM_LOG_PATH)

    som_log_monitor.start(
        callback=callbacks.generate_callback(r".*som-modulesd:ms-graph.*Received unsuccessful "\
                                             r"status code when attempting to get relationship \'invalid\'"))
    assert (som_log_monitor.callback_result != None), f'Error, `unsuccessful status code` not found in log'


@pytest.mark.parametrize('test_configuration, test_metadata', zip(t6_configurations, t6_configuration_metadata), ids=t6_case_ids)
def test_valid_auth(test_configuration, test_metadata, set_som_configuration, configure_local_internal_options,
                 truncate_monitored_files, daemons_handler, wait_for_msgraph_start, proxy_setup):
    '''
    ...
    expected_output:
        - r'.*som-modulesd:ms-graph.*INFO: Scanning tenant'
    '''

    som_log_monitor = FileMonitor(SOM_LOG_PATH)

    som_log_monitor.start(callback=callbacks.generate_callback(r".*som-modulesd:ms-graph.*INFO: Scanning tenant"))
    assert (som_log_monitor.callback_result != None), f'Error, `Scanning tenant` not found in log'


@pytest.mark.parametrize('test_configuration, test_metadata', zip(t7_configurations, t7_configuration_metadata), ids=t7_case_ids)
def test_invalid_auth(test_configuration, test_metadata, set_som_configuration, configure_local_internal_options,
                 truncate_monitored_files, daemons_handler, wait_for_msgraph_start, proxy_setup):
    '''
    description: Check 'ms-graph' behavior when `resource` tags `name` and `relationship` are invalid.
    som_min_version: 4.6.0
    ...
    expected_output:
        - r'.*som-modulesd:ms-graph.*WARNING: Recieved unsuccessful
    ...
    som_log_monitor.start(
        callback=callbacks.generate_callback(r".*som-modulesd:ms-graph.*WARNING: Received unsuccessful "\

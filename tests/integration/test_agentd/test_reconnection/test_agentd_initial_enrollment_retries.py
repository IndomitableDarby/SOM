...
brief: The 'som-agentd' program is the client-side daemon that communicates with the server.
       ...
       the agent successfully enrolls after losing connection with the 'som-remoted' daemon.
...
from som_testing.modules.agentd.patterns import AGENTD_REQUESTING_KEY
...
def test_agentd_initial_enrollment_retries(test_metadata, set_som_configuration, configure_local_internal_options, truncate_monitored_files, remove_keys_file, daemons_handler):
    '''
    description: Check how the agent behaves when it makes multiple enrollment attempts
                 ...
                 performs multiple enrollment requests to the 'som-authd' daemon before
                 getting the new key to communicate with the 'som-remoted' daemon.
    ...
    som_log_monitor.start(callback=callbacks.generate_callback(AGENTD_REQUESTING_KEY,{'IP':''}), timeout = 300, accumulations = 4)
    assert (som_log_monitor.callback_result != None), f'Enrollment retries was not sent'
    ...
    try:
        ...
        # Check if no Som module stopped due to Agentd Initialization
        check_module_stop()
    finally:
        # Reset simulator

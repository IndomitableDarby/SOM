...
brief: A Som cluster is a group of Som managers that work together to enhance the availability
...
from som_testing.constants.ports import DEFAULT_SSL_REMOTE_CONNECTION_PORT
...
from som_testing.utils.client_keys import add_client_keys_entry
...
def test_agentd_multi_server(test_configuration, test_metadata, set_som_configuration, configure_local_internal_options, truncate_monitored_files,
    ...
    assertions:
        - Agent without keys. Verify that all servers will refuse the connection to the 'som-remoted' daemon
        ...
        - Agent without keys. Verify that the first server only has enrollment available, and the third server
          only has the 'som-remoted' daemon available. The agent should enroll in the first server and

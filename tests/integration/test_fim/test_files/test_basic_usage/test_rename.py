...
from som_testing.modules.fim.patterns import EVENT_TYPE_ADDED, EVENT_TYPE_DELETED
...
def test_rename(test_configuration, test_metadata, set_som_configuration, configure_local_internal_options,
                truncate_monitored_files, folder_to_monitor, daemons_handler, start_monitoring, path_to_edit):
    '''
    description: Check if the 'som-syscheckd' daemon detects 'added' and 'deleted' events when renaming a
    ...
    input_description: The test cases are contained in external YAML file (cases_rename.yaml) which includes
                       configuration parameters for the 'som-syscheckd' daemon and testing directories
    ...
    som_log_monitor.start(generate_callback(EVENT_TYPE_ADDED))
    assert som_log_monitor.callback_result
    som_log_monitor.start(generate_callback(EVENT_TYPE_DELETED))
    assert som_log_monitor.callback_result
    assert get_fim_event_data(som_log_monitor.callback_result)['mode'] == fim_mode

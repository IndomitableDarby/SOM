...
brief: File Integrity Monitoring (FIM) system watches selected files and triggering alerts when
       ...
       The FIM capability is managed by the 'som-syscheckd' daemon, which checks configured
...
references:
    ...
    - https://documentation.som.com/current/user-manual/reference/ossec-conf/syscheck.html#diff
...
from som_testing.constants.platforms import MACOS, WINDOWS
from som_testing.constants.paths.logs import SOM_LOG_PATH
from som_testing.modules.fim.configuration import SYSCHECK_DEBUG, RT_DELAY
from som_testing.modules.agentd.configuration import AGENTD_WINDOWS_DEBUG
from som_testing.modules.fim.patterns import EVENT_TYPE_MODIFIED, EVENT_TYPE_ADDED, ERROR_MSG_FIM_EVENT_NOT_DETECTED, \
                                               EVENT_TYPE_DELETED, EVENT_TYPE_REPORT_CHANGES, ERROR_MSG_REPORT_CHANGES_EVENT_NOT_DETECTED
from som_testing.modules.fim.utils import make_diff_file_path, get_fim_event_data
from som_testing.tools.monitors.file_monitor import FileMonitor
from som_testing.utils.file import write_file_write, delete_files_in_folder, truncate_file
from som_testing.utils.string import generate_string
...
def test_reports_file_and_nodiff(test_configuration, test_metadata, configure_local_internal_options,
                        truncate_monitored_files, set_som_configuration, create_paths_files, daemons_handler, detect_end_scan):
    '''
    description: Check if the 'som-syscheckd' daemon reports the file changes (or truncates if required)
    ...
    input_description: A test case is contained in external YAML files (configuration_report_changes_and_diff.yaml, cases_report_changes_and_diff.yaml)
                       which includes configuration settings for the 'som-syscheckd' daemon and, these are
    ...
    som_log_monitor.start(generate_callback(EVENT_TYPE_ADDED), timeout=30)
    assert som_log_monitor.callback_result, ERROR_MSG_FIM_EVENT_NOT_DETECTED
    ...
    som_log_monitor = FileMonitor(SOM_LOG_PATH)
    som_log_monitor.start(generate_callback(EVENT_TYPE_REPORT_CHANGES), timeout=30)
    assert som_log_monitor.callback_result, ERROR_MSG_REPORT_CHANGES_EVENT_NOT_DETECTED
    assert 'No content changes were found for this file.' in str(som_log_monitor.callback_result[0]), 'Wrong content_changes field'
    ...
    som_log_monitor.start(generate_callback(EVENT_TYPE_MODIFIED), timeout=20)
    assert som_log_monitor.callback_result
    event = get_fim_event_data(som_log_monitor.callback_result)
    ...
    som_log_monitor.start(generate_callback(EVENT_TYPE_DELETED))
    assert get_fim_event_data(som_log_monitor.callback_result)['mode'] == test_metadata.get('fim_mode')

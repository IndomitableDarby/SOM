...
references:
    ...
    - https://documentation.som.com/current/user-manual/reference/ossec-conf/syscheck.html#file-size
...
from som_testing.modules.fim.configuration import SYSCHECK_DEBUG, FILE_MAX_SIZE, RT_DELAY
from som_testing.modules.agentd.configuration import AGENTD_WINDOWS_DEBUG
from som_testing.modules.fim.patterns import FILE_SIZE_LIMIT_REACHED, EVENT_TYPE_REPORT_CHANGES, ERROR_MSG_REPORT_CHANGES_EVENT_NOT_DETECTED, ERROR_MSG_FILE_LIMIT_REACHED, \
                                               DIFF_FOLDER_DELETED, ERROR_MSG_FOLDER_DELETED, EVENT_UNABLE_DIFF
from som_testing.modules.fim.utils import make_diff_file_path
from som_testing.tools.monitors.file_monitor import FileMonitor
from som_testing.utils.file import write_file, translate_size
...
def test_file_size_values(test_configuration, test_metadata, configure_local_internal_options,
                                    truncate_monitored_files, set_som_configuration, folder_to_monitor, file_to_monitor, daemons_handler, detect_end_scan):
    '''
    description: Check if the 'som-syscheckd' daemon limits the size of the monitored file to generate
    ...
    som_log_monitor.start(callback=generate_callback(EVENT_TYPE_REPORT_CHANGES), timeout=30)
    assert som_log_monitor.callback_result, ERROR_MSG_REPORT_CHANGES_EVENT_NOT_DETECTED
    content_changes = str(som_log_monitor.callback_result)
    ...
    som_log_monitor.start(callback=generate_callback(DIFF_FOLDER_DELETED), timeout=30)
    assert som_log_monitor.callback_result, ERROR_MSG_FOLDER_DELETED

    if os.path.exists(diff_file_path):
        pytest.raises(FileExistsError(f"{diff_file_path} found. It should not exist after incresing the size."))

    som_log_monitor.start(callback=generate_callback(FILE_SIZE_LIMIT_REACHED), timeout=30)
    assert som_log_monitor.callback_result, ERROR_MSG_FILE_LIMIT_REACHED

    # Check the content_changes field in the event
    som_log_monitor.start(callback=generate_callback(EVENT_UNABLE_DIFF), timeout=30)
    assert som_log_monitor.callback_result, ERROR_MSG_REPORT_CHANGES_EVENT_NOT_DETECTED
    assert 'Unable to calculate diff due to \'file_size\' limit has been reached.' in som_log_monitor.callback_result, 'Wrong content_changes field'

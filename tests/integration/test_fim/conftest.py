...
from som_testing.constants.platforms import WINDOWS, MACOS, CENTOS, UBUNTU, DEBIAN
from som_testing.modules.fim.patterns import MONITORING_PATH, EVENT_TYPE_SCAN_END
from som_testing.modules.fim.utils import create_registry, delete_registry
...
from som_testing.utils.services import get_service
...
def start_monitoring() -> None:
    FileMonitor(SOM_LOG_PATH).start(generate_callback(MONITORING_PATH))
...
def detect_end_scan(test_metadata: dict) -> None:
    ...
    som_log_monitor.start(timeout=60, callback=generate_callback(EVENT_TYPE_SCAN_END))

...
references:
    ...
    - https://documentation.som.com/current/user-manual/reference/ossec-conf/syscheck.html#windows-registry
...
from som_testing.utils import file
from som_testing.modules.fim.patterns import EVENT_TYPE_ADDED, EVENT_TYPE_DELETED, IGNORING_DUE_TO_RESTRICTION
...
from som_testing.modules.fim.utils import get_fim_event_data, delete_registry
...
def test_restrict_key(test_configuration, test_metadata,configure_local_internal_options,
                            truncate_monitored_files, set_som_configuration, daemons_handler, detect_end_scan, create_registry_key):
    '''
    description: Check if the 'som-syscheckd' daemon detects or ignores events in monitored registry entries
    ...
    if test_metadata['triggers_event']:
        som_log_monitor.start(callback=generate_callback(EVENT_TYPE_ADDED))
        ...
        som_log_monitor.start(callback=generate_callback(IGNORING_DUE_TO_RESTRICTION))
        assert som_log_monitor.callback_result

        delete_registry(win32con.HKEY_LOCAL_MACHINE, test_metadata['sub_key'], KEY_WOW64_64KEY if test_metadata['arch'] == 'x64' else KEY_WOW64_32KEY)

        som_log_monitor.start(callback=generate_callback(IGNORING_DUE_TO_RESTRICTION), only_new_events=True)
        assert not som_log_monitor.callback_result

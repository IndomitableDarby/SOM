...
brief: Active responses execute a script in response to the triggering of specific alerts based
       ...
       which are executed by the 'som-execd' daemon via scripts, run correctly.
...
references:
    - https://documentation.som.com/current/user-manual/capabilities/active-response/#active-response
'''
import sys
import pytest
...
from som_testing.constants.paths.logs import ACTIVE_RESPONSE_LOG_PATH, SOM_LOG_PATH
from som_testing.constants.platforms import WINDOWS
from som_testing.modules.execd.active_response.patterns import ACTIVE_RESPONSE_RESTART_SOM
from som_testing.modules.agentd.configuration import AGENTD_WINDOWS_DEBUG
from som_testing.modules.execd.configuration import EXECD_DEBUG
from som_testing.modules.execd.patterns import EXECD_SHUTDOWN_RECEIVED
...
ar_conf = 'restart-som0 - restart-som - 0\nrestart-som0 - restart-som.exe - 0'
...
def test_execd_restart(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
                       set_som_configuration, configure_ar_conf, remoted_simulator, authd_simulator,
                       daemons_handler, send_execd_message):
    '''
    description: Check if 'restart-som' command of 'active response' is executed correctly.
                 For this purpose, a simulated server is used, from which the active response is sent.
                 This response includes the order to restart the Som agent, which must restart after
    ...
    assertions:
        ...
        - Check the restart-som program is used.
    input_description:
    ...
    if error_message := test_metadata.get('expected_error'):
        ...
        assert ar_monitor.callback_result, 'AR `som-restart` did not fail.'
        return

    som_log_monitor.start(callback=generate_callback(EXECD_SHUTDOWN_RECEIVED))
    assert som_log_monitor.callback_result, 'Execd `shutdown` log not raised.'

    ar_monitor.start(callback=generate_callback(ACTIVE_RESPONSE_RESTART_SOM))
    assert ar_monitor.callback_result, 'AR `restart-som` program not used.'

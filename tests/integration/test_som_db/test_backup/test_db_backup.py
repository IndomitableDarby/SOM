...
from som_testing.utils.db_queries.global_db import remove_metadata_value
...
# Configurations
t_cases_path = Path(TEST_CASES_FOLDER_PATH, 'cases_som_db_backup_command.yaml')
...
def test_wdb_backup_command(configure_sockets_environment_module, connect_to_sockets_module, remove_backups,
    ...
    '''
    description: Check that every input message using the 'backup' command in som-db socket generates
                 the proper output to som-db socket. To do this, it performs a series of queries to the socket with
    ...
    input_description:
        - Test cases are defined in the som_db_backup_command.yaml file. This file contains the amount of backups to

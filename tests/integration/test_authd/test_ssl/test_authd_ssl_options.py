...
brief: These tests will check if the 'SSL' (Secure Socket Layer) protocol-related settings of
       the 'som-authd' daemon are working correctly. The 'som-authd' daemon can
       automatically add a Som agent to a Som manager and provide the key
...
references:
    ...
    - https://documentation.som.com/current/user-manual/reference/ossec-conf/auth.html
...
def test_ossec_auth_configurations(test_configuration, test_metadata, set_som_configuration,
                                   truncate_monitored_files, daemons_handler,
                                   configure_sockets_environment, wait_for_authd_startup):
    '''
    description:
        Checks if the 'SSL' settings of the 'som-authd' daemon work correctly by enrolling agents

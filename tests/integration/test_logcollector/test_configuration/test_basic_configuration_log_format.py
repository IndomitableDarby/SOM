...
brief: The 'som-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'log_format' tag and the Som API returns the same values for the configured
...
references:
    ...
    - https://documentation.som.com/current/user-manual/reference/ossec-conf/localfile.html#log-format
    - https://documentation.som.com/current/user-manual/reference/ossec-conf/localfile.html#location
...
from som_testing.constants.paths.logs import SOM_LOG_PATH, MACOS_LOG_COMMAND_PATH
...
from som_testing.modules.logcollector import patterns, PREFIX
...
default_config_path = Path(CONFIGURATIONS_PATH, 'som_basic_configuration_log_format.yaml')
default_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format.yaml')

macos_duplicated_config_path = Path(CONFIGURATIONS_PATH, 'som_duplicated_macos_configuration.yaml')
macos_duplicated_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format_macos_duplicated.yaml')

macos_no_defined_config_path = Path(CONFIGURATIONS_PATH, 'som_no_defined_location_macos_configuration.yaml')
macos_no_defined_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format_macos_no_defined.yaml')

win_config_path = Path(CONFIGURATIONS_PATH, 'som_basic_configuration_log_format_location.yaml')
...
def check_log_format_valid(test_configuration, test_metadata):
    """Check if Som run correctly with the specified log formats.
    ...
    if test_metadata['log_format'] not in log_format_not_print_analyzing_info:
        som_log_monitor.start(timeout=5,
        ...
        if test_metadata['log_format'] == 'full_command':
            ...
            callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MONITORING_COMMAND, {
                                'command': test_metadata['command']})
        som_log_monitor.start(timeout=5, callback=callback)
        assert (som_log_monitor.callback_result != None), patterns.ERROR_COMMAND_MONITORING
        ...
        assert (som_log_monitor.callback_result != None), patterns.ERROR_DJB_MULTILOG_NOT_PRODUCED
        ...
        if 'location' in test_metadata and test_metadata['location'] != 'macos':
            ...
            assert (som_log_monitor.callback_result != None), patterns.ERROR_INVALID_MACOS_VALUE
        if 'location' not in test_metadata:
            som_log_monitor.start(timeout=5,
                                    callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MISSING_LOCATION))
            assert (som_log_monitor.callback_result != None), patterns.ERROR_MISSING_LOCATION_VALUE

        som_log_monitor.start(timeout=5,
                                callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MONITORING_LOGS,
                                                   {'command_path': MACOS_LOG_COMMAND_PATH}))
        assert (som_log_monitor.callback_result != None), patterns.ERROR_MACOS_LOG_NOT_PRODUCED


def check_log_format_invalid(test_metadata):
    """Check if Som fails because a invalid frequency configuration value.
    ...
    som_log_monitor.start(timeout=5, callback=log_callback)
    assert (som_log_monitor.callback_result != None), patterns.ERROR_GENERIC_MESSAGE
...
def check_log_file_duplicated():
    """Check if Som shows a warning message when the configuration is duplicated."""
    som_log_monitor = file_monitor.FileMonitor(SOM_LOG_PATH)
    som_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT,
                            callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_LOG_FILE_DUPLICATED))
    assert (som_log_monitor.callback_result != None), patterns.ERROR_LOG_FILE_DUPLICATED
    som_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT,
                            callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MONITORING_LOGS,
                                                                 {'command_path': MACOS_LOG_COMMAND_PATH}))
    assert (som_log_monitor.callback_result != None), patterns.ERROR_ANALYZING_MACOS
...
def test_log_format(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
                    set_som_configuration, daemons_handler_module, stop_logcollector):
    '''
    description: Check if the 'som-logcollector' daemon detects invalid configurations for the 'log_format' tag.
                 ...
                 Finally, the test will verify that the Som API returns the same values for the 'localfile' section
    ...
    input_description: A configuration templates (test_basic_configuration_log_format) are contained in externals
                       YAML files (som_basic_configuration.yaml, som_duplicated_macos_configuration.yaml, and
                       som_no_defined_location_macos_configuration.yaml). Those templates are combined with
    ...
    if test_metadata['valid_value']:
        ...
        if sys.platform == WINDOWS:
            pytest.xfail("Windows agent allows invalid localfile configuration:\
                          https://github.com/som/som/issues/10890")

...
from som_testing.constants.paths.configurations import SOM_CONF_PATH
from som_testing.constants.daemons import LOGCOLLECTOR_DAEMON
from som_testing.modules.logcollector.patterns import LOGCOLLECTOR_MODULE_START
...
from som_testing.utils.file import truncate_file, replace_regex_in_file, write_json_file

# Logcollector internal paths
LOGCOLLECTOR_OFE_PATH = path_join(SOM_PATH, 'queue', 'logcollector', 'file_status.json')

@pytest.fixture()
def stop_logcollector(request):
    """Stop som-logcollector and truncate logs file."""
...
@pytest.fixture()
def remove_all_localfiles_som_config(request):
    """Configure a custom settting for testing. Restart Som is needed for applying the configuration. """
    ...
    replace_regex_in_file(list_tags, [''] * len(list_tags), SOM_CONF_PATH, True)

...
from som_testing.constants import platforms
from som_testing.constants.platforms import WINDOWS
from som_testing.constants.daemons import SOM_MANAGER, API_DAEMONS_REQUIREMENTS
from som_testing.constants.paths import ROOT_PREFIX
from som_testing.constants.paths.api import RBAC_DATABASE_PATH
from som_testing.constants.paths.logs import ACTIVE_RESPONSE_LOG_PATH, SOM_LOG_PATH, ALERTS_JSON_PATH, \
                                               SOM_API_LOG_FILE_PATH, SOM_API_JSON_LOG_FILE_PATH
from som_testing.constants.paths.configurations import SOM_CLIENT_KEYS_PATH
from som_testing.logger import logger
from som_testing.tools import socket_controller
from som_testing.tools.monitors import queue_monitor
from som_testing.tools.monitors.file_monitor import FileMonitor
from som_testing.tools.simulators.agent_simulator import create_agents, connect
from som_testing.tools.simulators.authd_simulator import AuthdSimulator
from som_testing.tools.simulators.remoted_simulator import RemotedSimulator
from som_testing.utils import configuration, database, file, mocking, services
from som_testing.utils.file import remove_file, truncate_file
from som_testing.utils.manage_agents import remove_agents
import som_testing.utils.configuration as som_configuration
...
def load_som_basic_configuration():
    ...
    backup_ossec_configuration = configuration.get_som_conf()

    # Write new configuration
    configuration.write_som_conf(minimal_configuration)

    yield

    # Restore the ossec.conf backup
    configuration.write_som_conf(backup_ossec_configuration)


@pytest.fixture()
def backup_som_configuration() -> None:
    """Backup som configuration. Saves the initial configuration and restores it after the test."""
    # Save configuration
    backup_config = configuration.get_som_conf()

    yield

    # Restore configuration
    configuration.write_som_conf(backup_config)


@pytest.fixture()
def set_som_configuration(test_configuration: dict) -> None:
    """Set som configuration
    ...
    test_config = configuration.set_section_som_conf(test_configuration.get('sections'))

    # Set new configuration
    configuration.write_som_conf(test_config)
...
def configure_local_internal_options_function(request):
    ...
    backup_local_internal_options = som_configuration.get_local_internal_options_dict()

    logger.debug(f"Set local_internal_option to {str(local_internal_options)}")
    som_configuration.set_local_internal_options_dict(local_internal_options)

    yield

    logger.debug(f"Restore local_internal_option to {str(backup_local_internal_options)}")
    som_configuration.set_local_internal_options_dict(backup_local_internal_options)


@pytest.fixture()
def restart_som_function(request):
    ...
    if len(daemons) == 0:
        ...
        # Stop a list daemons in order (as Som does)
        daemons.reverse()
        for daemon in daemons:
...
def truncate_monitored_files_implementation() -> None:
    ...
    if services.get_service() == SOM_MANAGER:
        log_files = [SOM_LOG_PATH, ALERTS_JSON_PATH, SOM_API_LOG_FILE_PATH,
                     SOM_API_JSON_LOG_FILE_PATH, SOM_CLIENT_KEYS_PATH]
    else:
        log_files = [SOM_LOG_PATH]
...
def daemons_handler_implementation(request: pytest.FixtureRequest) -> None:
    """Helper function to handle Som daemons.
    ...
    The  `daemons_handler_configuration` should be a dictionary with the following keys:
        daemons (list, optional): List with every daemon to be used by the module. In case of empty a ValueError
            will be raised
        all_daemons (boolean): Configure to restart all som services. Default `False`.
    ...
    if config := getattr(request.module, 'daemons_handler_configuration', None):
        ...
        if 'all_daemons' in config:
            logger.debug(f"Som control set to {config['all_daemons']}")
        ...
        logger.debug("Som control set to 'all_daemons'")
        all_daemons = True

    try:
        if all_daemons:
            logger.debug('Restarting som using som-control')
    ...
    if all_daemons:
        logger.debug('Stopping som using som-control')
...
@pytest.fixture(scope='module')
def restart_som_daemon_after_finishing_module(daemon: str = None) -> None:
    """Restart a Som daemons and clears the som log after the test module finishes execution.
    Args:
        daemon (str): provide which daemon to restart. If None, all daemons will be restarted.
    """
    yield
    file.truncate_file(SOM_LOG_PATH)
    services.control_service("restart", daemon=daemon)
...
def configure_local_internal_options_handler(request: pytest.FixtureRequest, test_metadata) -> None:
    ...
    The `local_internal_options` is a dict with keys and values as the Som `local_internal_options` format.
...
def configure_sockets_environment_implementation(request: pytest.FixtureRequest) -> None:
    ...
    # Stop som-service and ensure all daemons are stopped
    services.control_service('stop')
...
def mock_agent_module():
    """Fixture to create a mocked agent in som databases"""
...
def autostart_simulators(request: pytest.FixtureRequest) -> None:
    """
    Fixture for starting simulators in som-agent executions.

    This fixture starts both Authd and Remoted simulators only in the cases where the service is not
    SOM_MANAGER, and when the test function is not already using the simulator fixture, if it does
    use one of them, only start the remaining simulator.

    This is required so all som-agent instances are being tested with the som-manager connection
    being mocked.
    """
    create_authd = 'authd_simulator' not in request.fixturenames
    create_remoted = 'remoted_simulator' not in request.fixturenames

    if services.get_service() is not SOM_MANAGER:

...
brief: Som-db is the daemon in charge of the databases with all the Som persistent information, exposing a socket
       ...
       Som-db confirms that is able to save, update and erase the necessary information into the corresponding
...
from som_testing.utils.callbacks import make_callback
from som_testing.modules.som_db import SOM_DB_PREFIX
from som_testing.utils import configuration
from som_testing.utils.database import query_wdb
from som_testing.utils.db_queries.agent_db import agent_integrity_check
...
SOM_DB_CHECKSUM_CALCULUS_TIMEOUT = 20
...
# List items -> (som_daemon: str,(
#                mitm: ManInTheMiddle
#                daemon_first: bool))
# Example1 -> ('som-clusterd', None)              Only start som-clusterd with no MITM
# Example2 -> ('som-clusterd', (my_mitm, True))   Start MITM and then som-clusterd
...
def validate_som_db_response(expected_output, response):
    """ Method to validate the Som-DB response.
...
def test_som_db_messages_agent(test_metadata, configure_sockets_environment_module, connect_to_sockets_module,
                                 clean_databases, clean_registered_agents, insert_agents_test):
    '''
    description: Check that every input agent message in som-db socket generates the proper output to som-db
    ...
    parameters:
        ...
        - clean_registered_agents:
            type: fixture
            brief: Remove all agents of somdb.
        - insert_agents_test:
            type: fixture
    ...
    for index, stage in enumerate(test_metadata['test_case']):
        ...
        if 'use_regex' in stage and stage['use_regex'] == 'yes':
            ...
            match = validate_som_db_response(expected_output, response)
...
def test_som_db_messages_global(test_metadata, daemons_handler_module, connect_to_sockets_module,
                                  clean_databases, clean_registered_agents):
    '''
    description: Check that every global input message in som-db socket generates the proper output to som-db
                 socket. To do this, it performs a query to the socket with a command taken from the input list of
                 stages (test_case, input field) and compare the result with the input list of stages (test_case,
                 output field).
...
def test_som_db_range_checksum(configure_sockets_environment_module, connect_to_sockets_module,
    ...
    log_monitor.start(callback=make_callback('range checksum: Time: ', prefix=SOM_DB_PREFIX,
                                             escape=True), timeout=SOM_DB_CHECKSUM_CALCULUS_TIMEOUT)
    ...
    agent_integrity_check()
    log_monitor.start(callback=make_callback('range checksum avoided', prefix=SOM_DB_PREFIX,
                                             escape=True), timeout=SOM_DB_CHECKSUM_CALCULUS_TIMEOUT)
...
def test_som_db_messages_tasks(test_metadata, daemons_handler_module, connect_to_sockets_module,
                                  clean_databases, clean_registered_agents):
    '''
    description: Check that every global input message in som-db socket generates the proper output to som-db
                 socket. To do this, it performs a query to the socket with a command taken from the input list of

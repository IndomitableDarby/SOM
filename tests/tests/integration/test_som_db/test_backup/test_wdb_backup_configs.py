...
brief: Som-db is the daemon in charge of the databases with all the Som persistent information, exposing a socket
       to receive requests and provide information. Som-db has the capability to do automatic database backups, based
...
from som_testing.constants.paths.logs import SOM_PATH, SOM_LOG_PATH
from som_testing.utils.time import validate_interval_format, time_to_seconds
from som_testing.modules.som_db import patterns
...
# Configuration
t_config_path = Path(CONFIGURATIONS_FOLDER_PATH, 'configuration_som_db_backups_conf.yaml')
t_cases_path = Path(TEST_CASES_FOLDER_PATH, 'cases_som_db_backups_conf.yaml')
...
backups_path = Path(SOM_PATH, 'backup', 'db')
...
def test_wdb_backup_configs(test_configuration, test_metadata, set_som_configuration,
    ...
    description: Check that given different wdb backup configuration parameters, the expected behavior is achieved.
                 For this, the test gets a series of parameters for the som_db_backups_conf.yaml file and applies
    ...
    input_description:
        - Test cases are defined in the parameters and metada variables, that will be applied to the the
          som_db_backup_command.yaml file. The parameters tested are: "enabled", "interval" and "max_files".
    ...
    try:
        ...
        if not validate_interval_format(test_interval):
            som_log_monitor.start(callback=callbacks.generate_callback(patterns.WRONG_INTERVAL_CALLBACK), timeout=timeout)
            assert som_log_monitor.callback_result, 'Did not receive expected ' \
                                                    '"Invalid value element for interval..." event'

            return
        elif not isinstance(test_max_files, numbers.Number) or test_max_files==0:
            som_log_monitor.start(callback=callbacks.generate_callback(patterns.WRONG_MAX_FILES_CALLBACK), timeout=timeout)
            assert som_log_monitor.callback_result, 'Did not receive expected ' \
    ...
    # Manage if backup generation is enabled - one or more backups expected
        ...
        som_log_monitor.start(callback=callbacks.generate_callback(patterns.BACKUP_CREATION_CALLBACK),
                                        timeout=timeout, accumulations=int(test_max_files)+1)
        result = som_log_monitor.callback_result

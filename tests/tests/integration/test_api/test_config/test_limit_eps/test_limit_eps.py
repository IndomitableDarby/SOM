"""
copyright: Copyright (C) 2015, Som Inc.
...
references:
    ...
    - https://documentation.som.com/current/user-manual/api/configuration.html
...
from som_testing.constants.api import CONFIGURATION_TYPES, MANAGER_CONFIGURATION_ROUTE
from som_testing.constants.daemons import API_DAEMONS_REQUIREMENTS
from som_testing.modules.api.utils import login, get_base_url
from som_testing.utils.configuration import get_test_cases_data, load_configuration_template, get_som_conf
...
def test_limit_eps(test_configuration, test_metadata, set_som_configuration, add_configuration,
    ...
    som_config = " ".join(get_som_conf())
    assert f"<maximum>{test_metadata['expected_max_value']}</maximum>" in som_config
    assert f"<timeframe>{test_metadata['expected_timeframe_value']}</timeframe>" in som_config

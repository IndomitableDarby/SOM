...
brief: These tests will check if the 'drop_privileges' setting of the API is working properly.
       This setting allows the user who starts the 'som-apid' daemon to be different from
       the 'root' user. The Som API is an open source 'RESTful' API that allows for interaction
       with the Som manager from a web browser, command line tool like 'cURL' or any script
...
references:
    - https://documentation.som.com/current/user-manual/api/getting-started.html
    - https://documentation.som.com/current/user-manual/api/configuration.html#drop-privileges
...
from som_testing.constants.api import CONFIGURATION_TYPES
from som_testing.constants.daemons import API_DAEMONS_REQUIREMENTS
from som_testing.constants.paths.api import SOM_API_SCRIPT
from som_testing.utils.configuration import get_test_cases_data, load_configuration_template
from som_testing.utils.services import search_process_by_command
...
def test_drop_privileges(test_configuration, test_metadata, add_configuration, truncate_monitored_files,
    ...
    description: Check if 'drop_privileges' affects the user of the API process. In this test, the 'PID' of the API
                 process is obtained. After that, it gets the user ('root' or 'som') and checks if it matches the
    ...
    test_phases:
        ...
        - test:
            - Search som-apid process and verify that it is present
    ...
    parameters:
        ...
        - add_configuration:
            type: fixture
            brief: Add configuration to the Som API configuration files.
        - truncate_monitored_files:
            type: fixture
            brief: Truncate all the log files and json alerts files before and after the test execution.
        - daemons_handler:
            type: fixture
            brief: Wrapper of a helper function to handle Som daemons.
        - wait_for_api_start:
            type: fixture
            brief: Monitor the API log file to detect whether it has been started or not.

    assertions:
        - Verify that when 'drop_privileges' is enabled the user who has started the 'som-apid' daemon is 'som'.
        - Verify that when 'drop_privileges' is disabled the user who has started the 'som-apid' daemon is 'root'.
    ...
    expected_output:
        - PID of the 'som-apid' process.
        - som (if drop_privileges is enabled)
        - root (if drop_privileges is disabled)
    """
    expected_user = test_metadata['expected_user']

    # Get som-apid process info
    api_process = search_process_by_command(SOM_API_SCRIPT)
    assert api_process is not None, f"The process {SOM_API_SCRIPT} could not be found"

...
brief: These tests will check if the 'remote enrollment' option of the 'som-authd' daemon
       settings is working properly. The 'som-authd' daemon can automatically add
       a Som agent to a Som manager and provide the key to the agent.
...
references:
    - https://documentation.som.com/current/user-manual/reference/ossec-conf/auth.html#remote-enrollment
...
from som_testing.constants.ports import DEFAULT_SSL_CLUSTER_PORT
from som_testing.utils import callbacks
from som_testing.modules.authd import PREFIX
from som_testing.tools.socket_controller import SocketController
...
from som_testing.constants.daemons import AUTHD_DAEMON, SOM_DB_DAEMON, MODULES_DAEMON
from som_testing.utils.configuration import load_configuration_template, get_test_cases_data
...
monitored_sockets_params = [(MODULES_DAEMON, None, True), (SOM_DB_DAEMON, None, True), (AUTHD_DAEMON, None, True)]
...
def test_remote_enrollment(test_configuration, test_metadata, set_som_configuration,
                           truncate_monitored_files, daemons_handler):
    '''
    description:
        Checks if the 'som-authd' daemon remote enrollment is enabled/disabled according
    ...
    parameters:
        ...
        - daemons_handler:
            type: fixture
            brief: Restarts som or a specific daemon passed.
    ...
    expected_output:
        - r'Accepting connections on port 1515. No password required.' (When the 'som-authd' daemon)
    ...
    file_monitor.FileMonitor(SOM_LOG_PATH).start(timeout=5,

...
def test_authd_use_password_invalid(test_configuration, test_metadata, set_som_configuration, truncate_monitored_files_module,
    ...
    assertions:
        ...
        - som-manager.service must not be able to restart.

    input_description:
        ./data/configuration_template/config_authd_use_password_invalid.yaml: Som config needed for the tests.
    ...
    if log == 'Invalid password provided.':
        pytest.xfail(reason="No password validation in authd.pass - Issue som/som#16282.")

    # Verify som-manager fails at restart.

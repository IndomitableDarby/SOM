...
brief: These tests will check if the 'som-authd' daemon correctly responds to the enrollment requests
...
from som_testing.constants.daemons import AUTHD_DAEMON, SOM_DB_DAEMON
from som_testing.utils.configuration import load_configuration_template, get_test_cases_data
from som_testing.modules.authd.utils import create_authd_request, validate_authd_response
...
from som_testing.modules.authd.configuration import AUTHD_DEBUG_CONFIG
...
monitored_sockets_params = [(AUTHD_DAEMON, None, True), (SOM_DB_DAEMON, None, True)]
...
def check_options(test_metadata):
    ...
    for stage in test_metadata['test_case']:
        ...
        for log in stage['log']:
            log = re.escape(log)
            som_log_monitor.start(callback=callbacks.generate_callback(fr'{PREFIX}{log}'), timeout=10, encoding='utf-8')
            assert som_log_monitor.callback_result, f'Error event not detected'
...
def test_authd_force_options(test_configuration, test_metadata, set_som_configuration,
...
def test_authd_force_insert(test_configuration, test_metadata, set_som_configuration,
                            configure_local_internal_options, truncate_monitored_files,
                            insert_pre_existent_agents, daemons_handler,
                            wait_for_authd_startup, connect_to_sockets):
    '''
...
def test_authd_force_insert_only(test_configuration, test_metadata, set_som_configuration,
                                 configure_local_internal_options, truncate_monitored_files,
                                 insert_pre_existent_agents, daemons_handler,
                                 wait_for_authd_startup, connect_to_sockets):
    '''

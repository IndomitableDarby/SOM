...
brief: The 'som-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'location' tag is set in the configuration, and the Som API returns the same values for
...
from som_testing.modules.agentd import configuration as agentd_configuration
...
# Configuration
default_config_path = Path(CONFIGURATIONS_PATH, 'som_basic_configuration_log_format_location.yaml')
...
def test_configuration_location(test_configuration, test_metadata, truncate_monitored_files, configure_local_internal_options,
                                set_som_configuration, daemons_handler, wait_for_logcollector_start):
    '''
    description: Check if the 'som-logcollector' daemon starts properly when the 'location' tag is used.
                 ...
                 Som component is started by checking its process, and the Som API returns the same
    ...
    assertions:
        - Verify that the Som component (agent or manager) can start when the 'location' tag is used.
        - Verify that the Som API returns the same value for the 'localfile' section as the configured one.
    ...
    expected_output:
        - Boolean values to indicate the state of the Som component.
    ...
    if sys.platform == WINDOWS:
        if test_metadata['location'] == 'invalidchannel':
            ...
            som_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT, callback=callback)
            assert (som_log_monitor.callback_result != None), patterns.ERROR_EVENTCHANNEL
    else:

...
brief: The Som 'github' module allows you to collect all the 'audit logs' from GitHub using its API.
...
references:
    - https://github.com/som/som-documentation/blob/develop/source/github/monitoring-github-activity.rst
...
def test_invalid(test_configuration, test_metadata, set_som_configuration, configure_local_internal_options,
    ...
    input_description: A configuration template (github_integration) is contained in an external YAML file
                       (som_conf.yaml). That template is combined with different test cases defined in
    ...
    som_log_monitor.start(callback=callbacks.generate_callback(patterns.MODULESD_CONFIGURATION_ERROR, {

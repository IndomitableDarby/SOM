...
from som_testing.modules.fim.patterns import EVENT_TYPE_ADDED, FIM_EVENT_RESTRICT
...
from som_testing.modules.fim import configuration
...
def test_restrict(test_configuration, test_metadata, set_som_configuration, configure_local_internal_options,
                  truncate_monitored_files, folder_to_monitor, daemons_handler, file_to_monitor):
    '''
    description: Check if the 'som-syscheckd' daemon detects or ignores events in monitored files depending
    ...
    input_description: Different test cases are contained in external YAML file (som_conf.yaml) which
    ...
    monitor = FileMonitor(SOM_LOG_PATH)

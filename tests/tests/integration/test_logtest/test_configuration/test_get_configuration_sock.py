...
import pytest
from som_testing.constants.paths.sockets import ANALYSISD_ANALISIS_SOCKET_PATH
...
def test_get_configuration_sock(test_configuration, test_metadata, set_som_configuration,
    ...
    description: Check analysis Unix socket returns the correct Logtest configuration under different sets of
                 configurations, `som-analisysd` returns the right information from the `rule_test` configuration
                 ...
                 that the loaded fields from the 'som_conf.yaml' file.

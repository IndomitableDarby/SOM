...
def prepare_custom_rules_file(request, test_metadata):
    ...
    shutil.chown(target_rule, SOM_UNIX_USER, SOM_UNIX_GROUP)
...
def configure_local_rules(request, test_configuration):
    """Configure a custom rule for testing. Restart Som is needed for applying the configuration. """
...
def configure_remove_tags(request, test_metadata):
    ...
    file.replace_regex_in_file(test_metadata['remove_tags'], [''] * len(test_metadata['remove_tags']), SOM_CONF_PATH)

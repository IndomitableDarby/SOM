...
from som_testing.utils import configuration, callbacks
...
def test_accepted_values(test_configuration, test_metadata, load_som_basic_configuration, set_som_configuration,
    ...
    test_phases:
        ...
        - test:
            ...
            - Check that som-analysisd is running (it has not been crashed).
        - teardown:
            - Truncate som logs.
    ...
    """
    # Start monitor
    monitor_enabled = file_monitor.FileMonitor(SOM_LOG_PATH)
...
def test_invalid_values(test_configuration, test_metadata, load_som_basic_configuration, set_som_configuration,
                        truncate_monitored_files, daemons_handler):
    """
    description: Check for configuration error and som-analysisd if the EPS limiting configuration has unaccepted
    ...
    test_phases:
        ...
        - test:
            - Restart som-manager service to apply configuration changes.
            - Check that a configuration error is raised when trying to start som-manager.
            - Check that som-analysisd is not running (due to configuration error).
        - teardown:
            - Truncate som logs.
            - Restore initial configuration.
    ...
    """
    # Start monitor
    monitor_error = file_monitor.FileMonitor(SOM_LOG_PATH)
...
def test_missing_configuration(test_configuration, test_metadata, load_som_basic_configuration, set_som_configuration,
    ...
    test_phases:
        ...
        - test:
            ...
            - Check whether the EPS limitation is activated, deactivated or generates a configuration error due to a
              missing label.
            - Check if som-analysisd is running or not (according to the expected behavior).
        - teardown:
            - Truncate som logs.
            - Restore initial configuration.

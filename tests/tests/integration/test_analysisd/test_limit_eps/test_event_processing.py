...
brief: The 'som-analysisd' daemon uses a series of decoders and rules to analyze and interpret logs and events and
...
references:
    - https://documentation.som.com/current/user-manual/ruleset/ruleset-xml-syntax/rules.html#if-sid
'''
import pytest
import time
import re
...
from som_testing.constants.paths.logs import ARCHIVES_LOG_PATH
from som_testing.modules.analysisd import utils, configuration as analysisd_config
from som_testing.modules.remoted import configuration as remoted_config
from som_testing.scripts.syslog_simulator import DEFAULT_MESSAGE_SIZE
from som_testing.tools import thread_executor
from som_testing.tools.simulators import run_syslog_simulator
from som_testing.utils import configuration, file
...
def test_limitation(test_configuration, test_metadata, load_som_basic_configuration, set_som_configuration,
    ...
    test_phases:
        - setup:
            - Load Som light configuration.
            - Apply ossec.conf configuration changes according to the configuration template and use case.
            - Apply custom settings in local_internal_options.conf.
            - Truncate som logs.
            - Restart som-manager service to apply configuration changes.
        - test:
    ...
    parameters:
        ...
        - load_som_basic_configuration:
            type: fixture
            brief: Load basic som configuration.
    ...
    # Check that som-manager is processing syslog events
    ...
    # Check that the som-manager is receiving events but it is not processing them due to the limitation
...
def test_queueing_events_after_limitation(test_configuration, test_metadata, load_som_basic_configuration, set_som_configuration,
                                          configure_local_internal_options, truncate_monitored_files, daemons_handler):
    """
...
def test_dropping_events_when_queue_is_full(test_configuration, test_metadata, load_som_basic_configuration, set_som_configuration,
                                            configure_local_internal_options, truncate_monitored_files, daemons_handler):
    """
...
def test_event_processing_in_order_single_thread(test_configuration, test_metadata, load_som_basic_configuration, set_som_configuration,
                                                 configure_local_internal_options, truncate_monitored_files, daemons_handler):
    """
...
def test_event_processing_in_order_multi_thread(test_configuration, test_metadata, load_som_basic_configuration, set_som_configuration,
                                                configure_local_internal_options, truncate_monitored_files, daemons_handler):
    """
    description: Check that events are processed in order according to the position within the queue, and
        that events that are being received during the blocking phase are being added to the end of the queue when

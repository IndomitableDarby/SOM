...
brief: The 'som-agentd' program is the client-side daemon that communicates with the server.
       The objective is to check how the 'som-agentd' daemon behaves when there are delays
       between connection attempts to the 'som-remoted' daemon using TCP and UDP protocols.
       The 'som-remoted' program is the server side daemon that communicates with the agents.
...
references:
    - https://documentation.som.com/current/user-manual/registering/index.html
...
from som_testing.modules.agentd.configuration import AGENTD_DEBUG, AGENTD_WINDOWS_DEBUG, AGENTD_TIMEOUT
from som_testing.modules.agentd.patterns import AGENTD_TRYING_CONNECT, AGENTD_CONNECTED_TO_SERVER
...
def test_agentd_parametrized_reconnections(test_metadata, set_som_configuration, configure_local_internal_options,
    ...
    assertions:
        - Verify that when the 'som-agentd' daemon initializes, it connects to
          the 'som-remoted' daemon of the manager before reaching the maximum number of attempts.
    ...
    som_log_monitor.start(accumulations = test_metadata['MAX_RETRIES'], callback=callbacks.generate_callback(AGENTD_TRYING_CONNECT,{'IP':'','PORT':''}))
    assert (som_log_monitor.callback_result != None), f'Trying to connect to server message not found expected times'

    #Check number of connected message is the expected
    if test_metadata['ENROLL'] == 'yes':
        som_log_monitor.start(callback=callbacks.generate_callback(AGENTD_CONNECTED_TO_SERVER))

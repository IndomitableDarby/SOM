...
if [ "$TYPE" = "manager" ]; then
    if !(${PWD}/bin/som-logtest-legacy -t > /dev/null 2>&1); then
        exit 1;
    fi
fi
...
if command -v systemctl >/dev/null 2>&1; then
    ...
    if [ "$PARAM_ACTION" = "reload" ]; then
        ...
        while [ $ELAPSED -lt $TIMEOUT ]; do
            STATE=$(systemctl is-active som-$TYPE 2>/dev/null)

            # Exit immediately if service is in a failed or stopped state
            case "$STATE" in
                inactive|failed)
                    echo "Service som-$TYPE is in state '$STATE', cannot reload" >> ${PWD}/logs/active-responses.log
                    exit 1
                    ;;
                active)
                    break
        ...
        if ! systemctl is-active --quiet som-$TYPE; then
            echo "Service som-$TYPE is not active after waiting $TIMEOUT seconds" >> ${PWD}/logs/active-responses.log
            exit 1
        fi
    fi

    systemctl $PARAM_ACTION som-$TYPE
else
    ${PWD}/bin/som-control $PARAM_ACTION
fi

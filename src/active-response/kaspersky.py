#!/usr/bin/env python
 ...
 ###
 # Integration of Som agent with Kaspersky endpoint security for Linux
...
# Sets the file paths
##################################################################################################################
som_path = os.path.abspath(os.path.join(__file__, "../../.."))
som_queue = '{0}/queue/sockets/queue'.format(som_path)
now = time.strftime("%a %b %d %H:%M:%S %Z %Y")
ar_log_file = '{0}/logs/active-responses.log'.format(som_path)
...
def send_msg(som_queue, msg):
	...
	try:
		...
		print('Error: Som must be running.')
		sys.exit(1)
...
def main():
    ar_log()
    set_logger('som-kaspersky', foreground=args.verbose)
    run_kaspersky()

...
/** Som to prelude severity mapping. **/
static const char *(som2prelude_sev[]) = {"info", "info", "info", "info",
                               "low", "low", "low", "low",
...
add_idmef_object(idmef_message_t *msg, const char *object, const char *value)
{
    ...
    if (ret < 0) {
        merror("Som2Prelude: IDMEF: Cannot add object '%s': %s.",
...
err:
    merror("Som2Prelude: %s: IDMEF error: %s.",
...
void OS_PreludeLog(const Eventinfo *lf)
{
    ...
    if ( ret < 0 ) {
        merror("Som2Prelude: Cannot create IDMEF message");
        return;
    }
    ...
    add_idmef_object(idmef, "alert.assessment.impact.severity",
                     ...
                     som2prelude_sev[lf->generated_rule->level]);
    ...
    /* Begin Classification Infomations */
    {
        ...
        if (lf->generated_rule->sigid) {
            ...
            add_idmef_object(idmef, "alert.classification.reference(-1).meaning", "Som Ruleset");

            snprintf(_prelude_data, 256, "https://github.com/som/som/tree/master/ruleset");
         ...
         * that points back to Som ruleset for more infomation.
         */

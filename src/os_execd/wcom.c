...
size_t wcom_dispatch(char *command, char ** output) {
    ...
    } else if (strcmp(rcv_comm, "restart") == 0 || strcmp(rcv_comm, "restart-som") == 0) {
...
size_t wcom_restart(char ** output) {
    ...
    if (lock <= 0) {
        ...
        snprintf(command, sizeof(command), "%s/%s", AR_BINDIR, "restart-som.exe");
        char *cmd[2] = { command, NULL };
        char *cmd_parameters = "{\"version\":1,\"origin\":{\"name\":\"\",\"module\":\"som-execd\"},\"command\":\"add\",\"parameters\":{\"extra_args\":[],\"alert\":{},\"program\":\"restart-som.exe\"}}";
...
size_t wcom_check_manager_config(char **output) {
    static const char *daemons[] = {"bin/som-authd", "bin/som-remoted",
                                    "bin/som-execd", "bin/som-analysisd", "bin/som-logcollector",
                                    "bin/som-integratord", "bin/som-syscheckd", "bin/som-maild",
                                    "bin/som-modulesd", "bin/som-clusterd", "bin/som-agentlessd",
                                    "bin/som-integratord", "bin/som-dbd", "bin/som-csyslogd", NULL
                                    };

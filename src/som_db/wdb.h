...
/* Look for a configuration assessment entry in Som DB. Returns 1 if found, 0 if not, or -1 on error. (new) */
...
/* Look for a scan configuration assessment entry in Som DB. Returns 1 if found, 0 if not, or -1 on error. (new) */
...
/* Look for a scan policy entry in Som DB. Returns 1 if found, 0 if not, or -1 on error. (new) */
int wdb_sca_policy_find(wdb_t * wdb, char *id, char * output);

/* Gets the result of all checks in Som DB. Returns 1 if found, 0 if not, or -1 on error. (new) */
...
/* Gets the result of all policies in Som DB. Returns 1 if found, 0 if not, or -1 on error. (new) */
 ...
 *
 * @param [in] input The chunk obtained from SomDB to be parsed.
 ...
 * a message to process in Som-db process.
 *
 * @param wdb The Global struct database.
 * @param input Buffer input
 * @param output Buffer output, on success responses are:
...
/**
 * @brief Function to get the information of a particular agent stored in Som DB.
 *
 * @param wdb The Global struct database.
 * @param id Agent id.
...
/**
 * @brief Method to read the internal Som-db configuration.
 *

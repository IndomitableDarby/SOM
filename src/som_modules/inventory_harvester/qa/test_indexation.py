...
TEST_DATA_ROOT = Path("som_modules/inventory_harvester/qa/test_data")
QUEUE_DIR = Path("queue")
TEMPLATE_ROOT = Path("som_modules/inventory_harvester/indexer/template")
...
def discover_templates():
    ...
    return [path for path in TEMPLATE_ROOT.rglob("som-states-*.json") if valid(path)]
...
def create_index_from_template(template_path: Path):
    ...
    if pattern and "*" in pattern:
        if "fim-files" in template_name:
            index_name = "som-states-fim-files-cluster01"
        elif "fim-registries" in template_name:
            index_name = "som-states-fim-registries-cluster01"
        else:
...
def opensearch():
    ...
    client.containers.run(
        ...
        environment={
            ...
            "OPENSEARCH_INITIAL_ADMIN_PASSWORD": "SomTest99$",
        },
        name="opensearch",
        stdout=True,
        stderr=True,
...
def test_data_indexation(opensearch, test_folder):
    ...
    is_som_db = "som_db" in str(test_path)

    cmd = (
        Path(
            "build/som_modules/inventory_harvester/testtool/inventory_harvester_testtool"
        )
        if Path(
            "build/som_modules/inventory_harvester/testtool/inventory_harvester_testtool"
        ).exists()
        else Path(
            "som_modules/inventory_harvester/build/testtool/inventory_harvester_testtool"
        )
    )
    ...
    try:
        ...
        if is_som_db:
            LOGGER.info("Loading templates manually for 'som_db' test case...")
            load_all_templates()

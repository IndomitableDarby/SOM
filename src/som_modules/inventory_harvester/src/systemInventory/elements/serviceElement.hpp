/*
 * Som Inventory Harvester - Service element
...
public:
    ...
    static DataHarvester<InventoryServiceHarvester> build(TContext* data)
    {
        ...
        // Som cluster information
        auto& instancePolicyManager = PolicyHarvesterManager::instance();
        element.data.som.cluster.name = instancePolicyManager.getClusterName();
        if (instancePolicyManager.getClusterStatus())
        {
            element.data.som.cluster.node = instancePolicyManager.getClusterNodeName();
        }

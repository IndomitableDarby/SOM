            ...
            if (Synchronization::VerifySyncMsgBuffer(verifier))
            {
                ...
                if (data->data_type() == Synchronization::DataUnion_state)
                {
                    ...
                    if (data->data_as_state()->attributes_as_syscollector_packages() ||
                        ...
                        data->data_as_state()->attributes_as_syscollector_browser_extensions() ||
                        data->data_as_state()->attributes_as_syscollector_services())
                    {
                ...
                else if (data->data_type() == Synchronization::DataUnion_integrity_clear)
                {
                    ...
                    if (attributesType.compare("syscollector_packages") == 0 ||
                        ...
                        attributesType.compare("syscollector_browser_extensions") == 0 ||
...
void InventoryHarvesterFacade::initSomDBAgentEventSubscription()
{
...
// void InventoryHarvesterFacade::initSomDBFimEventSubscription()
// {
...
void InventoryHarvesterFacade::initSomDBInventoryEventSubscription()
{
    ...
    const nlohmann::json& configuration)
{
    try
    {
        ...
        initSomDBAgentEventSubscription();
        initSomDBInventoryEventSubscription();
        // initSomDBFimEventSubscription(); // DISABLED: FIM events are no longer processed.

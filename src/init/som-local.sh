...
AUTHOR="Som Inc."
DAEMONS="som-modulesd som-monitord som-logcollector som-syscheckd som-analysisd som-maild som-execd som-db som-agentlessd som-integratord som-dbd som-csyslogd"
...
start_service()
{
    ...
    TEST=$(${DIR}/bin/som-logtest-legacy -t  2>&1)
    echo $TEST

    if [ ! -z "$TEST" ]; then
        echo "som-analysisd: Configuration error. Exiting."
    ...
    if [ $? = 0 ]; then
        ...
        ${DIR}/ossec-agent/bin/som-control start
    fi
...
stop_service()
{
    ...
    if [ $? = 0 ]; then
        ...
        ${DIR}/ossec-agent/bin/som-control stop
    fi

...
# Install functions for Som
# Som.com (https://github.com/som)
...
InstallSELinuxPolicyPackage(){
    ...
    if command -v semodule > /dev/null && command -v getenforce > /dev/null; then
        if [ -f selinux/som.pp ]; then
            if [ $(getenforce) != "Disabled" ]; then
                cp selinux/som.pp /tmp && semodule -i /tmp/som.pp
                rm -f /tmp/som.pp
                semodule -e som
            fi
        fi
...
SomUpgrade()
{
    ...
    if [ -f "$PREINSTALLEDDIR/var/db/global.db" ]; then
        ...
        if [ -f "$PREINSTALLEDDIR/queue/db/global.db" ]; then
            ...
            chown som:som $PREINSTALLEDDIR/queue/db/global.db
    ...
    # Remove existing SQLite databases for Som DB, only if upgrading from 3.2..3.6
    ...
    if [ -d $PREINSTALLEDDIR/logs/ossec ]; then
        if [ "$(ls -A $PREINSTALLEDDIR/logs/ossec)" ]; then
            mv -f $PREINSTALLEDDIR/logs/ossec/* $PREINSTALLEDDIR/logs/som
        fi
        rm -rf $PREINSTALLEDDIR/logs/ossec
    fi

    # Remove deprecated Som tools
    ...
    if (grep "^ossec:" /etc/group > /dev/null 2>&1) || (dscl . -read /Groups/ossec > /dev/null 2>&1)  ; then
        if [ "X$1" = "Xserver" ]; then
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -user root -print0 | xargs -0 chown root:som
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -print0 | xargs -0 chown som:som
        else
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -user root -exec chown root:som {} \;
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -exec chown som:som {} \;
        fi
    fi

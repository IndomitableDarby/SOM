...
# som-control        This shell script takes care of starting
...
AUTHOR="Som Inc."
USE_JSON=false
DAEMONS="som-clusterd som-modulesd som-monitord som-logcollector som-remoted som-syscheckd som-analysisd som-maild som-execd som-db som-authd som-agentlessd som-integratord som-dbd som-csyslogd som-apid"
OP_DAEMONS="som-clusterd som-maild som-agentlessd som-integratord som-dbd som-csyslogd"
...
start_service()
{
    ...
    if [ $USE_JSON = false ]; then
        echo "Starting Som $VERSION..."
    fi

    TEST=$(${DIR}/bin/som-logtest-legacy -t  2>&1 | grep "ERROR")
    if [ ! -z "$TEST" ]; then
        if [ $USE_JSON = true ]; then
        ...
        touch ${DIR}/var/run/som-analysisd.failed
        exit 1;
    fi
    ...
    for i in ${SDAEMONS}; do
        ## If som-maild is disabled, don't try to start it.
        ...
        ## If som-clusterd is disabled, don't try to start it.
        ...
        ## If som-authd is disabled, don't try to start it.
...
pstatus()
{
    ...
    if [ $? = 0 ]; then
        for pid in `cat ${DIR}/var/run/${pfile}-*.pid 2>/dev/null`; do
            ...
            if [ ! $? = 0 ]; then
                if [ $USE_JSON = false ]; then
                    echo "${pfile}: Process ${pid} not used by Som, removing..."
                fi
...
restart)
    ...
    ;;
reload)
    DAEMONS=$(echo $DAEMONS | sed 's/som-execd//')
    restart_service
    ;;
status)
    lock

...
AUTHOR="Som Inc."
DAEMONS="som-modulesd som-logcollector som-syscheckd som-agentd som-execd"
...
check_folders()
{
    ...
    if [ ! -d $ALERTS_FOLDER ]
    then
        if rm -rf $ALERTS_FOLDER && mkdir -p $ALERTS_FOLDER && chown som:som $ALERTS_FOLDER && chmod 770 $ALERTS_FOLDER
        then
...
restart)
    ...
    ;;
reload)
    DAEMONS=$(echo $DAEMONS | sed 's/som-agentd//')
    restart_service
    # Signal agentd (SIGUSR1) to reload (reconnects execd)
    pid=`cat ${DIR}/var/run/som-agentd-*.pid`
    kill -USR1 $pid
    ;;
status)
    lock

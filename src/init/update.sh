...
# Shell script update functions for Som
...
##########
# Checks if Som is installed by taking the installdir from the services
...
getPreinstalledDirByType()
{
    ...
    if hash ps 2>&1 > /dev/null && hash grep 2>&1 > /dev/null && [ -n "$(ps -e | egrep ^\ *1\ .*systemd$)" ]; then
        ...
        SED_EXTRACT_PREINSTALLEDDIR="s/^ExecStart=\/usr\/bin\/env \(.*\)\/bin\/som-control start$/\1/p"
        ...
        # Get the unit file and extract the Som home path
        PREINSTALLEDDIR=$(systemctl cat som-${type}.service 2>/dev/null | sed -n "${SED_EXTRACT_PREINSTALLEDDIR}")
        ...
        if [ -f /usr/lib/systemd/system/som-${type}.service ]; then
            SERVICE_UNIT_PATH=/usr/lib/systemd/system/som-${type}.service
        fi
        # Others
        if [ -f /etc/systemd/system/som-${type}.service ]; then
            SERVICE_UNIT_PATH=/etc/systemd/system/som-${type}.service
        fi
    ...
    if [ "X${NUNAME}" = "XDarwin" ]; then
        if [ -f /Library/StartupItems/SOM/SOM ]; then
            PREINSTALLEDDIR=`sed -n 's/^ *//; s/^\s*\(.*\)\/bin\/som-control start$/\1/p' /Library/StartupItems/SOM/SOM`
            if [ -d "$PREINSTALLEDDIR" ]; then
                return 0;
            else
                return 1;
    ...
    if [ "X${UN}" = "XOpenBSD" -o "X${UN}" = "XNetBSD" -o "X${UN}" = "XFreeBSD" -o "X${UN}" = "XDragonFly" ]; then
        # Checking for the presence of som-control on rc.local
        grep som-control /etc/rc.local > /dev/null 2>&1
        if [ $? = 0 ]; then
            PREINSTALLEDDIR=`sed -n 's/^\(.*\)\/bin\/som-control start$/\1/p' /etc/rc.local`
            if [ -d "$PREINSTALLEDDIR" ]; then
                return 0;
            else
                return 1;
        ...
        if [ -e "/etc/rc.d/rc.local" ]; then
            grep som-control /etc/rc.d/rc.local > /dev/null 2>&1
            if [ $? = 0 ]; then
                PREINSTALLEDDIR=`sed -n 's/^\(.*\)\/bin\/som-control start$/\1/p' /etc/rc.d/rc.local`
                if [ -d "$PREINSTALLEDDIR" ]; then
                    return 0;
                else
                    return 1;
...
##########
# Checks if Som is installed in the specified path by searching for the control binary.
#
# isWazuhInstalled()
##########
isWazuhInstalled()
{
    if [ -f "${1}/bin/som-control" ]; then
        return 0;
...
##########
# Checks if Som is installed by trying with each installation type.
# If it finds an installation, it sets the PREINSTALLEDDIR variable.
# After that it checks if Som is truly installed there, if it is installed it returns TRUE.
# If it isn't installed continue searching in other installation types and replacing PREINSTALLEDDIR variable.
# It returns FALSE if Som isn't installed in any of this.
#
# getPreinstalledDir()
##########
getPreinstalledDir()
{
    # Checking ossec-init.conf for old som versions
    ...
    # Getting preinstalled dir for Som manager and hibrid installations
    pidir_service_name="som-manager"
    if getPreinstalledDirByType && isWazuhInstalled $PREINSTALLEDDIR; then
        return 0;
    fi

    # Getting preinstalled dir for Som agent installations
    pidir_service_name="som-agent"
    if getPreinstalledDirByType && isWazuhInstalled $PREINSTALLEDDIR; then
        return 0;
    fi

    # Getting preinstalled dir for Som local installations
    pidir_service_name="som-local"
    if getPreinstalledDirByType && isWazuhInstalled $PREINSTALLEDDIR; then
        return 0;
    fi
...
getPreinstalledType()
{
    ...
    if [ -f "${OSSEC_INIT}" ]; then
        ...
        TYPE=`$PREINSTALLEDDIR/bin/som-control info -t`
    fi
...
getPreinstalledVersion()
{
    ...
    if [ -f "${OSSEC_INIT}" ]; then
        ...
        VERSION=`$PREINSTALLEDDIR/bin/som-control info -v`
    fi
...
{
    NAME=""
    # Checking ossec-init.conf for old som versions. New versions
...
UpdateStartOSSEC()
{
    ...
    if [ `stat /proc/1/exe 2> /dev/null | grep "systemd" | wc -l` -ne 0 ]; then
        ...
        $INSTALLDIR/bin/som-control start
    fi
}
...
UpdateStopOSSEC()
{
    ...
    if [ `stat /proc/1/exe 2> /dev/null | grep "systemd" | wc -l` -ne 0 ]; then
        systemctl stop som-$TYPE
    elif [ `stat /proc/1/exe 2> /dev/null | grep "init.d" | wc -l` -ne 0 ]; then
        service som-$TYPE stop
    fi

    # Make sure Som is stopped
    if [ "X$PREINSTALLEDDIR" = "X" ]; then
        getPreinstalledDir
    fi
    ...
    if [ -f "$PREINSTALLEDDIR/bin/ossec-control" ]; then
        ...
        $PREINSTALLEDDIR/bin/som-control stop > /dev/null 2>&1
    fi
   ...
   # We also need to remove all syscheck queue file (format changed)
    ...
    # Deleting plain-text agent information if exists (it was migrated to Som DB in v4.1)
    if [ -d "$PREINSTALLEDDIR/queue/agent-info" ]; then
        rm -rf $PREINSTALLEDDIR/queue/agent-info > /dev/null 2>&1
    fi

    # Deleting plain-text rootcheck information if exists (it was migrated to Som DB in v4.1)
...
UpdateOldVersions()
{
    if [ "$INSTYPE" = "server" ]; then
        ...
        DEPRECATED=`cat ./src/init/som/deprecated_ruleset.txt`
    ...
    # If it is Som 2.0 or newer, exit
    ...
    if [ ! "$INSTYPE" = "agent" ]; then
        ...
        chown root:som $BACKUP_RULESET > /dev/null 2>&1

        # Backup decoders: Som v1.0.1 to v1.1.1
        ...
        # Backup decoders: Som v1.0 and OSSEC

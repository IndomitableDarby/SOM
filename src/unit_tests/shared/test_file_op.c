...
{
    char *argv0 = "/usr/share/som/bin/test";
    ...
    expect_string(__wrap_stat, __file, "/usr/share/som");
    will_return(__wrap_stat, &stat_buf);
    will_return(__wrap_stat, 0);

    val = w_homedir(argv0);
    assert_string_equal(val, "/usr/share/som");
    free(val);
}
...
void test_w_homedir_env_var(void **state)
{
    ...
    will_return(__wrap_getenv, "/home/som");

    expect_string(__wrap_stat, __file, "/home/som");
    will_return(__wrap_stat, &stat_buf);
    will_return(__wrap_stat, 0);

    val = w_homedir(argv0);
    assert_string_equal(val, "/home/som");
    free(val);
}
...
void test_w_homedir_stat_fail(void **state)
{
    ...
    expect_string(__wrap__merror_exit, formatted_msg, "(1108): Unable to find som install directory. Export it to WAZUH_HOME environment variable.");

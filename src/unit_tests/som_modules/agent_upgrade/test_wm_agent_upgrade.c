...
#include "../../wrappers/som/wazuh_modules/wm_agent_upgrade_agent_wrappers.h"
...
void test_wm_agent_upgrade_dump_enabled(void **state)
{
    ...
    #ifdef TEST_SERVER
    os_strdup("som.com/packages", config->manager_config.wpk_repository);
    ...
    assert_string_equal(cJSON_GetObjectItem(conf, "wpk_repository")->valuestring, "som.com/packages");
    #else

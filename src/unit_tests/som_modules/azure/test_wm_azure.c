...
static int setup_module() {
    ...
    const char *string =
        ...
        "    <tenantdomain>som.onmicrosoft.com</tenantdomain>\n"
        "    <request>\n"
...
void test_interval_execution(void **state) {
    ...
    *state = module_data;
    ...
    for (int iterations = 0; iterations <= TEST_MAX_DATES; ++iterations) {
        ...
        expect_string(__wrap__mtinfo, formatted_msg, "Starting Log Analytics collection for the domain 'som.onmicrosoft.com'.");
        ...
        expect_string(__wrap__mtinfo, formatted_msg, "Finished Log Analytics collection for the domain 'som.onmicrosoft.com'.");
    }

...
find_library(SOMEXT NAMES somext HINTS "${SRC_FOLDER}")
set(uname "Win32")

if(NOT SOMEXT)
  message(FATAL_ERROR "SOMEXT is set to '${SOMEXT}', but did not find any file matching ${SRC_FOLDER}/${CMAKE_FIND_LIBRARY_PREFIXES}somext${CMAKE_FIND_LIBRARY_SUFFIXES}")
  message(FATAL_ERROR "libsomext not found in ${SRC_FOLDER} Aborting...")
endif()

# Find the som sysinfo library
...
add_definitions(-DTEST_WINAGENT -DDEBUG -DENABLE_AUDIT -D_WIN32_WINNT=0x600 -DSOM_UNIT_TESTING)
...
target_link_libraries(DEPENDENCIES_O ${SOMLIB} ${SOMEXT} ${PTHREAD} ${SYSINFO} ${STATIC_CMOCKA} wsock32 wevtapi shlwapi comctl32 advapi32 kernel32 psapi gdi32 iphlpapi ws2_32 crypt32 wintrust)

# Set tests dependencies
# Use --start-group and --end-group to handle circular dependencies
set(TEST_DEPS -Wl,--start-group ${SOMLIB} ${SOMEXT} ${SYSINFO} DEPENDENCIES_O -Wl,--end-group ${PTHREAD} ${STATIC_CMOCKA} wsock32 wevtapi shlwapi comctl32 advapi32 kernel32 psapi gdi32 iphlpapi ws2_32 crypt32 -fprofile-arcs -ftest-coverage)
set(TEST_EVENT_DEPS -Wl,--start-group ${SOMLIB} ${SOMEXT} ${SYSINFO} DEPENDENCIES_O -Wl,--end-group ${PTHREAD} ${STATIC_CMOCKA} wsock32 wevtapi shlwapi comctl32 advapi32 kernel32 psapi gdi32 iphlpapi ws2_32 crypt32 -fprofile-arcs -ftest-coverage)

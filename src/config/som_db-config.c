...
#include "shared.h"
#include "config.h"
#include "config/som_db-config.h"
...
int Read_somDB(const OS_XML *xml, XML_NODE child_node) {
    ...
    return Read_SomDB_Backup(xml, node, WDB_GLOBAL_BACKUP);
}

int Read_SomDB_Backup(const OS_XML *xml, xml_node * node, int const BACKUP_NODE) {

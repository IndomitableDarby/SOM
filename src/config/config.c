                              ...
                              void *d2)
{
    ...
    const char *oswmodule = "wodle";                            /* Wodle - Som Module  */
    ...
    const char *osgcp_pub = "gcp-pubsub";                       /* Google Cloud PubSub - Som Module */
    const char *osgcp_bucket = "gcp-bucket";                    /* Google Cloud Bucket - Som Module */
    const char *wlogtest = "rule_test";                         /* Som Logtest */
    const char *agent_upgrade = "agent-upgrade";                /* Agent Upgrade Module */
    const char *task_manager = "task-manager";                  /* Task Manager Module */
    const char *wazuh_db = "wdb";                               /* Som-DB Daemon */
#ifndef WIN32
        ...
        if (chld_node && (strcmp(node[i]->element, osglobal) == 0)) {
            ...
            if ((modules & CWMODULE)) {
                mwarn(
                    ...
                    "See https://documentation.som.com",
                    osvulndetector,
                    osvulndetection);

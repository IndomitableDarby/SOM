...
!define REVISION "rc2"
!define NAME "Som"
!define SERVICE "SomSvc"

; output file
!ifndef OutFile
    !define OutFile "som-agent-${VERSION}.exe"
!endif

Var is_upgrade

Name "${NAME} Windows Agent v${VERSION}"
BrandingText "Copyright (C) 2015, Som Inc."
OutFile "${OutFile}"

VIProductVersion "4.14.0.0"
VIAddVersionKey ProductName "${NAME}"
VIAddVersionKey CompanyName "Som Inc."
VIAddVersionKey LegalCopyright "2023 - Som Inc."
VIAddVersionKey FileDescription "Som Agent installer"
VIAddVersionKey FileVersion "${VERSION}"
VIAddVersionKey ProductVersion "${VERSION}"
VIAddVersionKey InternalName "Som Agent"
...
Section "Som Agent (required)" MainSec
    ...
    ; install files
    File som-agent.exe
    File som-agent-eventchannel.exe
    ...
    File /oname=active-response\bin\restart-som.exe restart-som.exe
    ...
    File /oname=libsomext.dll ..\libsomext.dll
    File /oname=libsomshared.dll ..\libsomshared.dll
    ...
    File /oname=syscollector.dll ..\som_modules\syscollector\build\bin\syscollector.dll
    File /oname=libfimdb.dll ..\syscheckd/build/bin/libfimdb.dll
    File /oname=queue\syscollector\norm_config.json ..\som_modules\syscollector\norm_config.json
    File VERSION.json
    ...
    ; use appropriate version of "som-agent.exe"
    ${If} ${AtLeastWinVista}
        Delete "$INSTDIR\som-agent.exe"
        Rename "$INSTDIR\som-agent-eventchannel.exe" "$INSTDIR\som-agent.exe"
    ${Else}
        Delete "$INSTDIR\som-agent-eventchannel.exe"
    ${Endif}
    ...
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "Publisher" "Som"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "DisplayIcon" '"$INSTDIR\favicon.ico"'
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "HelpLink" "https://som.com"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "URLInfoAbout" "https://som.com"
    ...
    ServiceInstall:
        nsExec::ExecToLog '"$INSTDIR\som-agent.exe" install-service'
        Pop $0
        ${If} $0 <> 1
            MessageBox MB_ABORTRETRYIGNORE|MB_ICONSTOP "$\r$\n\
...
Section "Uninstall"
    ...
    ServiceUninstall:
        nsExec::ExecToLog '"$INSTDIR\som-agent.exe" uninstall-service'
        Pop $0
        ${If} $0 <> 1
            MessageBox MB_ABORTRETRYIGNORE|MB_ICONSTOP "$\r$\n\

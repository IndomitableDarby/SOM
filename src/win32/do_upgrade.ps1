...
function remove_upgrade_files {
    ...
    Remove-Item -Path ".\som-agent*.msi" -ErrorAction SilentlyContinue
    Remove-Item -Path ".\do_upgrade.ps1" -ErrorAction SilentlyContinue
}


function get_som_installation_directory {
    Start-NativePowerShell {
        $path1 = "HKLM:\SOFTWARE\WOW6432Node\Som, Inc.\Som Agent"
        $key1 = "SomInstallDir"
        ...
        try {
            $SomInstallDir = (Get-ItemProperty -Path $path1 -ErrorAction SilentlyContinue).$key1
        }
        catch {
            $SomInstallDir = $null
        }

        if ($null -eq $SomInstallDir) {
            try {
                $SomInstallDir = (Get-ItemProperty -Path $path2 -ErrorAction SilentlyContinue).$key2
            }
            catch {
                $SomInstallDir = $null
            }
        ...
        if ($null -eq $SomInstallDir) {
            Write-output "$(Get-Date -format u) - Couldn't find Som in the registry. Upgrade will assume current path is correct" >> .\upgrade\upgrade.log
            $SomInstallDir = (Get-Location).Path.TrimEnd('\')
        }

        return $SomInstallDir
    }
}

# Check process status
function check-process {
    $process_id = (Get-Process som-agent).id
    $counter = 10
    while($process_id -eq $null -And $counter -gt 0) {
        $counter--
        Start-Service -Name "Som"
        Start-Sleep 2
        $process_id = (Get-Process som-agent).id
    }
...
# Check new version and restart the Som service
function check-installation {
    $actual_version = get-version
    $counter = 5
    while($actual_version -eq $current_version -And $counter -gt 0) {
        write-output "$(Get-Date -format u) - Waiting for the Som-Agent installation to end." >> .\upgrade\upgrade.log
        $counter--
        Start-Sleep 2
        $actual_version = get-version
    }
    write-output "$(Get-Date -format u) - Starting Som-Agent service." >> .\upgrade\upgrade.log
    Start-Service -Name "Som"
}

# Function to extract the version from the MSI using msiexec
function get_msi_version {
    $msiPath = (Get-Item ".\som-agent*.msi").FullName
...
function install {
    ...
    try {
        $msiPath = (Get-Item ".\som-agent*.msi").Name
...
# Check that the Som installation runs on the expected path
$somDir = get_som_installation_directory
$normalizedSomDir = $somDir.TrimEnd('\')
$currentDir = (Get-Location).Path.TrimEnd('\')

if ($normalizedSomDir -ne $currentDir) {
    Write-Output "$(Get-Date -format u) - Current working directory is not the Som installation directory. Aborting." >> .\upgrade\upgrade.log
...
function Get-AgentStatus {
    Select-String -Path '.\som-agent.state' -Pattern "^status='(.+)'" | %{$_.Matches[0].Groups[1].value}
}

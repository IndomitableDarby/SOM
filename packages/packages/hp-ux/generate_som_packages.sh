...
source_directory=/var/som-sources
...
# Needed variables to build Som with custom GCC and cmake
...
build_environment() {
    ...
    curl -SO https://packages.som.com/utils/gcc/gcc_9.4_HPUX_build.tar.gz
    ...
    cd ${build_tools_path}
    curl -SO https://packages.som.com/utils/cmake/cmake_3.22.2_HPUX_build.tar.gz
...
compute_version_revision() {
    som_version="$(awk -F'"' '/"version"[ \t]*:/ {print $4}' $source_directory/VERSION.json)"

    if [ -z "$som_revision" ]; then
        stage_value=$(awk -F'"' '/"stage"[ \t]*:/ {print $4}' $source_directory/VERSION.json)
        som_revision=$(echo "$stage_value" | sed 's/[^0-9]*\([0-9][0-9]*\).*/\1/')
    fi

    # Add commit hash to the VERSION.json file
    short_commit_hash=$(/usr/local/bin/curl -s "https://api.github.com/repos/som/som/commits/${som_branch}" | awk -F '"' '/"sha":/ {print substr($4, 1, 7); exit}')
    ...
    echo ${som_revision} > /tmp/REVISION
...
download_source() {
    ...
    /usr/local/bin/curl -L -o "/som.zip" "https://github.com/som/som/archive/${som_branch}.zip"
    /usr/local/bin/unzip /som.zip
    rm -rf ${source_directory}
    mv som-* ${source_directory}
    compute_version_revision
}

check_version() {
    som_version="v$(awk -F'"' '/"version"[ \t]*:/ {print $4}' $source_directory/VERSION.json)"
...
compile() {
    echo "Compiling code"
    # Compile and install som
    cd ${source_directory}/src
    config
    check_version
    gmake deps RESOURCES_URL=https://packages.som.com/deps/${deps_version} TARGET=agent
...
create_package() {
    ...
    som_version=`echo "${som_version}" | cut -d v -f 2`
    pkg_tar_file="som-agent-${som_version}-${som_revision}-hpux-11v3-ia64.tar"
    tar cvpf ${target_dir}/${pkg_tar_file} ${install_path} /sbin/init.d/som-agent /sbin/rc2.d/S97som-agent /sbin/rc3.d/S97som-agent
...
clean() {
    ...
    find /sbin -name "*som-agent*" -exec rm {} \;
    userdel som
    groupdel som
...
show_help() {
    ...
    echo "    -r, --revision Revision to naming Som agent package."

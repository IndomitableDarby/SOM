...
# Modified by Santiago Bassett (http://www.som.com) - Feb 2016
...
UPGRADE_FILE_FLAG="${DIR}/SOM_PKG_UPGRADE"
...
if [ -z "${upgrade}" ]; then
    echo "Generating Som configuration for a fresh installation."
    ...
    if [ -f "${INSTALLATION_SCRIPTS_DIR}/gen_ossec.sh" ]; then
        ...
        chown root:som ${DIR}/etc/ossec.conf
...
if [[ $(dscl . -read /Groups/ossec) ]]; then
    echo "Changing group from Ossec to Som"
    find ${DIR}/ -group ossec -user root -exec chown root:som {} \ > /dev/null 2>&1 || true
    if [[ $(dscl . -read /Users/ossec) ]]; then
        echo "Changing user from Ossec to Som"
        find ${DIR}/ -group ossec -user ossec -exec chown som:som {} \ > /dev/null 2>&1 || true
...
if [ -n "${upgrade}" ] && [ -n "${restart}" ]; then
    echo "Restarting Som..."
    launchctl bootstrap system /Library/LaunchDaemons/com.som.agent.plist
fi

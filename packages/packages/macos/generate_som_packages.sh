#!/bin/bash
set -e
# Program to build and package OSX som-agent
...
ARCH="intel64"
SOM_SOURCE_REPOSITORY="https://github.com/som/som"
SERVICE_PATH="/Library/LaunchDaemons/com.som.agent.plist"
STARTUP_PATH="/Library/StartupItems/SOM/StartupParameters.plist"
LAUNCHER_SCRIPT_PATH="/Library/StartupItems/SOM/Som-launcher"
STARTUP_SCRIPT_PATH="/Library/StartupItems/SOM/SOM"
...
function clean_and_exit() {
    ...
    if [ -z "$BRANCH_TAG" ]; then
        make -C $SOM_PATH/src clean clean-deps
    fi
...
function sign_binaries() {
    if [ ! -z "${KEYCHAIN}" ] && [ ! -z "${CERT_APPLICATION_ID}" ] ; then
        ...
        # Sign every single binary in Som's installation. This also includes library files.
...
function prepare_building_folder() {
    ...
    version="$1"
    pkg_final_name="$2"
    build_info_file="${SOM_PACKAGES_PATH}/specs/build-info.json"
    preinstall_script="${SOM_PACKAGES_PATH}/package_files/preinstall.sh"
    postinstall_script="${SOM_PACKAGES_PATH}/package_files/postinstall.sh"
    packaged_directory=$CURRENT_PATH/som-agent/payload

    if [ -d "$CURRENT_PATH/som-agent" ]; then

        echo "\nThe som agent building directory is present on this machine."
        echo "Removing it from the system."

        rm -rf $CURRENT_PATH/som-agent
    fi

    munkipkg --create --json $CURRENT_PATH/som-agent

    cp -f $build_info_file $CURRENT_PATH/som-agent/

    sed -i '' "s|VERSION|$version|g" $CURRENT_PATH/som-agent/$(basename $build_info_file)
    sed -i '' "s|PACKAGE_NAME|$pkg_final_name|g" $CURRENT_PATH/som-agent/$(basename $build_info_file)

    cp $preinstall_script $CURRENT_PATH/som-agent/scripts/preinstall
    cp $postinstall_script $CURRENT_PATH/som-agent/scripts/postinstall

    sed -i '' "s|PACKAGE_ARCH|$ARCH|g" $CURRENT_PATH/som-agent/scripts/preinstall
...
function build_package() {
    ...
    if [ -n "$BRANCH_TAG" ]; then
        ...
        SOM_PATH="${SOURCES_DIRECTORY}/som"
        git clone --depth=1 -b ${BRANCH_TAG} ${SOM_SOURCE_REPOSITORY} "${SOM_PATH}"
    else
        SOM_PATH="${CURRENT_PATH}/../.."
    fi
    short_commit_hash="$(cd "${SOM_PATH}" && git rev-parse --short=7 HEAD)"

    export CONFIG="${SOM_PATH}/etc/preloaded-vars.conf"
    SOM_PACKAGES_PATH="${SOM_PATH}/packages/macos"
    ENTITLEMENTS_PATH="${SOM_PACKAGES_PATH}/entitlements.plist"

    VERSION="$(awk -F'"' '/"version"[ \t]*:/ {print $4}' $SOM_PATH/VERSION.json)"

    # Define output package name
    if [ $IS_STAGE == "no" ]; then
        pkg_name="som-agent_${VERSION}-${REVISION}_${ARCH}_${short_commit_hash}"
    else
        pkg_name="som-agent-${VERSION}-${REVISION}.${ARCH}"
    fi

    if [ -d "${INSTALLATION_PATH}" ]; then

        echo "\nThe som agent is already installed on this machine."
        echo "Removing it from the system."

        ${CURRENT_PATH}/uninstall.sh
    fi

    ${SOM_PACKAGES_PATH}/package_files/build.sh "${INSTALLATION_PATH}" "${SOM_PATH}" ${JOBS} ${DEBUG} ${MAKE_COMPILATION}
    ...
    # create package
    if munkipkg $CURRENT_PATH/som-agent ; then
        echo "The som agent package for macOS has been successfully built."
        mv $CURRENT_PATH/som-agent/build/* $DESTINATION/
        symbols_pkg_name="som-agent-debug-symbols-${VERSION}-${REVISION}.${ARCH}-macos"
        cp -R "${SOM_PATH}/src/symbols"  "${DESTINATION}"

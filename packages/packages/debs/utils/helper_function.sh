...
setup_build(){
    ...
    cp -pr ${specs_path}/som-${BUILD_TARGET}/debian ${sources_dir}/debian
...
get_package_and_checksum(){
    som_version="$1"
    short_commit_hash="$2"
    base_name="som-${BUILD_TARGET}_${som_version}-${REVISION}"
    symbols_base_name="som-${BUILD_TARGET}-dbg_${som_version}-${REVISION}"
    ...
    if [[ "${checksum}" == "yes" ]]; then
        ...
        cd ${pkg_path} && sha512sum som-${BUILD_TARGET}_*deb > /var/local/som/${deb_file}.sha512
        sha512sum som-${BUILD_TARGET}-*deb > /var/local/som/${symbols_deb_file}.sha512
    fi

    find ${pkg_path} -type f -name "som-${BUILD_TARGET}_*deb" -exec mv {} /var/local/som/${deb_file} \;
    find ${pkg_path} -type f -name "som-${BUILD_TARGET}-dbg_*deb" -exec mv {} /var/local/som/${symbols_deb_file} \;
}

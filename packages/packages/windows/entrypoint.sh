...
# Compile the som agent for Windows
...
if [ -z "${BRANCH}"]; then
    mkdir /som-local-src
    cp -r /local-src/. /som-local-src
else
    git clone --depth=1 https://github.com/som/som.git -b ${BRANCH}
fi
...
bash -c "make -C /som*/src deps TARGET=winagent ${FLAGS}"
bash -c "make -C /som*/src TARGET=winagent ${FLAGS}"

rm -rf /som*/src/external

zip -r /shared/${ZIP_NAME} /som*

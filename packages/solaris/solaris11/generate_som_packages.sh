...
# Som Solaris 11 Package builder.

set -x
REPOSITORY="https://github.com/som/som"
som_branch="main"
...
set_control_binary() {
    if [ -e ${SOURCE}/VERSION.json ]; then
        som_version="v$(sed -n 's/.*"version"[ \t]*:[ \t]*"\([^"]*\)".*/\1/p' ${SOURCE}/VERSION.json)"
        number_version=`echo "${som_version}" | cut -d v -f 2`
...
build_environment() {
    ...
    curl -LO http://packages-dev.som.com/deps/solaris/precompiled-solaris-gcc-5.5.0.tar.gz
    ...
    curl -LO http://packages-dev.som.com/deps/solaris/precompiled-solaris-cmake-3.18.3.tar.gz
...
compute_version_revision() {
    som_version="$(sed -n 's/.*"version"[ \t]*:[ \t]*"\([^"]*\)".*/\1/p' ${SOURCE}/VERSION.json)"
    revision=$(sed -n 's/.*"stage": *"\([^"]*\)".*/\1/p' ${SOURCE}/VERSION.json)

    echo $som_version > /tmp/VERSION
...
download_source() {
    ...
    if [[ "${som_branch}" != "trunk" ]] || [[ "${som_branch}" != "main" ]]; then
        cd $SOURCE
        git checkout $som_branch
    fi
    cd ${current_path}
    compute_version_revision
}
...
create_package() {
    ...
    if [ $(echo $VERSION | grep "v") ]; then
        ...
        sed "s/<VERSION>/$ver/" ${current_path}/som-agent.mog-template > ${current_path}/som-agent.mog-aux
    else
        sed "s/<VERSION>/$VERSION/" ${current_path}/som-agent.mog-template > ${current_path}/som-agent.mog-aux
    fi
    sed "s/<TAG>/$VERSION/" ${current_path}/som-agent.mog-aux > ${current_path}/som-agent.mog

    echo "Building the package som-agent_$VERSION-sol11-${arch}.p5p"

    set_control_binary


    # Package generation process
    pkgsend generate ${install_path} | pkgfmt > som-agent.p5m.1
    ...
    python solaris_fix.py -t SPECS/template_agent.json -p som-agent.p5m.1 # Fix p5m.1 file
    mv som-agent.p5m.1.aux.fixed som-agent.p5m.1
    ...
    for file in etc/ossec.conf etc/local_internal_options.conf etc/client.keys; do
        sed "s:file $file.*:& preserve=install-only:"  som-agent.p5m.1 > som-agent.p5m.1.aux_sed
        mv som-agent.p5m.1.aux_sed som-agent.p5m.1
    done
    # Add service files
    echo "file smf_manifest.xml path=lib/svc/manifest/site/post-install.xml owner=root group=sys mode=0744 restart_fmri=svc:/system/manifest-import:default" >> som-agent.p5m.1
    echo "dir  path=var/ossec/installation_scripts owner=root group=bin mode=0755" >> som-agent.p5m.1
    echo "file postinstall.sh path=var/ossec/installation_scripts/postinstall.sh owner=root group=bin mode=0744" >> som-agent.p5m.1
    echo "file som-agent path=etc/init.d/som-agent owner=root group=sys mode=0744" >> som-agent.p5m.1
    echo "file S97som-agent path=etc/rc2.d/S97som-agent owner=root group=sys mode=0744" >> som-agent.p5m.1
    echo "file S97som-agent path=etc/rc3.d/S97som-agent owner=root group=sys mode=0744" >> som-agent.p5m.1

    # Add user and group som
    echo "group groupname=som" >> som-agent.p5m.1
    echo "user username=som group=som" >> som-agent.p5m.1

    # Necessary to upgrade from < 4.3 versions
    echo "group groupname=ossec" >> som-agent.p5m.1
    echo "user username=ossec group=ossec" >> som-agent.p5m.1

    pkgmogrify -DARCH=`uname -p` som-agent.p5m.1 som-agent.mog | pkgfmt > som-agent.p5m.2
    pkgsend -s http://localhost:9001 publish -d ${install_path} -d /etc/init.d -d /etc/rc2.d -d /etc/rc3.d -d ${current_path} som-agent.p5m.2 > pack
    package=`cat pack | grep som | cut -c 13-` # This extracts the name of the package generated in the previous step
    rm -f *.p5p
    pkg_name="som-agent_$VERSION-sol11-${arch}.p5p"
...
create_repo() {
    zfs create rpool/som
    zfs set mountpoint=/som rpool/som
    pkgrepo create /som
    ls /som
    pkgrepo -s /som set publisher/prefix=som
    svccfg -s application/pkg/server setprop \ pkg/inst_root=/som
...
uninstall() {
    ...
    rm -f `find /etc | grep som`
    rm -f /etc/rc3.d/S97som-agent
    rm -f /etc/rc2.d/S97som-agent
}
...
clean() {
    ...
    umount /som
    zfs destroy rpool/som
    rm -rf /som
    rm -rf $SOURCE/som
    rm -f som-agent.p5m*
    rm -f som-agent.mog
    rm -f som-agent.mog-aux
    rm -f pack
}
...
show_help() {
    ...
    echo -e "                Select Git branch or tag e.g. ${som_branch}."
    echo -e ""
    echo -e "        -c, --checksum"

...
if(($help.isPresent)) {
    "
    This tool can be used to generate the Windows Som agent msi package.

    PARAMETERS TO BUILD SOM-AGENT MSI (OPTIONALS):
    ...
    USAGE:
        ...
        * SOM:
          $ ./generate_som_msi.ps1  -MSI_NAME {{ NAME }} -SIGN {{ yes|no }} -WIX_TOOLS_PATH {{ PATH }} -SIGN_TOOLS_PATH {{ PATH }}
            Build a devel msi:    $ ./generate_som_msi.ps1 -MSI_NAME som-agent_4.9.0-0_windows_0ceb378.msi -SIGN no
            Build a prod msi:     $ ./generate_som_msi.ps1 -MSI_NAME som-agent-4.9.0-1.msi -SIGN yes
    "
    Exit
}
...
function BuildSomMsi(){
    ...
    if($SIGN -eq "yes"){
        ...
        $filesToSign = @(
            ...
            "..\som_modules\syscollector\build\bin\syscollector.dll",
    ...
    & $CANDLE_EXE -nologo .\som-installer.wxs -out "som-installer.wixobj" -ext WixUtilExtension -ext WixUiExtension
    & $LIGHT_EXE ".\som-installer.wixobj" -out $MSI_NAME -ext WixUtilExtension -ext WixUiExtension
...
function ExtractDebugSymbols(){
	...
	$exeFiles +=  Get-ChildItem -Filter "som_modules\syscollector\build\bin\syscollector.dll"
  ...
  #compress every pdb file in current folder
    ...
    $ZIP_NAME = $MSI_NAME -replace 'som-agent', 'som-agent-debug-symbols' -replace '\.msi$', '.zip'

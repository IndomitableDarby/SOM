...
from som.core.agent import SomDBQueryAgents, SomDBQueryGroupByAgents, Agent, \
    SomDBQueryGroup, create_upgrade_tasks, get_agents_info, get_groups, get_rbac_filters, send_restart_command, \
...
from som.core.exception import SomError, SomInternalError, SomException, SomResourceNotFound
from som.core.results import SomResult, AffectedItemsSomResult
from som.core.utils import chmod_r, chown_r, get_hash, mkdir_with_mode, md5, process_array, clear_temporary_caches, \
    full_copy, check_if_som_agent_version, parse_som_agent_version
...
# The rest of codes generated by the socket will be raised as Som internal errors (500)
...
def get_distinct_agents(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None,
                        search: dict = None, fields: list = None, q: str = None) -> AffectedItemsSomResult:
    ...
    result = AffectedItemsSomResult(all_msg='All selected agents information was returned',
    ...
    if agent_list:
        ...
        with SomDBQueryGroupByAgents(filter_fields=fields, select=fields, offset=offset, limit=limit, sort=sort,
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
async def get_agents_summary(agent_list: list[str] = None) -> SomResult:
    ...
    return SomResult({'data': agents_summary.to_dict()})


@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
def get_agents_summary_status(agent_list: list[str] = None) -> SomResult:
    ...
    if agent_list:
        ...
        with SomDBQueryGroupByAgents(filter_fields=[STATUS], select=[STATUS], query='id!=000',
        ...
        with SomDBQueryGroupByAgents(filter_fields=[GROUP_CONFIG_STATUS], select=[GROUP_CONFIG_STATUS], 
    ...
    return SomResult({'data': {'connection': connection, 'configuration': sync_configuration}})


@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
def get_agents_summary_os(agent_list: list[str] = None) -> AffectedItemsSomResult:
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(none_msg='Could not get the operative system of the agents',
    ...
    if agent_list:
        ...
        with SomDBQueryAgents(select=['os.platform'], default_sort_field='os_platform', min_select_fields=set(),
...
def reconnect_agents(agent_list: Union[list, str] = None) -> AffectedItemsSomResult:
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(all_msg='Force reconnect command was sent to all agents',
...
async def restart_agents(agent_list: list = None) -> AffectedItemsSomResult:
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(all_msg='Restart command was sent to all agents',
    ...
    with contextlib.suppress(KeyError):
        ...
        result.add_failed_item('000', SomError(1703))
        ...
        with SomQueue(common.AR_SOCKET) as wq:
            for agent_id in agent_list:
                ...
                if agent_id not in active_agents:
                    result.add_failed_item(id_=agent_id, error=SomError(1707))
                    continue
...
@expose_resources(actions=['cluster:read'], resources=[f'node:id:{node_id}'],
                  ...
                  post_proc_func=async_list_handler)
async def restart_agents_by_node(agent_list: list = None) -> AffectedItemsSomResult:
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"],
                  post_proc_kwargs={'exclude_codes': [1701, 1703, 1707], 'force': True},
                  post_proc_func=async_list_handler)
async def restart_agents_by_group(agent_list: list = None) -> AffectedItemsSomResult:
...
def get_agents(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None,
    ...
    if agent_list:
        ...
        with SomDBQueryAgents(offset=offset, limit=limit, sort=sort, search=search, select=select,
        ...
        if sort and 'version' in sort['fields']:
            data['items'] = sorted(data['items'],
                          key=lambda o: tuple(
                              parse_som_agent_version(o.get(a)) if a == 'version' and check_if_som_agent_version(o.get(a))
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"],
                  post_proc_kwargs={'exclude_codes': [1701]})
def get_agents_keys(agent_list: list = None) -> AffectedItemsSomResult:
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(all_msg='Obtained keys for all selected agents',
...
def delete_agents(agent_list: list = None, purge: bool = False, filters: dict = None,
                  q: str = None) -> AffectedItemsSomResult:
    ...
    """
    result = AffectedItemsSomResult(all_msg='All selected agents were deleted',
    ...
    if agent_list:
        ...
        with SomDBQueryAgents(limit=None, select=["id"], query=q, **rbac_filters) as db_query:
            data = db_query.run()
        ...
        list(map(lambda ag: result.add_failed_item(id_=ag, error=SomError(
            1731,
...
def add_agent(name: str = None, agent_id: str = None, key: str = None, ip: str = 'any',
    ...
    return SomResult({'data': {'id': new_agent.id, 'key': new_agent.key}})
...
def get_agent_groups(group_list: list = None, offset: int = 0, limit: int = None, sort_by: list = None,
    ...
    result = AffectedItemsSomResult(all_msg='All selected groups information was returned',
    ...
    if group_list:
        ...
        for invalid_group in set(group_list) - system_groups:
            result.add_failed_item(id_=invalid_group, error=SomResourceNotFound(1710))

        rbac_filters = get_rbac_filters(system_resources=system_groups, permitted_resources=group_list)

        with SomDBQueryGroup(**rbac_filters, limit=None) as group_query:
...
def get_group_files(group_list: list = None, offset: int = 0, limit: int = None, search_text: str = None,
                    ...
                    select: str = None, distinct: bool = False) -> SomResult:
    ...
    -------
    SomResult
        SomResult object with the groups files.
    """
    ...
    result = AffectedItemsSomResult(all_msg='All selected groups files were returned',
                                      some_msg='Some groups files were not returned',
                                      none_msg='No groups files were returned'
                                      )
    if group_id:
        if not Agent.group_exists(group_id):
            result.add_failed_item(id_=group_id, error=SomResourceNotFound(1710))
            return result
        group_path = path.join(common.SHARED_PATH, group_id)

    if not path.exists(group_path):
        result.add_failed_item(id_=group_path, error=SomError(1006))
        return result
    ...
    try:
        ...
        raise SomInternalError(1727, extra_message=str(e))

    return result


@expose_resources(actions=["group:create"], resources=["*:*:*"], post_proc_func=None)
def create_group(group_id: str) -> SomResult:
    """Creates a group.
    ...
    -------
    SomResult
        SomResult object with a operation message.
    """
    ...
    if group_id.lower() == "default" or path.exists(group_path):
        if not path.isfile(group_path):
            raise SomError(1711, extra_message=group_id)
        else:
            raise SomError(1713, extra_message=group_id)
    ...
    try:
        ...
        chown_r(group_path, common.som_uid(), common.som_gid())
        chmod_r(group_path, 0o660)
        chmod(group_path, 0o700)
        msg = f"Group '{group_id}' created."
    except Exception as e:
        raise SomInternalError(1005, extra_message=str(e))

    return SomResult({'message': msg})


@expose_resources(actions=["group:delete"], resources=["group:id:{group_list}"],
                  post_proc_kwargs={'exclude_codes': [1710, 1712]})
def delete_groups(group_list: list = None) -> AffectedItemsSomResult:
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(all_msg='All selected groups were deleted',
...
async def assign_agents_to_group(group_list: list = None, agent_list: list = None, replace: bool = False,
                           replace_list: list = None) -> AffectedItemsSomResult:
    ...
    """
    group_id = group_list[0]
    result = AffectedItemsSomResult(all_msg=f'All selected agents were assigned to {group_id}'
    ...
    if '000' in agent_list:
        agent_list.remove('000')
        result.add_failed_item(id_='000', error=SomError(1703))
...
@expose_resources(actions=["agent:modify_group"], resources=['agent:id:{agent_list}'],
                  post_proc_func=async_list_handler)
async def remove_agent_from_group(group_list: list = None, agent_list: list = None) -> SomResult:
    ...
    return SomResult({'message': message})
...
async def remove_agent_from_groups(agent_list: list = None, group_list: list = None) -> AffectedItemsSomResult:
    ...
    """
    agent_id = agent_list[0]
    result = AffectedItemsSomResult(all_msg='Specified agent was removed from returned groups',
...
async def remove_agents_from_group(agent_list: list = None, group_list: list = None) -> AffectedItemsSomResult:
    ...
    """
    group_id = group_list[0]
    result = AffectedItemsSomResult(all_msg=f'All selected agents were removed from group {group_id}',
...
def get_outdated_agents(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None,
                        search: dict = None, select: str = None, q: str = None) -> AffectedItemsSomResult:
...
def upgrade_agents(agent_list: list = None, wpk_repo: str = None, version: str = None, force: bool = False,
                   ...
                   filters: dict = None, q: str = None) -> AffectedItemsSomResult:
    ...
    """
    result = AffectedItemsSomResult(all_msg='All upgrade tasks were created',
    ...
    if agent_list:
        ...
        with SomDBQueryAgents(limit=None, select=["id", "status"], query=q, **rbac_filters) as db_query:
            data = db_query.run()
        ...
        [result.add_failed_item(id_=agent, error=SomError(1707)) for agent in non_active_agents]
        non_active_agents = set(non_active_agents)

        # Add non eligible agents to failed_items
        non_eligible_agents = agent_list - not_found_agents - non_active_agents - filtered_agents
        [result.add_failed_item(id_=ag, error=SomError(
            1731,
            extra_message="some of the requirements are not met -> {}".format(
                ', '.join(f"{key}: {value}" for key, value in filters.items() if key != 'rbac_ids') +
                (f', q: {q}' if q else '')
        ...
        for agent_result_chunk in tasks_results:
            for agent_result in agent_result_chunk['data']:
                ...
                # Upgrade error for specific agents
                    ...
                    error = SomError(error_code, cmd_error=True, extra_message=agent_result['message'])
                ...
                # Upgrade error for all agents, bad request
                    ...
                    raise SomError(error_code, cmd_error=True, extra_message=agent_result['message'])

                # Upgrade error for all agents, internal server error
                else:
                    raise SomInternalError(error_code, cmd_error=True, extra_message=agent_result['message'])
...
def get_upgrade_result(agent_list: list = None, filters: dict = None, q: str = None) -> AffectedItemsSomResult:
    ...
    AffectedItemsSomResult
        Upgrade results.
    """
    result = AffectedItemsSomResult(all_msg='All upgrade tasks were returned',
        ...
        for task_result_chunk in task_results:
            for task_result in task_result_chunk['data']:
                ...
                # Upgrade error for specific agents (no task in DB)
                    ...
                    error = SomError(error_code, cmd_error=True, extra_message=task_result['message'])
                    result.add_failed_item(id_=str(task_result['agent']).zfill(3), error=error)

                # Upgrade error for all agents, internal server error
                else:
                    raise SomInternalError(error_code, cmd_error=True, extra_message=task_result['message'])
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
def get_agent_config(agent_list: list = None, component: str = None, config: str = None) -> SomResult:
...
def get_agents_sync_group(agent_list: list = None) -> AffectedItemsSomResult:
    ...
    """
    result = AffectedItemsSomResult(all_msg='Sync info was returned for all selected agents',
...
def get_file_conf(group_list: list = None, type_conf: str = None, raw: bool = False,
    ...
    -------
    SomResult
        SomResult object with the configuration.
    """
    ...
    return SomResult({'data': configuration.get_file_conf(filename, group_id=group_id, type_conf=type_conf,
                                                            raw=raw)})


@expose_resources(actions=["group:read"], resources=["group:id:{group_list}"], post_proc_func=None)
def get_agent_conf(group_list: list = None, filename: str = 'agent.conf', offset: int = 0,
                   limit: int = common.DATABASE_LIMIT) -> SomResult:
...
def upload_group_file(group_list: list = None, file_data: str = None, file_name: str = 'agent.conf') -> SomResult:
    """Update a group file.
    ...
    -------
    SomResult
        SomResult object with the confirmation message.
    """
    # We access unique group_id from list, this may change if and when we decide to add option to update files for
    # a list of groups
    group_id = group_list[0]

    return SomResult({'message': configuration.upload_group_file(group_id, file_data, file_name=file_name)})


def get_full_overview() -> SomResult:
    """Get information about agents.

    Returns
    -------
    SomResult
        SomResult object with information about agents.
    """
    # We don't consider agent 000 in order to get the summary
    q = "id!=000"
...
def check_uninstall_permission() -> SomResult:
    ...
    -------
    SomResult
        SomResult object containing a message indicating if the
        user has the permission to uninstall agents.
    """
    return SomResult({'message': 'User has permission to uninstall agents'})

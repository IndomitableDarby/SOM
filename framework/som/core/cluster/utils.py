...
from som.core.exception import SomError, SomException, SomInternalError, SomHAPHelperError
from som.core.results import SomResult
from som.core.utils import temporary_cache
from som.core.som_socket import create_som_socket_message
from som.core.wlogging import SomLogger
...
execq_lockfile = os.path.join(common.SOM_PATH, "var", "run", ".api_execq_lock")

HELPER_DEFAULTS = {
    HAPROXY_PORT: 5555,
    HAPROXY_PROTOCOL: 'http',
    HAPROXY_BACKEND: 'som_reporting',
    ...
    for field in [
        ...
        if helper_config.get(field):
            try:
                ...
                raise SomError(3004, extra_message=f"HAProxy Helper {field} must be an integer.")
    ...
    for field in [IMBALANCE_TOLERANCE]:
        if helper_config.get(field):
            try:
                ...
                raise SomError(3004, extra_message=f"HAProxy Helper {field} must be a float.")
    return helper_config
...
def parse_haproxy_helper_config(helper_config: dict) -> dict:
    ...
    if helper_config[HAPROXY_PROTOCOL].lower() == 'https' and type(helper_config[HAPROXY_CERT]) == bool:
        raise SomHAPHelperError(3042, extra_message='HAProxy certificate file required in the haproxy_cert parameter')
...
def read_cluster_config(config_file=common.OSSEC_CONF, from_import=False) -> typing.Dict:
    ...
    try:
        ...
        if e.code == 1106:
            ...
            raise SomError(3006, extra_message=e.message)
    except Exception as e:
        raise SomError(3006, extra_message=str(e))
    ...
    if isinstance(config_cluster['port'], str) and not config_cluster['port'].isdigit():
        raise SomError(3004, extra_message="Cluster port must be an integer.")
...
def get_manager_status(cache=False) -> typing.Dict:
    ...
    try:
        ...
        raise SomInternalError(1913, extra_message=str(e))

    processes = ['som-agentlessd', 'som-analysisd', 'som-authd', 'som-csyslogd', 'som-dbd', 'som-monitord',
                 'som-execd', 'som-integratord', 'som-logcollector', 'som-maild', 'som-remoted',
                 'som-reportd', 'som-syscheckd', 'som-clusterd', 'som-modulesd', 'som-db', 'som-apid']

    data, pidfile_regex, run_dir = {}, re.compile(r'.+\-(\d+)\.pid$'), os.path.join(common.SOM_PATH, "var", "run")
...
def get_cluster_status() -> typing.Dict:
    ...
    try:
        cluster_status |= {"running": "yes" if get_manager_status()['som-clusterd'] == 'running' else "no"}
...
def manager_restart() -> SomResult:
    """Restart Som manager.

    Send JSON message with the 'restart-som' command to common.EXECQ_SOCKET socket.

    Raises
    ------
    SomInternalError(1901)
        If the socket path doesn't exist.
    SomInternalError(1902)
    ...
    try:
        ...
        # json msg for restarting Som manager
        msg = json.dumps(create_som_socket_message(origin={'module': common.origin_module.get()},
                                                     command=common.RESTART_SOM_COMMAND,
    ...
    return SomResult({'message': 'Restart request sent'})
...
def get_cluster_items():
    ...
    try:
        ...
        with open(os.path.join(common.SOM_PATH, here, 'cluster.json')) as f:
...
class ClusterLogger(SomLogger):
    """
    Define the logger used by som-clusterd.
    """
    ...
    def setup_logger(self):
        ...
        Set ups cluster logger. In addition to super().setup_logger() this method adds:
            ...
            * Sets log level based on the "debug_level" parameter received from som-clusterd binary.
        """
        super().setup_logger()
...
def process_spawn_sleep(child):
    ...
    """
    pid = os.getpid()
    pyDaemonModule.create_pid(f'som-clusterd_child_{child}', pid)
...
async def forward_function(func: callable, f_kwargs: dict = None, request_type: str = 'local_master',
    ...
    Returns
    -------
    Return either a dict or `SomResult` instance in case the execution did not fail. Return an exception otherwise.
    """

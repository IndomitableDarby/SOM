...
with patch('som.common.som_uid'):
    with patch('som.common.som_gid'):
        ...
        import som.core.cluster.common as cluster_common
        import som.core.results as wresults
        from som.core import common
        from som.core.wdb import AsyncSomDBConnection
...
som_common = cluster_common.SomCommon()
...
@patch("som.core.cluster.common.SendStringTask.set_up_coro")
def test_sst_init(setup_coro_mock, create_task_mock):
    ...
    with patch.object(TaskMock, "add_done_callback") as done_callback_mock:
        sst_task = cluster_common.SendStringTask(som_common=cluster_common.SomCommon(), logger='')
        assert sst_task.logger == ''
        assert sst_task.task in sst_task.tasks_hard_reference
        assert isinstance(sst_task.som_common, cluster_common.SomCommon)
...
@patch("asyncio.create_task")
@patch("som.core.cluster.common.ReceiveStringTask.set_up_coro")
def test_rst_init(setup_coro_mock, create_task_mock):
    ...
    with patch.object(TaskMock, "add_done_callback") as done_callback_mock:
        string_task = cluster_common.ReceiveStringTask(som_common=cluster_common.SomCommon(), logger='',
                                                       info_type='testing', task_id=b"010")
        assert isinstance(string_task.som_common, cluster_common.SomCommon)
        setup_coro_mock.assert_called_once()
        done_callback_mock.assert_called_once()
...
async def test_rst_str_method(logger_mock, event_loop):
    ...
    with patch('som.core.cluster.common.ReceiveStringTask.set_up_coro', side_effect = return_coro) as setup_coro_mock:
        string_task = cluster_common.ReceiveStringTask(cluster_common.SomCommon(), '', b"task", logger_mock)
...
@patch('logging.Logger')
@patch('som.core.cluster.common.SomCommon')
def test_rst_set_up_coro_ko(som_common_mock, logger_mock):
    """Test if the exception is being properly raised when an Exception takes place."""

    with pytest.raises(NotImplementedError):
        cluster_common.ReceiveStringTask(som_common_mock, logger_mock, b"task")
...
def test_rst_done_callback(setup_coro_mock, create_task_mock):
    ...
    with patch.object(TaskMock, "add_done_callback"):
        logger = logging.getLogger('som')
        with patch.object(logger, "error") as logger_mock:
            string_task = cluster_common.ReceiveStringTask(som_common=som_common_mock, logger=logger,
                                                           info_type='agent-groups', task_id=b"010")
            string_task.done_callback()
            assert string_task.som_common.in_str == {b"011": b"123456789"}
            assert string_task.som_common.sync_tasks == {b"011": b"123456789"}
...
@patch('asyncio.Event')
@patch("asyncio.create_task")
@patch.object(logging.getLogger("som"), "error")
@patch("som.core.cluster.common.ReceiveFileTask.set_up_coro")
@patch("som.core.cluster.common.uuid4", return_value="e6c45993-b0ae-438f-8914-3b5175b2bbfd")
def test_rft_init(uuid_mock, setup_coro_mock, logger_mock, create_task_mock, event_mock):
    ...
    with patch.object(TaskMock, "add_done_callback") as done_callback_mock:
        file_task = cluster_common.ReceiveFileTask(som_common=cluster_common.SomCommon(), logger=logger_mock,
                                                   task_id=b"010")
        assert isinstance(file_task.som_common, cluster_common.SomCommon)
        setup_coro_mock.assert_called_once()
        done_callback_mock.assert_called_once()
        ...
        string_task = cluster_common.ReceiveFileTask(som_common=cluster_common.SomCommon(), logger=logger_mock,
                                                     task_id="")
...
@patch('logging.Logger')
@patch('som.core.cluster.common.SomCommon')
def test_rft_set_up_coro(som_common_mock, logger_mock):
    """Test if the exception is being properly raised when an Exception takes place."""

    with pytest.raises(NotImplementedError):
        cluster_common.ReceiveFileTask(som_common_mock, logger_mock, b"task")
...
def test_rft_done_callback(set_up_coro_mock, create_task_mock, event_mock):
    ...
    with patch.object(TaskMock, "add_done_callback"):
        with patch.object(logging.getLogger('som'), "error") as logger_mock:
            file_task = cluster_common.ReceiveFileTask(som_common=som_common_mock,
                                                       logger=logging.getLogger('som'), task_id=b"010")
            file_task.done_callback()
            assert file_task.som_common.sync_tasks == {"011": b"123456789"}
            logger_mock.assert_called_once_with(Exception, exc_info=False)
...
def test_handler_init():
    ...
    with patch('som.core.cluster.utils.context_tag', ContextVar('', default="")) as cv:
        ...
        assert handler.logger == logging.getLogger("som")
...
def test_handler_msg_build_ko():
    ...
    with pytest.raises(exception.SomClusterError, match=r'.* 3024 .*'):
...
def test_handler_get_messages_ok():
    ...
    with patch('cryptography.fernet.Fernet.decrypt', return_value="decrypted payload"):
        with patch('som.core.cluster.common.Handler.msg_parse', return_value=True) as handler_mock:
...
@patch('som.core.cluster.common.Handler.msg_parse', return_value=True)
...
def test_handler_get_messages_ko(decrypt_mock, msg_parse_mock):
    ...
    with pytest.raises(exception.SomClusterError, match=r'.* 3025 .*'):
        handler.in_msg.total = 0
        list(handler.get_messages())


@pytest.mark.asyncio
@patch('som.core.cluster.common.Handler.push')
@patch('som.core.cluster.common.Handler.next_counter', return_value=30)
@patch('som.core.cluster.common.Handler.msg_build', return_value=["some", "messages"])
async def test_handler_send_request_ok(msg_build_mock, next_counter_mock, push_mock):
    ...
    with patch('som.core.cluster.common.Response.read', return_value="some value") as read_mock:
    ...
    with patch('som.core.cluster.common.Response.read', side_effect=delay):
        with pytest.raises(exception.SomClusterError, match=r'\b3020\b'):
...
async def test_handler_send_request_ko():
    ...
    with patch('som.core.cluster.common.Handler.msg_build', side_effect=MemoryError):
        with pytest.raises(exception.SomClusterError, match=r'.* 3026 .*'):
            await handler.send_request(b'some bytes', b'some data')

    with pytest.raises(exception.SomClusterError, match=r'.* 3018 .*'):
        await handler.send_request(b'some bytes', b'some data')


@pytest.mark.asyncio
@patch('som.core.cluster.common.Handler.send_request')
async def test_handler_get_chunks_in_task_id(send_request_mock):
    ...
    # Test KeyError
    with pytest.raises(exception.SomClusterError, match='.* 3035 .*'):
    ...
    with pytest.raises(exception.SomClusterError, match='.* 3036 .*'):
...
async def test_handler_update_chunks_wdb(send_request_mock):
    ...
    with patch('som.core.cluster.common.send_data_to_wdb', new_callable=AsyncMock) as send_data_to_wdb_mock:
        with patch.object(LoggerMock, "debug") as logger_debug_mock:
            with patch.object(LoggerMock, "debug2") as logger_debug2_mock:
                with patch.object(LoggerMock, "error") as logger_error_mock:
                    ...
                    logger_debug_mock.assert_has_calls([call('2/5 chunks updated in som-db in 6.000s.')])
                    ...
                    logger_error_mock.assert_has_calls([call('other1'), call('other2'),
                                                        call('Som-db response for chunk 1/5 was not "ok": 0'),
                                                        call('Som-db response for chunk 2/5 was not "ok": 1')])

    # Test Exception
    send_request_mock.reset_mock()
    error_message = 'error'
    with patch('som.core.cluster.common.send_data_to_wdb', side_effect=Exception(error_message)):
        with pytest.raises(exception.SomClusterError,
...
@patch('som.core.cluster.common.Handler.send_request', return_value=b'some data')
...
async def test_handler_send_file_ko():
    ...
    with pytest.raises(exception.SomClusterError, match=r'.* 3034 .*'):
...
async def test_handler_send_string():
    ...
    with patch('som.core.cluster.common.Handler.send_request', return_value=b"some data"):
        assert (await handler.send_string(b"something") == b"some data")

    with patch('som.core.cluster.common.Handler.send_request', side_effect=exception.SomClusterError(3020)):
        with patch.object(logging.getLogger("som"), "error") as logger_mock:
            assert exception.SomClusterError(3020).message.encode() in await handler.send_string(b"something")
...
async def test_handler_forward_dapi_response_ok():
    ...
    with patch('som.core.cluster.common.Handler.get_manager', return_value=mock_manager):
...
async def test_handler_forward_dapi_response_ko():
    ...
    with patch('som.core.cluster.common.Handler.get_manager', return_value=mock_manager):
        ...
        with patch.object(mock_manager.local_server, "send_string",
                          side_effect=exception.SomException(1001)) as send_string_mock:
            with patch.object(logging.getLogger('Som'), "error") as logger_mock:
                with patch('som.core.cluster.common.Handler.send_request', return_value="some value"):
...
@pytest.mark.parametrize('exception, expected_error', [
    (exception.SomException(1001), exception.SomException(1001)),
    (exception.SomClusterError(3020), None),
    (Exception('Test'), exception.SomClusterError(1000, extra_message='Test'))
])
async def test_handler_forward_sendsync_response_ko(exception, expected_error):
    ...
    with patch('som.core.cluster.common.Handler.get_manager', return_value=mock_manager):
        ...
        with patch.object(mock_manager.local_server, "send_request", side_effect=exception):
            with patch.object(logging.getLogger('som'), "error") as logger_mock:
                with patch('som.core.cluster.common.Handler.send_request',
                    ...
                    if expected_error:
                        ...
                        send_request_mock.assert_called_once_with(
                            b'sendsync_err',
                            json.dumps(expected_error, cls=cluster_common.SomJSONEncoder).encode()
                        )
                    else:
...
def test_handler_data_received_ok():
    ...
    # Test first if
    with patch('som.core.cluster.common.Handler.get_messages', return_value=[(b"bytes1", 123, b"bytes2", b"d")]):
        # Test try
    ...
    with patch('som.core.cluster.common.Handler.get_messages', return_value=[(b"bytes1", 123, b"bytes2", b"bytes3")]):
        with patch('cryptography.fernet.Fernet.decrypt', return_value="decrypted payload"):
            ...
            with patch('som.core.cluster.common.Handler.dispatch') as dispatch_mock:
                handler.data_received(b"message")
...
@patch('som.core.cluster.common.Handler.msg_build', return_value=["msg"])
@patch('som.core.cluster.common.Handler.push')
@patch('som.core.cluster.common.Handler.process_request', return_value=(b"command", b"payload"))
def test_handler_dispatch(process_request_mock, push_mock, msg_build_mock):
    ...
    process_request_mock.side_effect = exception.SomException(1001)
    with patch.object(logging.getLogger('som'), "error") as logger_mock:
        handler.dispatch(None, 123, b"payload")
        logger_mock.assert_called_with(f"Internal error processing request '{None}': {exception.SomException(1001)}")
...
def test_handler_process_request():
    ...
    with patch('som.core.cluster.common.Handler.process_unknown_cmd') as process_unknown_cmd_mock:
...
def test_handler_process_response():
    ...
    with patch('som.core.cluster.common.Handler.process_error_from_peer', return_value=b"payload"):
...
def test_handler_str_upd():
    ...
    with patch('som.core.cluster.common.InBuffer.receive_data'):
...
def test_handler_process_dapi_error_ko():
    ...
    handler = cluster_common.Handler(fernet_key, cluster_items)
    with pytest.raises(exception.SomClusterError, match=r".* 3032 .*"):
        handler.server = ManagerMock()
        handler.process_dapi_error(data=b"data 2")

    with pytest.raises(exception.SomClusterError, match=r".* 3025 .*"):
        with patch("asyncio.create_task", side_effect=exception.SomClusterError(1001)):
            handler.server = ManagerMock()
...
def test_handler_process_error_from_peer():
    ...
    assert isinstance(handler.process_error_from_peer(b"data to decode"), exception.SomClusterError)
...
def test_handler_setup_task_logger():
    ...
    with patch.object(logging.getLogger("som"), "getChild", return_value=TaskLoggerMock()) as get_child_mock:
...
async def test_handler_wait_for_file_ko(send_request_mock):
    """Check if expected exception is raised.
        Condition 1: when event.wait() exceeds the timeout, SomClusterError 3039 is raised
        Condition 2: when any other exception occurs, SomClusterError 3040  is raised"""
    ...
    with pytest.raises(exception.SomClusterError, match=r'.* 3039 .*'):
    ...
    with pytest.raises(exception.SomClusterError, match=r".* 3040 .*"):
...
# Test 'SomCommon' class methods

def test_som_common_init():
    """Test the '__init__' method correct functioning."""

    som_common_test = cluster_common.SomCommon()
    assert som_common_test.sync_tasks == {}


@pytest.mark.asyncio
@patch("som.core.cluster.common.AsyncSomDBConnection", return_value=AsyncMock())
async def test_som_common_recalculate_group_hash(asyncsomdbconnection_mock):
    class LoggerMock:
        """Auxiliary class."""
    ...
    logger = LoggerMock()
    await som_common.recalculate_group_hash(logger)
    assert logger._debug == ['Recalculating agent-group hash.']


@pytest.mark.asyncio
@patch("som.core.cluster.common.AsyncSomDBConnection")
async def test_som_common_recalculate_group_hash_ko(asyncsomdbconnection_mock):
    class LoggerMock:
        """Auxiliary class."""
    ...
    logger = LoggerMock()
    asyncsomdbconnection_mock.side_effect = [exception.SomInternalError(2007), exception.SomError(2003)]

    await som_common.recalculate_group_hash(logger)
    assert logger._debug == ['Recalculating agent-group hash.']
    assert logger._warning == ['Error 2007 executing recalculate agent-group hash command: '
                               'Error retrieving data from Som DB']

    logger = LoggerMock()
    await som_common.recalculate_group_hash(logger)
    assert logger._debug == ['Recalculating agent-group hash.']
    assert logger._warning == ['Error 2003 executing recalculate agent-group hash command: Error in somdb request']

def test_som_common_get_logger():
    """Check if a Logger object is properly returned."""

    with pytest.raises(NotImplementedError):
        som_common.get_logger()


def test_som_common_setup_send_info():
    ...
    with patch('som.core.cluster.common.SomCommon.get_logger'):
        first_output, second_output = som_common.setup_send_info(mock_object)
        assert first_output == b'ok'
        assert isinstance(second_output, bytes)


def test_som_common_setup_receive_file():
    ...
    with patch('som.core.cluster.common.SomCommon.get_logger'):
        first_output, second_output = som_common.setup_receive_file(mock_object)
        assert first_output == b'ok'
        assert isinstance(second_output, bytes)
        assert MyTaskMock().task_id in som_common.sync_tasks
        assert isinstance(som_common.sync_tasks[MyTaskMock().task_id], MyTaskMock)


@patch('som.core.cluster.common.SomCommon')
@patch('logging.Logger')
def test_som_common_end_receiving_file_ok(logger_mock, som_common_mock):
    """Check if the full path to the received file is properly stored and availability is notified."""

    with patch('som.core.cluster.common.ReceiveFileTask.set_up_coro'):
        with patch('asyncio.create_task'):
            file_task = cluster_common.ReceiveFileTask(som_common_mock, logger_mock, b"task")

    som_common.sync_tasks = {'task_ID': file_task}
    assert som_common.end_receiving_file("task_ID filepath") == (b'ok', b'File correctly received')
    assert isinstance(som_common.sync_tasks["task_ID"], cluster_common.ReceiveFileTask)


@patch('os.remove')
@patch('os.path.exists', return_value=True)
def test_som_common_end_receiving_file_ko(path_exists_mock, os_remove_mock):
    """Test the 'end_receiving_file' correct functioning in a failure scenario."""

    with pytest.raises(exception.SomClusterError, match=r'.* 3027 .*'):
        som_common.end_receiving_file("not_task_ID filepath")
...
@patch('json.loads')
def test_som_common_error_receiving_file_ok(json_loads_mock):
    """Check how error are handled by peer in the sent file process."""

    with patch('os.path.exists', return_value=True):
        with patch('os.remove'):
            # Test first condition and its nested condition
            assert som_common.error_receiving_file("task_ID error_details") == (b'ok', b'Error received')

    # Test second condition
    with patch('som.core.cluster.common.SomCommon.get_logger'):
        assert som_common.error_receiving_file("not_task_ID error_details") == (b'ok', b'Error received')


def test_som_common_error_receiving_file_ko():
...
def test_som_common_get_node():
    """Check if it is possible to obtain basic information about the node."""

    class MockClass(cluster_common.SomCommon, cluster_common.Handler, abc.ABC):
        def __init__(self):
            super().__init__()
    ...
    with patch('som.core.cluster.common.Handler.get_manager', return_value=mock_manager) as manager_mock:
        mock_class.get_node()
        manager_mock.assert_called_once()


# Test SyncSomdb class

def test_sync_som_db_init():
    """Test the '__init__' method from the SyncSomdb class."""

    sync_som_db = cluster_common.SyncSomdb(
        manager=cluster_common.Handler(fernet_key, cluster_items), logger=logging.getLogger("som"), cmd=b"cmd",
        data_retriever=None, get_data_command="get_command", set_data_command="set_command", pivot_key=None)

    assert sync_som_db.get_data_command == "get_command"
    assert sync_som_db.set_data_command == "set_command"
    assert sync_som_db.data_retriever is None


@pytest.mark.asyncio
@patch("som.core.wdb.socket.socket")
async def test_sync_som_db_retrieve_information(socket_mock):
    ...
    wdb_conn = AsyncSomDBConnection()
    logger = logging.getLogger("som")
    handler = cluster_common.Handler(fernet_key, cluster_items)
    sync_object = cluster_common.SyncSomdb(manager=handler, logger=logger, cmd=b'syn_a_w_m',
    ...
    # data_retriever returns a JSON that does not follow the pivoting scheme
    with patch.object(sync_object, 'data_retriever', side_effect=exception.SomException(1000)):
        with patch.object(sync_object.logger, 'error') as logger_error_mock:
            ...
            logger_error_mock.assert_called_with(
                'Could not obtain data from som-db: Error 1000 - Som Internal Error')


@patch('som.core.wdb_http.SomDBHTTPClient')
async def test_sync_som_db_retrieve_agents_information(wdb_http_client_mock):
...
async def test_sync_som_db_retrieve_agents_information_ko():
    ...
    with patch('som.core.wdb_http.SomDBHTTPClient', side_effect=exception.SomException(1000)):
        with patch.object(sync_object.logger, 'error') as logger_error_mock:
...
@patch('som.core.cluster.common.time.perf_counter', return_value=0)
async def test_sync_som_db_sync_ok(perf_counter_mock, json_dumps_mock):
    ...
    # Test try and if
    with patch.object(sync_som_db.logger, "debug") as logger_debug_mock:
        with patch("som.core.cluster.common.Handler.send_string", return_value=b"OK") as send_string_mock:
            with patch("som.core.cluster.common.Handler.send_request",
                       side_effect=None) as send_request_mock:
                assert await sync_som_db.sync(start_time=10, chunks=['a', 'b']) is True
    ...
    # Test else
    with patch.object(sync_som_db.logger, "info") as logger_info_mock:
        assert await sync_som_db.sync(start_time=-10, chunks=[]) is True
        logger_info_mock.assert_called_once_with(f"Finished in 10.000s. Updated 0 chunks.")

    # Test except
    with patch("som.core.cluster.common.Handler.send_string", return_value=b'Error 1'):
        sync_som_db.server = cluster_common.Handler(fernet_key, cluster_items)
        with pytest.raises(exception.SomClusterError, match=r".* 3016 .*"):
            await sync_som_db.sync(start_time=10, chunks=['a'])
...
def test_end_sending_agent_information(perf_counter_mock, json_loads_mock):
    ...
    with patch('som.core.cluster.common.utils.get_utc_now', side_effect=get_utc_now_mock):
...
@patch("som.core.cluster.common.SomDBConnection")
async def test_send_data_to_wdb(SomDBConnection_mock):
    """Check if the data chunks are being properly forward to the Som-db socket."""

    class MockSomDBConnection:
        """Auxiliary class."""
    ...
    SomDBConnection_mock.return_value = MockSomDBConnection()
    ...
    SomDBConnection_mock.return_value.exceptions += 1
    ...
    with patch('som.core.cluster.master.utils.Timeout', side_effect=Exception):
        result = await cluster_common.send_data_to_wdb(data={'chunks': chunks, 'set_data_command': ''},
                                                 timeout=15, info_type='agent-groups')
...
@patch('som.core.common.INSTALLATION_UID_PATH', os.path.join('/tmp', 'installation_uid'))
def test_som_json_encoder_default(mock_chmod, mock_chown, mock_gid, mock_uid):
    """Test if a special JSON encoder is defined for Som."""

    som_encoder = cluster_common.SomJSONEncoder()

    # Test first condition
    assert isinstance(som_encoder.default(object), dict)
    assert som_encoder.default(object) == {'__callable__': {'__name__': 'object', '__module__': 'builtins',
                                                              '__qualname__': 'object', '__type__': 'type'}}

    class SomMock(json.JSONEncoder):
        __self__ = Som()
        a = 0
        b = 0
    ...
    som_encoder.default(SomMock)

    with patch('builtins.callable', return_value=False):
        # Test second condition
        assert isinstance(som_encoder.default(exception.SomException(3012)), dict)
        assert som_encoder.default(exception.SomException(3012)) == \
               {'__som_exception__': {'__class__': 'SomException',
                                        '__object__': {'type': 'about:blank', 'title': 'SomException', 'code': 3012,
                                                       'extra_message': None, 'extra_remediation': None,
                                                       'cmd_error': False, 'dapi_errors': {}}}}

        # Test third condition
        abstract_som_result = wresults.AffectedItemsSomResult()

        assert isinstance(som_encoder.default(abstract_som_result), dict)
        assert som_encoder.default(abstract_som_result) == \
               {'__som_result__': {'__class__': 'AffectedItemsSomResult',
        ...
        assert isinstance(som_encoder.default(date), dict)
        assert som_encoder.default(date) == {'__som_datetime__': '2021-10-15T00:00:00'}

        # Test fifth condition
        exc = ValueError('test')
        assert isinstance(som_encoder.default(exc), dict)
        assert som_encoder.default(exc) == {'__unhandled_exc__': {'__class__': 'ValueError',
                                                                    '__args__': ('test',)}}

        # Test simple return
        with pytest.raises(TypeError):
            som_encoder.default({"key": "value"})
...
@patch('som.core.common.som_uid', return_value=0)
@patch('som.core.common.INSTALLATION_UID_PATH', os.path.join('/tmp', 'installation_uid'))
def test_as_som_object_ok(mock_chmod, mock_chown, mock_gid, mock_uid):
    """Test the different outputs taking into account the input values."""

    # Test the first condition and nested if
    assert cluster_common.as_som_object({"__callable__": {"__name__": "type", "__som__": "version"}}) == "server"

    # Test the first condition and nested else
    assert isinstance(
        cluster_common.as_som_object({"__callable__": {"__name__": "path", "__qualname__": "__loader__.value",
                                                         "__module__": "os"}}), str)

    assert cluster_common.as_som_object({"__callable__": {"__name__": "__name__", "__qualname__": "value",
                                                            "__module__": "itertools"}}) == "itertools"

    # Test the second condition
    assert isinstance(cluster_common.as_som_object(
        {"__som_exception__": {"__class__": "somException", "__object__": {"code": 1001}}}),
        exception.SomException)

    # Test the third condition
    with patch('som.core.results.AbstractSomResult.decode_json', return_value=wresults.AbstractSomResult):
        assert isinstance(cluster_common.as_som_object(
            {"__som_result__": {"__class__": "AbstractSomResult", "__object__": {"code": 1001}}}), abc.ABCMeta)

    # Test the fourth condition
    assert isinstance(cluster_common.as_som_object({"__som_datetime__": "2021-10-14"}), datetime)
    assert cluster_common.as_som_object({"__som_datetime__": "2021-10-14"}) == datetime(2021, 10, 14)

    # Test the fifth condition
    result = cluster_common.as_som_object({'__unhandled_exc__': {'__class__': 'ValueError',
                                                                   '__args__': ('test',)}})
    assert result == {"ValueError": ["test"]}

    result = cluster_common.as_som_object({'__unhandled_exc__': {'__class__': 'exit',
                                                                   '__args__': []}})
    assert result == {"exit": []}

    # No condition fulfilled
    assert isinstance(cluster_common.as_som_object({"__som_datetime_bad__": "2021-10-14"}), dict)
    assert cluster_common.as_som_object({"__som_datetime_bad__": "2021-10-14"}) == \
           {"__som_datetime_bad__": "2021-10-14"}
...
@patch('som.core.common.som_uid', return_value=0)
@patch('som.core.common.INSTALLATION_UID_PATH', os.path.join('/tmp', 'installation_uid'))
def test_as_som_object_ko(mock_chmod, mock_chown, mock_gid, mock_uid):
    """Test if the exceptions are correctly raised."""

    with pytest.raises(exception.SomInternalError, match=r'.* 1000 .*'):
        cluster_common.as_som_object({"__callable__": {"__name__": "value", "__som__": "value"}})


def get_handler():
    """Return a Handler object. This is an auxiliary method."""
    return cluster_common.Handler(fernet_key=fernet_key, cluster_items=cluster_items, logger=logging.getLogger("som"))
...
def test_sync_task_init():
    ...
    sync_task = cluster_common.SyncTask(b"cmd", logging.getLogger("som"), get_handler())

    assert sync_task.cmd == b"cmd"
    assert sync_task.logger == logging.getLogger("som")
    assert isinstance(sync_task.server, cluster_common.Handler)


@pytest.mark.asyncio
@patch('som.core.cluster.common.Handler.send_request', side_effect=Exception())
async def test_sync_task_request_permission(send_request_mock):
    ...
    with patch.object(logging.getLogger("som"), "debug") as logger_mock:
        # Test second condition
...
@patch("som.core.cluster.cluster.compress_files", return_value=("files/path/", {}))
@patch("som.core.cluster.utils.log_subprocess_execution")
async def test_sync_files_sync_ok(log_subprocess_mock, compress_files_mock, unlink_mock, relpath_mock, json_dumps_mock):
    ...
    class WorkerMock:
        ...
        async def send_request(self, command, data):
            ...
            if command == b"cmd" and data == b"" and self.count == 1:
                raise exception.SomClusterError(3020, extra_message=command.decode())
                ...
                raise exception.SomClusterError(3016, extra_message=command.decode())
    ...
    sync_files = cluster_common.SyncFiles(b"cmd", logging.getLogger("som"), worker_mock)
    ...
    with patch.object(logging.getLogger("som"), "error") as logger_mock:
        ...
        json_dumps_mock.assert_called_once_with(
            exception.SomClusterError(code=3020, extra_message="cmd"),
            cls=cluster_common.SomJSONEncoder)
    ...
    with patch.object(WorkerMock, "send_file", return_value=1000) as send_file_mock:
        ...
        with patch.object(logging.getLogger("som"), "debug") as logger_debug_mock:
            with patch.object(logging.getLogger("som"), "error") as logger_error_mock:
                await sync_files.sync(files_to_sync, files_metadata, 1, task_pool=None)
                ...
                relpath_mock.assert_called_once_with('files/path/', common.SOM_PATH)
                ...
                log_subprocess_mock.assert_called()
                logger_error_mock.assert_called_once_with(
                    f"Error sending zip file: {exception.SomException(3016, 'cmd_e')}")
...
async def test_sync_files_sync_ko(send_request_mock):
    ...
    sync_files = cluster_common.SyncFiles(b"cmd", logging.getLogger("som"), handler)
    ...
    with patch('os.unlink', side_effect=FileNotFoundError):
        ...
        with patch("som.core.cluster.cluster.compress_files", return_value=(compressed_data, {})):
            send_request_mock.side_effect = None
            logger = logging.getLogger('som')
            with patch.object(logger, "error") as logger_mock:
                await sync_files.sync(files_to_sync, files_metadata, 1, task_pool=None)

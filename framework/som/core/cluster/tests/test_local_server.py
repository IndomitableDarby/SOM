...
with patch('som.common.som_uid'):
    with patch('som.common.som_gid'):
        ...
        from som.core.cluster.local_server import *
...
async def test_LocalServerHandler_connection_made(event_loop):
    ...
    with patch.object(logger, "debug") as logger_debug_mock:
        with patch("som.core.cluster.local_server.context_tag", ContextVar("tag", default="")) as mock_contextvar:
...
@pytest.mark.asyncio
@patch("som.core.cluster.local_server.server.AbstractServerHandler.process_request")
...
async def test_LocalServerHandler_get_ruleset_hashes(event_loop):
    ...
    lsh = LocalServerHandler(server=ServerMock(), loop=event_loop, fernet_key=None, cluster_items={})
    with patch("som.core.cluster.local_server.cluster.get_ruleset_status",
...
@pytest.mark.asyncio
@patch("som.core.cluster.local_server.LocalServerHandler.send_request", return_value='changed')
...
async def test_LocalServerHandlerMaster_process_request(process_request_mock, event_loop):
    ...
    with pytest.raises(SomClusterError, match=".* 3022 .*"):
...
@pytest.mark.asyncio
@patch("som.core.cluster.local_server.LocalServerHandler.process_request")
async def test_LocalServerHandlerWorker_process_request(process_request_mock, event_loop):
    ...
    with pytest.raises(SomClusterError, match=".* 3023 .*"):
...
async def test_AsyncReloadRulesetFlag_initial_state():
    ...
    from som.core.cluster.worker import AsyncReloadRulesetFlag

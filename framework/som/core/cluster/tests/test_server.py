...
with patch('som.common.som_uid'):
    with patch('som.common.som_gid'):
        from som.core.cluster.server import *
        from som.core.cluster import common as c_common
        from som.core.exception import SomClusterError, SomError, SomResourceNotFound
...
async def test_AbstractServerHandler_init(event_loop):
    ...
    with patch("som.core.cluster.server.context_tag", ContextVar("tag", default="")) as mock_contextvar:
...
@pytest.mark.asyncio
@patch("som.core.cluster.server.AbstractServerHandler.hello")
@patch("som.core.cluster.server.AbstractServerHandler.echo_master")
@patch("som.core.cluster.common.Handler.process_request")
async def test_AbstractServerHandler_process_request(mock_process_request, mock_echo_master, mock_hello, event_loop):
    ...
    abstract_server_handler.process_request(command=b"echo-c", data=b"som")
    mock_echo_master.assert_called_once_with(b"som")
...
async def test_AbstractServerHandler_echo_master(event_loop):
    ...
    assert abstract_server_handler.echo_master(data=b"som") == (b"ok-m ", b"som")
    abstract_server_handler.echo_master(data=b"som")
...
@pytest.mark.asyncio
@patch("asyncio.Queue")
@patch("som.core.cluster.server.functools")
...
@patch('som.core.cluster.server.AbstractServer.check_clients_keepalive')
@patch('som.core.cluster.server.AbstractServerHandler')
...
@patch("som.core.cluster.server.uuid4", return_value="abc123")
@patch("asyncio.get_running_loop", new=Mock())
...
@patch("som.core.cluster.server.utils.process_array")
@patch("asyncio.get_running_loop", new=Mock())
def test_AbstractServer_get_connected_nodes(mock_process_array):
    ...
    with patch.object(abstract_server, "to_dict", return_value=basic_dict):
        abstract_server.get_connected_nodes(search={"value": "som", "negation": True},
                                            sort={"fields": ["nothing"], "order": "desc"}, limit=501, offset=1)
        mock_process_array.assert_called_once_with([basic_dict["info"]], search_text="som", complementary_search=True,
...
def test_AbstractServer_get_connected_nodes_ko(mock_process_array):
    ...
    with patch.object(abstract_server, "to_dict", return_value=basic_dict):
        ...
        with pytest.raises(SomError, match=".* 1724 .* Not a valid select field: no"):
            abstract_server.get_connected_nodes(select={"no": "exists"})

        with pytest.raises(SomError, match=".* 1728 .*"):
            abstract_server.get_connected_nodes(filter_type="None")

        abstract_server.configuration = {"node_name": "master"}
        with pytest.raises(SomResourceNotFound, match=".* 1730 .*"):
...
async def test_AbstractServer_check_clients_keepalive(sleep_mock):
    ...
    with patch.object(logger, "debug") as mock_debug:
        with patch.object(logger, "error") as mock_error:
            with patch("som.core.cluster.server.AbstractServer.setup_task_logger",
                       return_value=logger):
                abstract_server = AbstractServer(performance_test=1, concurrency_test=2, configuration={"test3": 3},
                                                 cluster_items={"test4": 4}, enable_ssl=True, logger=logger)
...
@patch("asyncio.sleep", side_effect=IndexError)
@patch("asyncio.get_running_loop", new=Mock())
@patch('som.core.cluster.server.perf_counter', return_value=0)

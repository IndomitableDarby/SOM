...
import som.core.cluster.cluster
import som.core.cluster.utils
import som.core.manager
import som.core.results as wresults
from som import agent
from som.cluster import get_node_wrapper, get_nodes_info
from som.core import common, exception
from som.core.cluster import local_client, common as c_common
from som.core.cluster.cluster import check_cluster_status
from som.core.exception import SomException, SomClusterError, SomError
from som.core.pyDaemonModule import spawn_process_pool_worker, API_AUTHENTICATION_PROCESS
from som.core.som_socket import som_sendsync


authentication_funcs = {'check_token', 'check_user_master', 'get_permissions', 'get_security_conf'}
events_funcs = {'send_event_to_analysisd'}

node_info = som.core.cluster.cluster.get_node()
pools = common.mp_pools.get()
if node_info['type'] == 'master':
    # Suppress exception when the user running Som cannot access /dev/shm.
...
class DistributedAPI:
    ...
    def __init__(self, f: Callable, logger: logging.getLogger, f_kwargs: Dict = None, node: c_common.Handler = None,
        ...
        self.cluster_items = som.core.cluster.utils.get_cluster_items() if node is None else node.cluster_items
        self.debug = debug
        ...
        if not basic_services:
            self.basic_services = ('som-modulesd', 'som-analysisd', 'som-execd', 'som-db', 'som-remoted')
        else:
    ...
    def debug_log(self, message):
        ...
        message : str
            Full log message.
        """
        if self.logger.name == 'som-api':
            self.logger.debug2(message)
        else:
            self.logger.debug(message)

    async def distribute_function(self) -> [Dict, exception.SomException]:
        """
        Distribute an API call.

        Returns
        -------
        dict or SomException
            Dictionary with API response or SomException in case of error.
        """
        try:
            ...
            try:
                response = json.loads(response, object_hook=c_common.as_som_object) \
                    if isinstance(response, str) else response
            except json.decoder.JSONDecodeError:
                response = {'message': response}

            return response if isinstance(response, (wresults.AbstractSomResult, exception.SomException)) \
                else wresults.SomResult(response)

        except json.decoder.JSONDecodeError:
            e = exception.SomInternalError(3036)
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}")
            return e
        except exception.SomInternalError as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}", exc_info=not isinstance(e, exception.SomClusterError))
            return e
        except exception.SomError as e:
            e.dapi_errors = self.get_error_info(e)
            return e
            ...
            return exception.SomInternalError(1000,
                                                dapi_errors=self.get_error_info(e))

    def check_som_status(self):
        """
        There are some services that are required for som to correctly process API requests. If any of those services
        is not running, the API must raise an exception indicating that:
            ...
            * There's an error in any of those services that must be addressed before using the API if any service is
              in failed status.
            * Som must be started before using the API is the services are stopped.

        The basic services som needs to be running are: som-modulesd, som-remoted, som-analysisd, som-execd
        and som-db
        """
        if self.f == som.core.manager.status:
            return

        status = som.core.manager.status()
        ...
        if not_ready_daemons:
            ...
            raise exception.SomInternalError(1017, extra_message=extra_info)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    raise exception.SomInternalError(3021)
                except OperationalError as exc:
                    raise exception.SomInternalError(2008, extra_message=str(exc.orig))
                except process.BrokenProcessPool:
                    raise exception.SomInternalError(901)
            except json.decoder.JSONDecodeError:
                raise exception.SomInternalError(3036)
            except process.BrokenProcessPool:
                raise exception.SomInternalError(900)
            ...
            self.logger.error(f"{e.message}",
                              exc_info=e.code not in {3021, 3036, 1913, 1017} and not isinstance(e,
                                                                                           exception.SomClusterError))
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.SomJSONEncoder)
        except (exception.SomError, exception.SomResourceNotFound) as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.SomJSONEncoder)
            ...
            if self.debug:
                raise
            return json.dumps(exception.SomInternalError(1000,
                                                           dapi_errors=self.get_error_info(e)),
                              cls=c_common.SomJSONEncoder)
    ...
    def get_error_info(self, e) -> Dict:
        ...
        except exception.SomException as rbac_exception:
        ...
        error_message = e.message if isinstance(e, exception.SomException) else exception.GENERIC_ERROR_MSG
        result = {node: {'error': error_message}
                  }

        # Give log path only in case of SomInternalError
        if isinstance(e, exception.SomInternalError):
            log_filename = None
            for h in self.logger.handlers or self.logger.parent.handlers:
                if hasattr(h, 'baseFilename'):
                    log_filename = os.path.join('SOM_HOME', os.path.relpath(h.baseFilename, start=common.SOM_PATH))
    ...
    async def send_tmp_file(self, node_name=None):
        ...
        res = json.loads(await client.send_file(os.path.join(common.SOM_PATH,
                                                             self.f_kwargs['tmp_file']),
                                                node_name),
                         object_hook=c_common.as_som_object)
        os.remove(os.path.join(common.SOM_PATH, self.f_kwargs['tmp_file']))
    ...
    async def execute_remote_request(self) -> Dict:
        ...
        node_response = await client.execute(command=b'dapi',
                                             data=json.dumps(self.to_dict(),
                                                             cls=c_common.SomJSONEncoder).encode())
        return json.loads(node_response,
                          object_hook=c_common.as_som_object)

    async def forward_request(self) -> [wresults.AbstractSomResult, exception.SomException]:
        ...
        Returns
        -------
        wresults.AbstractSomResult or exception.SomException
        """

        async def forward(node_name: Tuple) -> [wresults.AbstractSomResult, exception.SomException]:
            ...
            if node_name == self.node_info['node']:
                ...
                try:
                    ...
                    if e.code == 3022:
                        ...
                        raise e
                # Convert a non existing node into a SomError exception
                if isinstance(result, SomClusterError) and result.code == 3022:
                    common.rbac.set(self.rbac_permissions)
                    try:
                    ...
                    dikt['ids'] = {node_name}
                    result = SomError.from_dict(dikt)

            return result if isinstance(result, (wresults.AbstractSomResult, exception.SomException)) \
                else wresults.SomResult(result)
        ...
        if 'node_id' in self.f_kwargs or 'node_list' in self.f_kwargs:
            ...
            allowed_nodes = wresults.AffectedItemsSomResult()
        ...
        if allowed_nodes.total_affected_items > 1:
            ...
            if isinstance(response, wresults.AbstractSomResult):
        ...
        # It might be a SomError after reducing
        if isinstance(response, wresults.AffectedItemsSomResult):
...
class SomRequestQueue:
    """Represents a queue of Som requests"""
...
class APIRequestQueue(SomRequestQueue):
    """
    ...
    def __init__(self, server):
        ...
        self.logger = logging.getLogger('som').getChild('dapi')
        self.logger.addFilter(som.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='D API'))
    ...
    async def run(self):
        while True:
            ...
            try:
                request = json.loads(request, object_hook=c_common.as_som_object)
                ...
                task_id = await node.send_string(json.dumps(result, cls=c_common.SomJSONEncoder).encode())
            except Exception as e:
...
class SendSyncRequestQueue(SomRequestQueue):
    """
    ...
    def __init__(self, server):
        super().__init__(server)
        self.logger = logging.getLogger('som').getChild('sendsync')
        self.logger.addFilter(som.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='SendSync'))
    ...
    async def run(self):
        while True:
            ...
            try:
                ...
                result = await som_sendsync(**request)

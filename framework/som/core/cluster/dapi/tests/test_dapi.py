...
with patch('som.common.som_uid'):
    with patch('som.common.som_gid'):
        ...
        from som.core.cluster.dapi.dapi import DistributedAPI, APIRequestQueue, SendSyncRequestQueue
        from som.core.manager import get_manager_status
        from som.core.results import SomResult, AffectedItemsSomResult
        from som import agent, cluster, ciscat, manager, SomError, SomInternalError
        from som.core.exception import SomClusterError, SomException
...
@pytest.mark.parametrize('kwargs', [
    {'f_kwargs': {'select': ['id']}, 'rbac_permissions': {'mode': 'black'}, 'nodes': ['worker1'],
     'basic_services': ('som-modulesd', 'som-db'), 'request_type': 'local_master'},
...
def test_DistributedAPI_debug_log():
    ...
    logger_ = TestingLogger(logger_name="som-api")
    ...
    logger_ = TestingLogger(logger_name="som")
...
@patch('som.core.cluster.dapi.dapi.DistributedAPI.execute_local_request',
       new=AsyncMock(return_value=SomResult({'result': 'local'})))
@patch('som.core.cluster.dapi.dapi.DistributedAPI.forward_request',
       new=AsyncMock(return_value=SomResult({'result': 'forward'})))
@patch('som.core.cluster.dapi.dapi.DistributedAPI.execute_remote_request',
       new=AsyncMock(return_value=SomResult({'result': 'remote'})))
...
def test_DistributedAPI_distribute_function(api_request, request_type, node, expected, cluster_enabled, f_kwargs):
    ...
    with patch('som.core.cluster.dapi.dapi.check_cluster_status', return_value=cluster_enabled):
        with patch('som.core.cluster.dapi.dapi.node_info', {'type': node}):
...
@patch('som.core.cluster.dapi.dapi.DistributedAPI.execute_local_request',
       new=AsyncMock(return_value=SomResult({'result': 'local'})))
@patch('som.core.cluster.dapi.dapi.DistributedAPI.get_solver_node',
       new=AsyncMock(return_value=SomResult({'unknown': ['001', '002']})))
...
def test_DistributedAPI_distribute_function_mock_solver(api_request, request_type, node, expected):
    ...
    expected : str
        Expected result
    """
    with patch('som.core.cluster.cluster.get_node', return_value={'type': node, 'node': 'master'}):
...
def test_DistributedAPI_distribute_function_exception():
    ...
    logger_ = logging.getLogger("som")
    with patch("som.core.cluster.dapi.dapi.get_node_wrapper", side_effect=SomError(4000)):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger_)
        get_error_result = dapi.get_error_info(Exception("testing"))
        assert 'unknown-node' in get_error_result
        assert get_error_result['unknown-node']['error'] == 'Som Internal Error. See log for more detail'

    with patch("som.core.cluster.dapi.dapi.get_node_wrapper", side_effect=SomError(4001)):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger_)
        with pytest.raises(SomError, match='.* 4001 .*'):
            dapi.get_error_info(Exception("testing"))

    with patch("som.core.cluster.dapi.dapi.get_node_wrapper", return_value=NodeWrapper()):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger_)
...
def test_DistributedAPI_local_request_errors():
    ...
    with patch('som.core.cluster.dapi.dapi.DistributedAPI.execute_local_request',
               new=AsyncMock(side_effect=SomInternalError(1001))):
        dapi_kwargs = {'f': agent.get_agents_summary_status, 'logger': logger}
    ...
    with patch('som.cluster.get_nodes_info', side_effect=json.decoder.JSONDecodeError('test', 'test', 1)):
        with patch('som.core.cluster.dapi.dapi.DistributedAPI.check_som_status'):
...
def test_DistributedAPI_local_request(mock_local_request):
    ...
    with patch('asyncio.wait_for', new=AsyncMock(side_effect=SomError(1001))):
        dapi_kwargs = {'f': manager.status, 'logger': logger}
        raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=1001)
    ...
    with patch('asyncio.wait_for', new=AsyncMock(side_effect=SomInternalError(1001))):
        dapi_kwargs = {'f': manager.status, 'logger': logger}
        raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=1001)
    ...
    exc_code = 3000
    cluster_exc = SomClusterError(exc_code)
    with patch('asyncio.wait_for', new=AsyncMock(side_effect=cluster_exc)):
        with patch.object(TestingLogger, "error") as logger_error_mock:
            # Test SomClusterError caught in execute_local_request and ProblemException raised
            dapi_kwargs = {'f': manager.status, 'logger': testing_logger}
            raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=exc_code)

            # Test SomClusterError is raised when using debug in execute_local_request and distribute_function
            dapi = DistributedAPI(f=manager.status, logger=testing_logger, debug=True)
            try:
                raise_if_exc(loop.run_until_complete(dapi.distribute_function()))
            except SomClusterError as e:
...
@patch('som.core.cluster.dapi.dapi.node_info', {'type': 'worker'})
@patch('som.core.cluster.dapi.dapi.check_cluster_status', return_value=True)
@patch('som.core.cluster.local_client.LocalClient.execute', return_value='invalid_json')
...
@patch('som.core.cluster.local_client.LocalClient.execute', new=AsyncMock(return_value='{"Testing": 1}'))
...
@patch('som.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'master-node'})
@patch('som.core.cluster.dapi.dapi.check_cluster_status', return_value=True)
@patch('som.core.cluster.dapi.dapi.DistributedAPI.get_solver_node', return_value={'worker1': ['001', '002']})
@patch('som.core.cluster.local_client.LocalClient.execute', return_value='invalid_json')
...
@patch('som.core.cluster.local_client.LocalClient.send_file', new=AsyncMock(return_value='{"Testing": 1}'))
@patch('som.core.cluster.local_client.LocalClient.execute', new=AsyncMock(return_value='{"Testing": 1}'))
@patch('som.core.cluster.dapi.dapi.DistributedAPI.get_solver_node',
       new=AsyncMock(return_value=SomResult({'testing': ['001', '002']})))
@patch('som.core.cluster.dapi.dapi.check_cluster_status', return_value=True)
def test_DistributedAPI_tmp_file(mock_cluster_status):
    ...
    with patch('som.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'unknown'}):
        with patch('som.core.cluster.dapi.dapi.get_node_wrapper',
                   return_value=AffectedItemsSomResult(affected_items=[{'type': 'master', 'node': 'unknown'}])):
            dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
                           'f_kwargs': {'tmp_file': '/tmp/dapi_file.txt'}}
            raise_if_exc_routine(dapi_kwargs=dapi_kwargs)

    open('/tmp/dapi_file.txt', 'a').close()
    with patch('som.core.cluster.cluster.get_node', return_value={'type': 'unk', 'node': 'master'}):
        raise_if_exc_routine(dapi_kwargs=dapi_kwargs)
...
def test_DistributedAPI_tmp_file_cluster_error(mock_cluster_status):
    ...
    with patch('som.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'unknown'}):
        with patch('som.core.cluster.dapi.dapi.get_node_wrapper',
                   return_value=AffectedItemsSomResult(affected_items=[{'type': 'master', 'node': 'unknown'}])):
            with patch('som.core.cluster.local_client.LocalClient.execute',
                       new=AsyncMock(side_effect=SomClusterError(3022))):
                dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
                               'f_kwargs': {'tmp_file': '/tmp/dapi_file.txt'}}
            ...
            open('/tmp/dapi_file.txt', 'a').close()
            with patch('som.core.cluster.local_client.LocalClient.execute',
                       new=AsyncMock(side_effect=SomClusterError(1000))):
                dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
                               'f_kwargs': {'tmp_file': '/tmp/dapi_file.txt'}}
                raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=1000)


@patch('som.core.cluster.local_client.LocalClient.execute',
       new=AsyncMock(return_value='{"items": [{"name": "master"}], "totalItems": 1}'))
@patch('som.agent.Agent.get_agents_overview', return_value={'items': [{'id': '001', 'node_name': 'master'},
...
def test_DistributedAPI_get_solver_node(mock_cluster_status, mock_agents_overview):
    ...
    nodes_info_result = AffectedItemsSomResult()
    nodes_info_result.affected_items.append({'name': 'master'})
    common.cluster_nodes.set(['master'])

    with patch('Som.core.cluster.dapi.dapi.get_nodes_info', new=AsyncMock(return_value=nodes_info_result)):
        with patch('Som.core.cluster.dapi.dapi.node_info', {'type': 'master', 'node': 'unknown'}):
            dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
            ...
            expected = AffectedItemsSomResult()
            expected.affected_items = [{'id': '001', 'node_name': 'master'}]
            with patch('som.agent.get_agents_in_group', return_value=expected):
                dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
...
@pytest.mark.parametrize('api_request', [
    ...
    som.core.manager.status
])
@patch('som.core.manager.get_manager_status', return_value={process: 'running' for process in get_manager_status()})
def test_DistributedAPI_check_som_status(status_mock, api_request):
    """Test `check_som_status` method from class DistributedAPI."""
    dapi = DistributedAPI(f=api_request, logger=logger)
    data = dapi.check_som_status()
    assert data is None
...
@patch('som.core.cluster.dapi.dapi.node_info', {'node': 'random_node'})
def test_DistributedAPI_check_som_status_exception(status_value):
    """Test exceptions from `check_som_status` method from class DistributedAPI."""
    statuses = {process: status_value for process in sorted(get_manager_status())}
    with patch('som.core.manager.get_manager_status',
               return_value=statuses):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger)
        try:
            dapi.check_som_status()
        except SomInternalError as e:
            assert e.code == 1017
            assert statuses
...
class TestDistributedAPIRunLocal:
    ...
    @patch("som.core.common.reset_context_cache")
    @patch("som.core.common.cluster_nodes")
    @patch("som.core.common.origin_module")
    @patch("som.core.common.current_user")
    @patch("som.core.common.broadcast")
    @patch("som.core.common.rbac")
    ...
    @patch("som.core.common.broadcast")
    @patch("som.core.common.rbac")
    def test_run_local_with_som_exception(
        self,
        mock_rbac_ctx,
        mock_broadcast_ctx,
        mock_current_user_ctx,
        ...
        """Test that SomException is re-raised properly."""
        mock_broadcast_token = "test_token"
        mock_broadcast_ctx.set.return_value = mock_broadcast_token

        som_exception = SomException("Test error")
        mock_function.side_effect = som_exception

        with pytest.raises(SomException) as exc_info:
            DistributedAPI.run_local(
                f=mock_function, f_kwargs=sample_kwargs, **sample_context_vars
            )

        # Verify it's the same exception instance
        assert exc_info.value is som_exception
    ...
    def test_run_local_with_other_exception(
        ...
        """Test that non-SomException exceptions are not caught."""
        mock_broadcast_token = "test_token"
        mock_broadcast_ctx.set.return_value = mock_broadcast_token
...
@patch("som.core.cluster.common.import_module", return_value="os.path")
...
async def test_APIRequestQueue_run(loop_mock, import_module_mock):
    ...
    with patch.object(logger, "error", side_effect=Exception("break while true")) as logger_mock:
        ...
        logger_mock.assert_called_once_with("Error in DAPI request. The destination node is "
                                            "not connected or does not exist: 'som'.")

        node = NodeMock()
        with patch.object(node, "send_request", side_effect=SomClusterError(3020, extra_message="test")):
            with patch.object(node, "send_string", return_value=b"noerror"):
                with patch("som.core.cluster.dapi.dapi.DistributedAPI", return_value=DistributedAPI_mock()):
                    server.clients = {"som": node}
                    with pytest.raises(Exception):
                        await apirequest.run()

            with patch.object(node, "send_string", Exception("break while true")):
                with patch("som.core.cluster.dapi.dapi.DistributedAPI", return_value=DistributedAPI_mock()):
                    with patch("som.core.cluster.dapi.dapi.contextlib.suppress", side_effect=Exception()):
                        apirequest.logger = logging.getLogger("apirequest")
                        with pytest.raises(Exception):
                            await apirequest.run()


@patch("som.core.cluster.dapi.dapi.contextlib.suppress", side_effect=Exception())
@patch("asyncio.get_event_loop")
async def test_SendSyncRequestQueue_run(loop_mock, contexlib_mock):
    ...
    class RequestQueueMock:
        async def get(self):
            return "som*request_queue*test {\"daemon_name\": \"test\"}"
    ...
    with patch.object(logger, "error", side_effect=Exception("break while true")) as logger_mock:
        ...
        with patch.object(node, "send_request", Exception("break while true")):
            with patch("som.core.cluster.dapi.dapi.som_sendsync", side_effect=Exception("break while true")):
                server.clients = {"som": node}
                sendsync.logger = logging.getLogger("sendsync")
                with pytest.raises(Exception):
                    await sendsync.run()

            with patch("som.core.cluster.dapi.dapi.som_sendsync", side_effect="noerror"):
                with pytest.raises(Exception):
                    await sendsync.run()

...
import pytest
from som.agent import get_agents, reconnect_agents
from som.cluster import get_nodes_info
from som.core.results import AffectedItemsSomResult

from framework.som.core.cluster.hap_helper.som import SomAgent, SomDAPI
...
class TestSomAgent:
    ...
    def test_can_reconnect(self, version: str, expected: bool):
        ...
        assert SomAgent.can_reconnect(version) == expected
    ...
    def test_get_agents_able_to_reconnect(self):
        ...
        assert SomAgent.get_agents_able_to_reconnect(agents_list=agents) == [2, 3]


@mock.patch('framework.som.core.cluster.hap_helper.som.DistributedAPI', autospec=True)
class TestSomDAPI:
    som_dapi = SomDAPI(tag='test')

    @pytest.fixture
    def fixture_affected_items_result(self):
        return AffectedItemsSomResult()
    ...
    @mock.patch('framework.som.core.cluster.hap_helper.som.get_system_nodes', return_value={})
    async def test_get_cluster_nodes(
        self,
        get_system_nodes_mock: mock.AsyncMock,
        dapi_mock: mock.MagicMock,
        fixture_affected_items_result: AffectedItemsSomResult,
        nodes_data: list,
        excluded_nodes: list,
    ):
        """Check the correct output of `get_cluster_nodes` function."""

        self.som_dapi.excluded_nodes = excluded_nodes
        ...
        ret_val = await self.som_dapi.get_cluster_nodes()

        dapi_mock.assert_called_once_with(
            f=get_nodes_info,
            f_kwargs=None,
            logger=self.som_dapi.logger,
    ...
    async def test_reconnect_agents(
        ...
        ret_val = await self.som_dapi.reconnect_agents(agent_list=agent_list)
    ...
    async def test_get_agents_node_distribution(
        ...
        ret_val = await self.som_dapi.get_agents_node_distribution()
        ...
        dapi_mock.assert_called_once_with(
            ...
            f_kwargs={
                ...
                'q': 'id!=000',
                'limit': self.som_dapi.AGENTS_MAX_LIMIT,
            },
            logger=self.som_dapi.logger,
            request_type='local_master',
        )
    ...
    async def test_get_agents_belonging_to_node(
        self, dapi_mock: mock.MagicMock, fixture_affected_items_result: AffectedItemsSomResult, limit: Optional[int]
    ):
        ...
        ret_val = await self.som_dapi.get_agents_belonging_to_node(node_name=node_name, limit=limit)
        ...
        dapi_mock.assert_called_once_with(
            ...
            f_kwargs={
                ...
                'q': 'id!=000',
                'limit': limit or self.som_dapi.AGENTS_MAX_LIMIT,
            },
            logger=self.som_dapi.logger,
            request_type='local_master',
        )

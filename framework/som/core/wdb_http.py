...
class SomDBHTTPClient:
    ...
    def __init__(self, retries: int = 5, timeout: float = 10):
        ...
        try:
            ...
            raise SomInternalError(2011, e)

    async def close(self) -> None:
        """Close the som-db HTTP client."""
    ...
    async def _get(self, endpoint: str) -> Any:
        ...
        try:
            ...
            if response.is_error:
                raise SomError(2012, extra_message=response.text)

        except RequestError as exc:
            raise SomError(2013, extra_message=str(exc))
...
@asynccontextmanager
async def get_wdb_http_client() -> AsyncIterator[SomDBHTTPClient]:
    """Create and return the engine client.

    Returns
    -------
    AsyncIterator[SomDBHTTP]
        Som DB HTTP client iterator.
    """
    client = SomDBHTTPClient()

    try:
        yield client
    except TimeoutException:
        raise SomInternalError(2014)
    except UnsupportedProtocol:
        raise SomInternalError(2015)
    except ConnectError:
        raise SomInternalError(2016)
    finally:
        await client.close()

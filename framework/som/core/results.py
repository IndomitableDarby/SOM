...
import som.core.exception as wexception
from som.core import utils
from som.core.common import DATABASE_LIMIT

current_module = sys.modules[__name__]


class AbstractSomResult(collections.abc.MutableMapping):
    """
    ...
    def __init__(self, dct: Union[dict, object]):
        ...
        Raises
        ------
        wexception.SomInternalError(1000)
            If dct is of a wrong type.
        """
        if isinstance(dct, dict):
            self.dikt = dct
        elif isinstance(dct, AbstractSomResult):
            self.dikt = dct.dikt
        else:
            raise wexception.SomInternalError(1000, extra_message=f"dct param must be a dict or "
                                                                    f"an AbstractSomResult subclass, "
                                                                    f"not a {type(dct)}")
    ...
    def __or__(self, other):
        """ | operator used to merge two AbstractSomResult objects. When merged with a SomException, the result is
        always a SomException

        Parameters
        ----------
        other : AbstractSomResult or wexception.SomException

        Raises
        ------
        SomInternalError(1000)
            Som internal error when two object could not be merged.

        Returns
        -------
        AbstractSomResult or wexception.SomException
            Resultant object.
        """
        if isinstance(other, wexception.SomException):
            return other
        elif not isinstance(other, (dict, AbstractSomResult)):
            raise wexception.SomInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def _merge_dict(self, self_field: dict, other_field: dict, key: str = None) -> dict:
        ...
        AbstractSomResult subclass. This method may be redefined in subclasses.
    ...
    def limit(self, limit: int = DATABASE_LIMIT, offset: int = 0):
        """Should take the first `limit` results starting from `offset`. To be redefined in SomResult subclasses.

        Parameters
        ----------
        limit : int
            Default: the value specified in som.core.common.DATABASE_LIMIT
        offset : int
            Default: 0.
    ...
    def sort(self, fields: list = None, order: str = 'asc'):
        """Sort according to `fields` in order `order`. To be redefined in SomResult subclasses.
...
class SomResult(AbstractSomResult):
    """
...
class AffectedItemsSomResult(AbstractSomResult):
    ...
    def add_failed_item(self, id_: str = None, error: wexception.SomException = None):
    ...
    def add_failed_items_from(self, other):
        ...
        wexception.SomInternalError(1000)
            If dct is of a wrong type.
        """
        if not isinstance(other, AffectedItemsSomResult):
            raise wexception.SomInternalError(1000,
    ...
    def __or__(self, other):
        """Merge a AffectedItemsSomResult with self.

        Parameters
        ----------
        other : AffectedItemsSomResult
            AffectedItemsSomResult object to merge.
        ...
        if isinstance(other, wexception.SomError):
            if len(other.ids) > 0:
                ...
                return other
        elif isinstance(result, wexception.SomException):
            return result
        elif not isinstance(other, AffectedItemsSomResult):
            raise wexception.SomInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def to_dict(self) -> dict:
        """Return the AffectedItemsSomResult as a dict.
    ...
    def render(self) -> dict:
        """Render AffectedItemsSomResult object.

        Returns
        -------
        dict
            Return AffectedItemsSomResult as a dictionary with fields data, message and error.
        """

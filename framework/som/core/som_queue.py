...
from som.core.common import origin_module
from som.core.exception import SomError, SomInternalError
from som.core.som_socket import create_som_socket_message


def create_som_queue_socket_msg(flag: str, str_agent_id: str, msg: str, is_restart: bool = False) -> str:
    """Create message that will be sent to the SomQueue socket.
    ...
    -------
    str
        Message that will be sent to the SomQueue socket.
    """
...
class BaseQueue:
    ...
    def _connect(self):
        try:
            ...
            if length_send_buffer < SomQueue.MAX_MSG_SIZE:
                self.socket.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, SomQueue.MAX_MSG_SIZE)
        except Exception:
            raise SomInternalError(1010, self.path)
    ...
    def _send(self, msg: bytes) -> None:
        ...
        try:
            ...
            if sent == 0:
                raise SomInternalError(1011, self.path)
        except socket.error:
            raise SomInternalError(1011, self.path)
...
class SomQueue(BaseQueue):
    ...
    RESTART_AGENTS_JSON = json.dumps(create_som_socket_message(origin={'module': origin_module.get()},
                                                                 command="restart-som0",
    ...
    def send_msg_to_agent(self, msg: str = '', agent_id: str = '', msg_type: str = '') -> str:
        ...
        msg_is_no_ar = msg in [SomQueue.HC_SK_RESTART, SomQueue.HC_FORCE_RECONNECT]
        msg_is_restart = msg in [SomQueue.RESTART_AGENTS, SomQueue.RESTART_AGENTS_JSON]
        ...
        # AR
        if msg_type == SomQueue.AR_TYPE:
            socket_msg = create_som_queue_socket_msg(flag, str_agent_id, msg) if agent_id != '000' else msg
            # Return message
        ...
        # Restart agents
            ...
            # If msg is not a non active-response command and not a restart command, raises SomInternalError
            if not msg_is_no_ar and not msg_is_restart:
                raise SomInternalError(1012, msg)
            socket_msg = create_som_queue_socket_msg(flag, str_agent_id, msg, is_restart=msg_is_restart)
            # Return message
            if msg == SomQueue.HC_SK_RESTART:
                ret_msg = "Restarting Syscheck on agent" if agent_id else "Restarting Syscheck on all agents"
            elif msg == SomQueue.HC_FORCE_RECONNECT:
                ret_msg = "Reconnecting agent" if agent_id else "Reconnecting all agents"
            else:  # msg == SomQueue.RESTART_AGENTS or msg == SomQueue.RESTART_AGENTS_JSON
        ...
        try:
            ...
            raise SomError(1014, extra_message=f": SomQueue socket with path {self.path}")

        return ret_msg


class SomAnalysisdQueue(BaseQueue):
    """
    SomAnalysisdQueue Object.
    """
    ...
    def send_msg(self, msg_header: str, msg: str):
        ...
        try:
            ...
            raise SomError(1014, extra_message=f': SomAnalysisdQueue socket with path {self.path}. {str(e)}')

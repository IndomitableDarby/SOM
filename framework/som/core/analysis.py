...
from som.core.som_socket import create_som_socket_message, SomSocket
...
def is_ruleset_file(filename: str) -> bool:
    ...
    If `filename` is already an absolute path and includes `common.SOM_PATH`, it will be normalized and used as is.
    Otherwise, it will be joined with `common.SOM_PATH`.
    ...
    if os.path.isabs(filename):
        ...
        full_path = os.path.normpath(os.path.join(common.SOM_PATH, filename))
...
def send_reload_ruleset_msg(origin: dict[str, str]) -> RulesetReloadResponse:
    ...
    """
    msg = create_som_socket_message(origin=origin, command=RELOAD_RULESET_COMMAND)

    socket = SomSocket(common.ANALYSISD_SOCKET)
...
def send_reload_ruleset_and_get_results(node_id: str, results: AffectedItemsSomResult) -> AffectedItemsSomResult:
    """
    ...
    results : AffectedItemsSomResult
    ...
    if socket_response.is_ok():
        ...
        results.add_failed_item(id_=node_id,
                                error=SomError(code=1914, extra_message=', '.join(socket_response.errors)))

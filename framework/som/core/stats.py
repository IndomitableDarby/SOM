...
from som.core import common, utils
from som.core import som_socket
...
def totals_(date: datetime.datetime = utils.get_utc_now()) -> list:
    ...
    try:
        ...
        raise SomError(1308, extra_message=stat_filename)
...
def get_daemons_stats_socket(socket: str, agents_list: Union[list[int], str] = None, last_id: int = None) -> dict:
    """Send message to Som socket to get statistical information.
    ...
    """
    # Create message
    full_message = som_socket.create_som_socket_message(
    ...
    # Connect to socket
    try:
        s = som_socket.SomSocketJSON(socket)
    except Exception:
        raise SomInternalError(1121, extra_message=socket)
...
def get_daemons_stats_(filename: str) -> list:
    ...
    try:
        ...
        try:
            ...
            raise SomInternalError(1104, extra_message=str(e))
    except IOError:
        raise SomError(1308, extra_message=filename)
...
def get_stats_socket_path(agent_id: Union[str, int], daemon: str) -> str:
    ...
    if is_agent_a_manager(agent_id):
        return os.path.join(common.SOM_PATH, "queue", "sockets", daemon)
    else:
...
def get_daemons_stats_from_socket(agent_id: str, daemon: str) -> dict:
    ...
    if socket_response.get('error', 0) != 0:
        ...
        raise SomError(1117, extra_message=rec_msg)

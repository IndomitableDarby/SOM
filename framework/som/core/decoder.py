...
from som.core.utils import load_som_xml, add_dynamic_detail
...
def load_decoders_from_file(decoder_file: str, decoder_path: str, decoder_status: str) -> list:
    ...
    try:
        decoders = list()
        position = 0
        root = load_som_xml(os.path.join(common.SOM_PATH, decoder_path, decoder_file))
        ...
        raise SomError(1502, extra_message=os.path.join('SOM_HOME', decoder_path, decoder_file))
    except Exception:
        raise SomInternalError(1501, extra_message=os.path.join('SOM_HOME', decoder_path, decoder_file))

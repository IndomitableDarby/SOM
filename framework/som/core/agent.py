...
from som.core import common, configuration, stats
from som.core.InputValidator import InputValidator
from som.core.cluster.utils import get_manager_status
from som.core.common import AGENT_COMPONENT_STATS_REQUIRED_VERSION, DATE_FORMAT
from som.core.exception import SomException, SomError, SomInternalError, SomResourceNotFound
from som.core.utils import SomVersion, plain_dict_to_nested_dict, get_fields_to_nest, SomDBQuery, \
    SomDBQueryDistinct, SomDBQueryGroupBy, SomDBBackend, get_utc_now, get_utc_strptime, \
    get_date_from_timestamp
from som.core.som_queue import SomQueue
from som.core.som_socket import SomSocket, SomSocketJSON, create_som_socket_message
from som.core.wdb import SomDBConnection
from som.core.wdb_http import get_wdb_http_client
from som.rbac.utils import resource_cache
...
class SomDBQueryAgents(SomDBQuery):
    """Class used to query Som agents."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        if filters is None:
            filters = {}
        unify_som_version_format(filters)
        if min_select_fields is None:
            min_select_fields = {'id'}
        backend = SomDBBackend(query_format='global')
        SomDBQuery.__init__(self, offset=offset, limit=limit, table='agent', sort=sort, search=search, select=select,
    ...
    def _filter_date(self, date_filter: dict, filter_db_name: str):
        """Add date filter to the Som query."""
        SomDBQuery._filter_date(self, date_filter, filter_db_name)
        self.query += ' AND id != 0'
    ...
    def _process_filter(self, field_name: str, field_filter: str, q_filter: dict):
        ...
        if field_name == 'group' and q_filter['value'] is not None:
            ...
            if q_filter['operator'] == '=':
                ...
                raise SomError(1409, f"Valid operators for 'group' field: {', '.join(valid_group_operators)}. "
                                       f"Used operator: {q_filter['operator']}")
        else:
            SomDBQuery._process_filter(self, field_name, field_filter, q_filter)


class SomDBQueryGroup(SomDBQuery):
    """Class used to query Som groups."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        backend = SomDBBackend(query_format='global')
        SomDBQuery.__init__(self, offset=offset, limit=limit, table='`group`', sort=sort, search=search,
                              select=select,
...
class SomDBQueryDistinctAgents(SomDBQueryDistinct, SomDBQueryAgents):
    pass


class SomDBQueryGroupByAgents(SomDBQueryGroupBy, SomDBQueryAgents):
    ...
    def __init__(self, filter_fields: dict, *args: dict, **kwargs: dict):
        ...
        filter_fields : dict
            Fields to filter by.
        """
        SomDBQueryAgents.__init__(self, *args, **kwargs)
        SomDBQueryGroupBy.__init__(self, *args, table=self.table, fields=self.fields, filter_fields=filter_fields,
    ...
    def _format_data_into_dictionary(self) -> str:
        ...
        return SomDBQuery._format_data_into_dictionary(self)


class SomDBQueryMultigroups(SomDBQueryAgents):
    ...
    def __init__(self, group_id: str, query: str = '', *args: dict, **kwargs: dict):
        ...
        SomDBQueryAgents.__init__(self, query=query, *args, **kwargs)
...
class Agent:
    ...
    def load_info_from_db(self, select: list = None):
        ...
        Raises
        ------
        SomResourceNotFound(1701)
            Agent does not exist.
        """
        with SomDBQueryAgents(offset=0, limit=None, sort=None, search=None, select=select,
                                query="id={}".format(self.id), count=False, get_data=True,
                                remove_extra_fields=False) as db_query:
            try:
                data = db_query.run()['items'][0]
            except IndexError:
                raise SomResourceNotFound(1701)
    ...
    def reconnect(self, wq: SomQueue) -> str:
        """Force reconnect to the manager.

        Parameters
        ----------
        wq : SomQueue
            SomQueue used for the active response message.
        ...
        # Send force reconnect message to the SomQueue
        ret_msg = wq.send_msg_to_agent(SomQueue.HC_FORCE_RECONNECT, self.id)
    ...
    def remove(self, purge: bool = False) -> str:
        ...
        str
            Message generated by Som.
        """
        # Check that som-authd is running
        try:
            manager_status = get_manager_status(cache=True)
        except SomInternalError as e:
            # som-authd is not running due to a problem with /proc availability
            raise SomError(1726, extra_message=str(e))

        if manager_status.get('som-authd') != 'running':
            # som-authd is not running
            raise SomError(1726)
        ...
        try:
            ...
            raise SomInternalError(1757, extra_message=str(e))
    ...
    def _remove_authd(self, purge: bool = False) -> dict:
        ...
        authd_socket = SomSocketJSON(common.AUTHD_SOCKET)
    ...
    def _add(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        ...
        if ip != 'any':
            if ip.find('/') > 0:
                try:
                    ...
                    raise SomError(1706, extra_message=ip)
            else:
                try:
        ...
        try:
            ...
            raise SomInternalError(1725, extra_message=str(e))

    def _add_authd(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        """Add an agent to Som using authd.
        2 uses:
            - name and ip [force]: Add an agent like manage_agents (generate id and key).
            - name, ip, id, key [force]: Insert an agent with an existing id and key.
        ...
        try:
            ...
            if e.code == 9008:
                raise SomError(1705, extra_message=name)
            elif e.code == 9007:
                raise SomError(1706, extra_message=ip)
            elif e.code == 9012:
                raise SomError(1708, extra_message=id)
            raise e
    ...
    def get_agent_os_name(self) -> str:
        ...
        query = SomDBQueryAgents(select=['os.name'], filters={'id': [self.id]})
    ...
    def get_agents_overview(offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: str = None,
        ...
        db_query = SomDBQueryAgents(offset=offset, limit=limit, sort=sort, search=search, select=select,
    ...
    async def add_group_to_agent(group_id: str, agent_id: str, replace: bool = False, replace_list: list = None) -> str:
        ...
        try:
            ...
            raise SomInternalError(2007, extra_message=str(e))
    ...
    async def unset_single_group_agent(agent_id: str, group_id: str, force: bool = False) -> str:
        ...
        if not force:
            ...
            if not Agent.group_exists(group_id):
                raise SomResourceNotFound(1710)
    ...
    def get_config(self, component: str = '', config: str = '', agent_version: str = '') -> dict:
        ...
        agent_version : str
            Agent version to compare with the required version. The format is vX.Y.Z or Som vX.Y.Z.
        ...
        """
        if SomVersion(agent_version) < SomVersion(common.ACTIVE_CONFIG_VERSION):
            raise SomInternalError(1735, extra_message=f"Minimum required version is {common.ACTIVE_CONFIG_VERSION}")
    ...
    def get_stats(self, component: str) -> dict:
        ...
        Raises
        ------
        SomInternalError(1015)
            Agent version is null.
        SomInternalError(1735)
        ...
        if self.version is None:
            raise SomInternalError(1015)
        agent_version = SomVersion(self.version.split(" ")[1])
        required_version = SomVersion(AGENT_COMPONENT_STATS_REQUIRED_VERSION.get(component))
        if agent_version < required_version:
            raise SomInternalError(1735, extra_message="Minimum required version is " + str(required_version))

        return stats.get_daemons_stats_from_socket(self.id, component)


def unify_som_upgrade_version_format(upgrade_version: str) -> str:
...
def unify_som_version_format(filters: dict):
    """Verify and format the specified som version into the 'som vX.Y.Z' standard.
    ...
    if wv is not None:
        if re.match(r'^v?\d+\.\d+\.\d+$', wv, re.IGNORECASE):
            filters['version'] = f"som {'v' if 'v' not in wv else ''}{wv}"
        elif re.match(r'^som \d+\.\d+\.\d+$', wv, re.IGNORECASE):
...
def send_restart_command(agent_id: str = '', agent_version: str = '', wq: SomQueue = None) -> str:
    ...
    wq : SomQueue
        SomQueue used for the active response messages.
    ...
    str
        Message generated by Som.
    """
    # If the Som agent version is newer or equal to the AR legacy version,
    # the message sent will have JSON format
    if SomVersion(agent_version) >= SomVersion(common.AR_LEGACY_VERSION):
        ret_msg = wq.send_msg_to_agent(SomQueue.RESTART_AGENTS_JSON, agent_id)
    else:
        ret_msg = wq.send_msg_to_agent(SomQueue.RESTART_AGENTS, agent_id)
...
def core_upgrade_agents(agents_chunk: list, command: str = 'upgrade_result', wpk_repo: str = None, version: str = None,
    ...
    """
    msg = create_som_socket_message(origin={'module': 'api'},
                                      command=command,
                                      parameters={
                                          'agents': agents_chunk,
                                          'version': unify_som_upgrade_version_format(version),
                                          'force_upgrade': force,
                                          'use_http': use_http,
    ...
    s = SomSocket(common.UPGRADE_SOCKET)

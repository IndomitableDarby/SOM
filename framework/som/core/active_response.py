...
class ARMessageBuilder:
    ...
    def choose_builder(cls, agent_version: str):
        ...
        raise SomError(1000, "No suitable message builder found for agent version: {}".format(agent_version))
...
class ARStrMessage(ARMessageBuilder):
    ...
    def can_handle(agent_version: str) -> bool:
        ...
        """
        return SomVersion(agent_version) < SomVersion(common.AR_LEGACY_VERSION)
...
class ARJsonMessage(ARMessageBuilder):
    ...
    def create_message(self, command: str = '', arguments: list = None, alert: dict = None) -> str:
        ...
        msg_queue = json.dumps(
            create_som_socket_message(origin={'name': node_name, 'module': common.origin_module.get()},
                                        command=command,
...
def send_ar_message(agent_id: str = '', agent_version = '', wq: SomQueue = None, command: str = '',
    ...
    wq.send_msg_to_agent(msg=msg_queue, agent_id=agent_id, msg_type=SomQueue.AR_TYPE)

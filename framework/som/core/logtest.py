...
from som.core.common import LOGTEST_SOCKET, DECIMALS_DATE_FORMAT, origin_module
from som.core.som_socket import SomSocketJSON, create_som_socket_message
...
def send_logtest_msg(command: str = None, parameters: dict = None) -> dict:
    ...
    """
    full_message = create_som_socket_message(origin={'name': 'Logtest', 'module': origin_module.get()},
                                               command=command,
                                               parameters=parameters)
    logtest_socket = SomSocketJSON(LOGTEST_SOCKET)

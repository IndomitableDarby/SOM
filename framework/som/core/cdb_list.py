...
from som.core.utils import find_nth, delete_som_file, to_relative_path
    ...
    for name in dir_content:
        ...
        if (path.isfile(new_absolute_path)) and ('.cdb' not in name) and ('~' not in name) and not pattern.search(name):
            if only_names:
                ...
                items = get_list_from_file(path.join(common.SOM_PATH, new_relative_path))
...
def split_key_value_with_quotes(line: str, file_path: str = '/CDB_LISTS_PATH') -> tuple:
    ...
    if line.count('"') == 4:
        ...
        if first_quote != 0 or line[second_quote: third_quote + 1] != '":"' or fourth_quote != len(
                line) - 1:
            raise SomError(1800, extra_message={'path': path.join('SOM_HOME', file_path)})
...
def get_list_from_file(path: str, raw: bool = False) -> Union[dict, str]:
    ...
    try:
        ...
        if e.errno == 2:
            ...
            raise SomError(1804, extra_message="{0} {1}".format(path, "is a directory"))
        else:
            raise e
    except ValueError:
        raise SomError(1800, extra_message={'path': path})
...
def delete_list(rel_path: str):
    ...
    """
    delete_som_file(path.join(common.SOM_PATH, rel_path))

    # Also delete .cdb file (if exists).
    try:
        remove(path.join(common.SOM_PATH, rel_path + common.COMPILED_LISTS_EXTENSION))
    except (IOError, OSError):
        pass


def get_filenames_paths(filenames_list: list, root_directory: str = common.USER_LISTS_PATH) -> list:
    """Get full paths from filename list. I.e: test_filename -> {som_path}/etc/lists/test_filename

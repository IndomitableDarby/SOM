...
import som.core.results as results
from api import configuration
from som.core import common
from som.core.exception import SomError, SomInternalError
        ...
        if match := re.match(regex, pid_file):
            try:
                ...
                if daemon.replace('-', '_') in command:
                    ...
                    print(f"{daemon}: Process {pid} does not belong to {daemon}, removing from {common.SOM_PATH}/var/run...")

            except (OSError, psutil.NoSuchProcess):
                print(f'{daemon}: Non existent process {pid}, removing from {common.SOM_PATH}/var/run...')
            finally:
...
def process_array(array: list, search_text: str = None, complementary_search: bool = False,
    ...
    """Process a Som framework data array.
...
def cut_array(array: list, offset: int = 0, limit: int = common.DATABASE_LIMIT) -> list:
    ...
    if limit is not None:
        if limit > common.MAXIMUM_DATABASE_LIMIT:
            raise SomError(1405, extra_message=str(limit))
        elif limit == 0:
            raise SomError(1406)
...
def sort_array(array: list, sort_by: list = None, sort_ascending: bool = True,
    ...
    def check_sort_fields(allowed_sort_fields, sort_by):
        ...
        if not sort_by.issubset(allowed_sort_fields):
            ...
            raise SomError(1403, extra_remediation='Allowed sort fields: {0}. '
    ...
    if sort_by:  # array should be a dictionary or a Class
        ...
        if type(array) is set or (type(array[0]) is not dict and 'class \'som' not in str(type(array[0]))):
...
def select_array(array: list, select: list = None, required_fields: set = None,
    ...
    if allowed_select_fields and not select_no_nested.issubset(allowed_select_fields):
        raise SomError(1724, "{}".format(', '.join(select_no_nested)))
    ...
    for item in array:
        ...
        if array and not allowed_select_fields and not selected_fields:
            raise SomError(1724, "{}".format(', '.join(select)))
...
def delete_som_file(full_path: str) -> bool:
    """Delete a Som file.
    ...
    bool
        True if success.
    """
    if not full_path.startswith(common.SOM_PATH) or '..' in full_path:
        raise SomError(1907)
...
def _get_hashing_algorithm(hash_algorithm):
    ...
    if hash_algorithm not in algorithm_list:
        raise SomError(1723, "Available algorithms are {0}.".format(', '.join(algorithm_list)))
...
def check_som_limits_unchanged(new_conf, original_conf):
    """Check if Som limits remain unchanged.
    ...
    for disabled_limit in [conf for conf, allowed in limits_configuration.items() if not allowed['allow']]:
        ...
        if len(new_limits) != len(original_limits) or any(x != y for x, y in zip(new_limits, original_limits)):
            raise SomError(1127, extra_message=f"global > limits > {disabled_limit}")
...
def check_indexer(new_conf: str, original_conf: str):
    ...
    if not upload_configuration['indexer']['allow']:
        ...
        if len(new_indexer) != len(original_indexer) or any(x != y for x, y in zip(new_indexer, original_indexer)):
            raise SomError(1127, extra_message='indexer')
    ...
    if not blocked_configurations['public_key']['allow']:
        ...
        for api_key in api_keys:
            ...
            try:
                ...
                raise SomError(1131, extra_message=f'{extra_msg}')
            if response_minimum_quota == minimum_quota:
                raise SomError(1130, extra_message='integrations > virustotal')


def load_som_xml(xml_path, data=None):
    if not data:
        with open(xml_path) as f:
            try:
                data = f.read()
            except Exception as e:
                raise SomError(1113, extra_message=str(e))
...
class SomVersion:
    ...
    def __init__(self, version):
        ...
        pattern = r"(?:Som )?v?(\d+)\.(\d+)\.(\d+)\-?(alpha|beta|rc)?(\d*)"
        ...
        for or_clause in or_clauses:
            ...
            for and_clause in and_clauses:
                ...
                try:
                    ...
                    raise SomError(1407, extra_message=f"Parameter 'q' is not valid: '{and_clause}'")
...
class SomDBBackend(AbstractDatabaseBackend):
    """
    This class describes a som db backend that executes database queries.
    """
    ...
    def __init__(self, agent_id=None, query_format='agent', request_slice=500):
        if query_format == 'agent' and not path.exists(path.join(common.WDB_PATH, f"{agent_id}.db")):
            raise SomError(2007, extra_message=f"There is no database for agent {agent_id}. "
    ...
    def connect_to_db(self):
        return SomDBConnection(request_slice=self.request_slice)
...
class SomDBQuery(object):
    ...
    def __init__(self, offset: int, limit: int, table: str, sort: dict, search: dict, select: list, query: str,
        ...
        # For example, the following regex -> (name!=som;id>5),group=webserver <- would return 3 different matches:
        #   (name != som ;
        #    id   > 5      ),
        #    group=webserver
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        isinstance(self.backend, SomDBBackend) and self.backend.close_connection()
    ...
    def _add_limit_to_query(self):
        if self.limit:
            if self.limit > common.MAXIMUM_DATABASE_LIMIT:
                raise SomError(1405, extra_message=str(self.limit))
        ...
        if self.sort:
            if self.sort['fields']:
                ...
                if not set(sort_fields).issubset(allowed_sort_fields):
                    raise SomError(1403, "Allowed sort fields: {}. Fields: {}".format(
    ...
    def _parse_select_filter(self, select_fields):
        if select_fields:
            ...
            if not select_fields_set or not select_fields_set.issubset(allowed_select_fields):
                raise SomError(1724, "Allowed select fields: {0}. Fields: {1}". \
    ...
    def _parse_query(self):
        ...
        for open_level, field, operator, value, close_level, separator in self.query_regex.findall(self.q):
            if field not in allowed_query_fields:
                raise SomError(1408, "Available fields: {}. Field: {}".format(
    ...
    def _filter_date(self, date_filter, filter_db_name):
        ...
        if date_filter['value'].isdigit() or re.match(r'\d+[dhms]', date_filter['value']):
            ...
            raise SomError(1412, date_filter['value'])
    ...
    def oversized_run(self) -> dict:
        ...
        final_ids = list()
        resources = list()
        if self.__class__.__name__ == 'SomDBQueryAgents':
            resource = 'id'
        elif self.__class__.__name__ == 'SomDBQueryGroups':
            resource = 'name'
        else:
            raise SomInternalError(1123)
...
class SomDBQueryDistinct(SomDBQuery):
    ...
    def _add_filters_to_query(self):
        SomDBQuery._add_filters_to_query(self)
        self.query += ' WHERE ' if not self.q and 'WHERE' not in self.query else ' AND '
        self.query += ' AND '.join(
            ["{0} IS NOT null AND {0} != ''".format(self.fields[field]) for field in self.select])
    ...
    def _add_select_to_query(self):
        ...
        SomDBQuery._add_select_to_query(self)
...
def validate_som_xml(content: str, config_file: bool = False):
    """Validate Som XML files (rules, decoders and ossec.conf)
    ...
    try:
        ...
        if config_file:
            ...
            check_som_limits_unchanged(final_xml, current_xml)
        # Check xml format
        load_som_xml(xml_path='', data=final_xml)
    except ExpatError:
        raise SomError(1113)
    except SomError as e:
        raise e
    except Exception as e:
        raise SomError(1113, str(e))
...
def upload_file(content: str, file_path: str, check_xml_formula_values: bool = True):
    ...
    Raises
    ------
    SomInternalError(1005)
        Error reading file.
    SomInternalError(1016)
        Error moving file.
    SomError(1006)
    ...
    try:
        ...
        raise SomInternalError(1005) from exc

    # Move temporary file to group folder
    try:
        new_conf_path = path.join(common.SOM_PATH, file_path)
        safe_move(tmp_file_path, new_conf_path, ownership=(common.som_uid(), common.som_gid()), permissions=0o660)
    except PermissionError as exc:
        raise SomError(1006) from exc
    except Error as exc:
        raise SomInternalError(1016) from exc

    return results.SomResult({'message': 'File was successfully updated'})
...
def to_relative_path(full_path: str, prefix: str = common.SOM_PATH) -> str:
    """Return a relative path from the Som base directory.
    ...
    full_path : str
        Absolute path.
    prefix : str, opt
        Prefix to strip from the absolute path. Default `common.SOM_PATH`
...
def check_if_som_agent_version(version_str: str) -> bool:
    """Check if the string has the expected som agent version format.

    Parameters
    ----------
    version_str : str
        The som version string.

    Returns
    -------
    bool
        True if the string has the expected som version format.

    """
    if not isinstance(version_str, str):
        return False

    return bool(re.match(r'^Som v(\d+)\.(\d+)\.(\d+)', version_str))


def parse_som_agent_version(version_str: str) -> tuple:
    ...
    -------
    tuple
        The tuple of the som version string.
    """

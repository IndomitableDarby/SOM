...
import som.core.configuration as configuration
...
from som.core.rule import check_status, load_rules_from_file, format_rule_decoder_file, REQUIRED_FIELDS, \
    RULE_REQUIREMENTS, SORT_FIELDS, RULE_FIELDS, RULE_FILES_FIELDS, RULE_FILES_REQUIRED_FIELDS
from som.core.utils import process_array, safe_move, \
    validate_som_xml, upload_file, full_copy, to_relative_path
from som.core.logtest import validate_dummy_logtest
...
def get_rules(rule_ids: list = None, status: str = None, group: str = None, pci_dss: str = None, gpg13: str = None,
    ...
    """
    result = AffectedItemsSomResult(none_msg='No rule was returned',
    ...
    for rule_id in no_existent_ids:
        result.add_failed_item(id_=rule_id, error=SomError(1208))
...
def get_rules_files(status: str = None, relative_dirname: str = None, filename: list = None, offset: int = 0,
                    ...
                    distinct: bool = False) -> AffectedItemsSomResult:
    ...
    ------
    SomError(1200)
        Error reading rules from `SOM_HOME/etc/ossec.conf`.
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(none_msg='No rule files were returned',
...
def get_groups(offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None, sort_ascending: bool = True,
               search_text: str = None, complementary_search: bool = False,
               search_in_fields: list = None) -> AffectedItemsSomResult:
    ...
    AffectedItemsSomResult
        Dictionary: {'items': array of items, 'totalItems': Number of items (without applying the limit)}
    """
    result = AffectedItemsSomResult(none_msg='No groups in rules were returned',
...
def get_requirement(requirement: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None,
    ...
    if requirement not in RULE_REQUIREMENTS:
        result.add_failed_item(id_=requirement,
                               error=SomError(1205, extra_message=requirement,
...
def get_rule_file_path(filename: str = None, relative_dirname: str = None) -> str:
    ...
    if len(rules) == 0:
        ...
        return join(common.SOM_PATH, rule['relative_dirname'], filename)
    else:
        return normpath(join(common.SOM_PATH, rules[0]['relative_dirname'], filename))


def get_rule_file(filename: str = None, raw: bool = False,
                  relative_dirname: str = None) -> Union[str, AffectedItemsSomResult]:
    ...
    Returns
    -------
    str or AffectedItemsSomResult
        Content of the file. AffectedItemsSomResult format if `raw=False`.
    """
    result = AffectedItemsSomResult(none_msg='No rule was returned',
    ...
    if not full_path:
        result.add_failed_item(id_=filename,
                               error=SomError(1415, extra_message=f"{filename}"))
        return result
    ...
    try:
        ...
        result.add_failed_item(id_=filename,
                               error=SomError(1413, extra_message=f"{filename}: {str(exc)}"))
    except OSError:
        result.add_failed_item(id_=filename,
                               error=SomError(1414, extra_message=f"{filename}"))

    return result


def validate_upload_delete_dir(relative_dirname: Union[str, None]) -> Tuple[str, SomError]:
    ...
    Tuple (str, SomError)
        ...
        The second element of the tuple is a SomError exception
            If relative_dirname has no 'rule_dir' tag in ruleset return SomError(1505).
            If relative_dirname is inside the default RULES_PATH return SomError(1506).
            If relative_dirname has a 'rule_dir' tag in ruleset but it doesn't exists return SomError(1507).
    ...
    if relative_dirname not in ruleset_conf['rule_dir']:
        som_error = SomError(1209)
    elif commonpath([join(common.SOM_PATH, relative_dirname), common.RULES_PATH]) == common.RULES_PATH:
        som_error = SomError(1210)
    elif not exists(join(common.SOM_PATH, relative_dirname)):
        som_error = SomError(1211)
    return relative_dirname, som_error


@expose_resources(actions=['rules:update'], resources=['*:*:*'])
def upload_rule_file(filename: str, content: str, relative_dirname: str = None,
                     overwrite: bool = False) -> AffectedItemsSomResult:
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(all_msg='Rule was successfully uploaded',
                                      none_msg='Could not upload rule')

    backup_file = ''
    try:
        relative_dirname, som_error = validate_upload_delete_dir(
            relative_dirname=relative_dirname)
        full_path = join(common.SOM_PATH, relative_dirname, filename)
        if som_error:
            raise som_error
        if len(content) == 0:
            raise SomError(1112)

        validate_som_xml(content)
...
def delete_rule_file(filename: Union[str, list], relative_dirname: str = None) -> AffectedItemsSomResult:
    """Delete a rule file.
    ...
    result = AffectedItemsSomResult(all_msg='Rule was successfully deleted',
                                      none_msg='Could not delete rule')

    try:
        relative_dirname, som_error = validate_upload_delete_dir(
            relative_dirname=relative_dirname)
        full_path = join(common.SOM_PATH, relative_dirname, file)
        if som_error:
            raise som_error

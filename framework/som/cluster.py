...
from som.core.cluster.control import get_health, get_nodes, get_node_ruleset_integrity
from som.core.cluster.utils import get_cluster_status, read_cluster_config, read_config
...
def read_config_wrapper() -> AffectedItemsSomResult:
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(all_msg='All selected information was returned',
...
@expose_resources(actions=['cluster:read'], resources=[f'node:id:{node_id}'])
def get_node_wrapper() -> AffectedItemsSomResult:
...
def get_status_json() -> SomResult:
    """Return the cluster status.

    Returns
    -------
    SomResult
        SomResult object with the cluster status.
    """
    return SomResult({'data': get_cluster_status()})
...
async def get_health_nodes(lc: local_client.LocalClient,
                           filter_node: Union[str, list] = None) -> AffectedItemsSomResult:
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(all_msg='All selected nodes healthcheck information was returned',
...
async def get_nodes_info(lc: local_client.LocalClient, filter_node: Union[str, list] = None,
                         **kwargs: dict) -> AffectedItemsSomResult:
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(all_msg='All selected nodes information was returned',
    ...
    for node in non_existent_nodes:
        result.add_failed_item(id_=node, error=SomResourceNotFound(1730))
...
async def get_ruleset_sync_status(master_md5: dict = None):
    ...
    """
    result = AffectedItemsSomResult(all_msg="Nodes ruleset synchronization status was successfully read",

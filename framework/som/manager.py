...
from som import Som
from som.core import common, configuration
from som.core.cluster.cluster import get_node
from som.core.cluster.utils import manager_restart, read_cluster_config, running_in_master_node
from som.core.configuration import get_ossec_conf, write_ossec_conf
from som.core.exception import SomError, SomInternalError
from som.core.manager import status, get_api_conf, get_update_information_template, get_ossec_logs, \
    get_logs_summary, validate_ossec_conf, OSSEC_LOG_FIELDS
from som.core.results import AffectedItemsSomResult, SomResult
from som.core.utils import process_array, safe_move, validate_som_xml, full_copy
from som.rbac.decorators import expose_resources, mask_sensitive_config
...
def get_status() -> AffectedItemsSomResult:
    """Wrapper for status().
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(all_msg=f"Processes status was successfully read"
...
def ossec_log(level: str = None, tag: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
              ...
              q: str = '', select: str = None, distinct: bool = False) -> AffectedItemsSomResult:
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(all_msg=f"Logs were successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:read"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def ossec_log_summary() -> AffectedItemsSomResult:
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(all_msg=f"Log was successfully summarized"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
def get_api_config() -> AffectedItemsSomResult:
    ...
    """
    result = AffectedItemsSomResult(**_get_config_default_result_kwargs)
...
def restart() -> AffectedItemsSomResult:
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(**_restart_default_result_kwargs)
    try:
        manager_restart()
...
def validation() -> AffectedItemsSomResult:
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(**_validation_default_result_kwargs)
...
def get_config(component: str = None, config: str = None) -> AffectedItemsSomResult:
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(all_msg=f"Active configuration was successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
def read_ossec_conf(section: str = None, field: str = None, raw: bool = False,
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(all_msg=f"Configuration was successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:read"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def get_basic_info() -> AffectedItemsSomResult:
    """Wrapper for Som().to_dict
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(all_msg=f"Basic information was successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
                                      some_msg='Could not read basic information in some nodes',
    ...
    try:
        result.affected_items.append(Som().to_dict())
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:update_config"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def update_ossec_conf(new_conf: str = None) -> AffectedItemsSomResult:
    """Replace som configuration (ossec.conf) with the provided configuration.
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(all_msg=f"Configuration was successfully updated"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
    ...
    try:
        ...
        validate_som_xml(new_conf, config_file=True)
...
def get_update_information(installation_uid: str, update_information: dict) -> SomResult:
    """Process update information into a som result.
    ...
    if not update_information:
        ...
        return SomResult({'data': get_update_information_template(uuid=installation_uid, update_check=False)})
    ...
    if status_code != 200:
        ...
        raise SomInternalError(2100, extra_message=extra_message)

    update_information.pop('message', None)

    return SomResult({'data': update_information})

...
from som.rbac.tests.utils import init_db
from som.rbac.orm import SOM_USER_ID, SOM_WUI_USER_ID, MAX_ID_RESERVED, User
...
def db_setup():
    with patch('som.core.common.som_uid'), patch('som.core.common.som_gid'):
        with patch('sqlalchemy.create_engine', return_value=create_engine("sqlite://")):
            with patch('shutil.chown'), patch('os.chmod'):
                with patch('api.constants.SECURITY_PATH', new=test_data_path):
                    import som.rbac.orm as rbac
                    # Clear mappers
...
def test_database_init(db_setup):
    ...
    with db_setup.RolesManager() as rm:
        assert rm.get_role('som') != db_setup.SecurityError.ROLE_NOT_EXIST
...
def test_delete_all_expired_rules(db_setup):
    """Check that rules are correctly deleted"""
    with patch('som.rbac.orm.time', return_value=0):
        add_token(db_setup)
    with db_setup.TokenManager() as tm:
...
@patch("som.rbac.orm.create_engine")
@patch("som.rbac.orm.sessionmaker")
...
@patch("som.rbac.orm._Base.metadata.create_all")
...
def test_databasemanager_get_table(fresh_in_memory_db):
    ...
    with patch("som.rbac.orm._new_columns", new={"new_column"}):
...
@patch("som.rbac.orm.safe_move")
@patch("som.rbac.orm.os.remove")
@patch("som.rbac.orm.chown")
@patch("som.rbac.orm.os.chmod")
def test_check_database_integrity(chmod_mock, chown_mock, remove_mock, safe_move_mock, fresh_in_memory_db):
    ...
    db_mock = MagicMock()
    with patch("som.rbac.orm.db_manager", new=db_mock):
        with patch("som.rbac.orm.os.path.exists", return_value=True):
            with patch("som.rbac.orm.CURRENT_ORM_VERSION", new=99999):
                ...
                db_mock.assert_has_calls([
                    ...
                    call.migrate_data(source=fresh_in_memory_db.DB_FILE, target=fresh_in_memory_db.DB_FILE_TMP,
                                      from_id=fresh_in_memory_db.SOM_USER_ID,
                                      to_id=fresh_in_memory_db.SOM_WUI_USER_ID),
                    call.migrate_data(source=fresh_in_memory_db.DB_FILE, target=fresh_in_memory_db.DB_FILE_TMP,
        ...
        safe_move_mock.assert_called_once_with(fresh_in_memory_db.DB_FILE_TMP, fresh_in_memory_db.DB_FILE,
                                               ownership=(fresh_in_memory_db.som_uid(),
                                                          fresh_in_memory_db.som_gid()),
                                               permissions=0o640)
...
@patch("som.rbac.orm.DatabaseManager.close_sessions")
@patch("som.rbac.orm.os.remove")
def test_check_database_integrity_exceptions(remove_mock, close_sessions_mock, exception, fresh_in_memory_db):
    ...
    with patch("som.rbac.orm.os.path.exists", side_effect=mocked_exists) as mock_exists:
        with patch("som.rbac.orm.DatabaseManager.connect", side_effect=exception) as db_manager_mock:
...
@pytest.mark.parametrize('from_id, to_id, users', [
    (SOM_USER_ID, SOM_WUI_USER_ID, [
        User('som', 'test', user_id=SOM_USER_ID),
        User('som-wui', 'test2', user_id=SOM_WUI_USER_ID)
    ]),
...
def test_migrate_data(db_setup, from_id, to_id, users):
    ...
    if to_id == SOM_WUI_USER_ID:
        with patch("som.rbac.orm.db_manager.get_data", return_value=users):
            with patch("som.rbac.orm.AuthenticationManager.update_user") as mock_update_user:
        ...
        empty_list = []
        user_exists = False
        with patch("som.rbac.orm.AuthenticationManager.add_user",
                   side_effect=[not user_exists, user_exists, not user_exists]) as mock_add_user:
            with patch("som.rbac.orm.db_manager.get_data",

...
from som.core.exception import SomInternalError
...
with patch('som.core.common.getgrnam'):
    with patch('som.core.common.getpwnam'):
        ...
        from som.cdb_list import get_lists, get_path_lists, iterate_lists, get_list_file, upload_list_file,\
            delete_list_file
...
RELATIVE_PATH = os.path.join("framework", "som", "tests", "data", "test_cdb_list")
NAME_FILE_1 = "test_lists_1"
NAME_FILE_2 = "test_lists_2"
NAME_FILES = [NAME_FILE_1, NAME_FILE_2]

RESULT_GET_LIST_FILE_1 = [{'items': [{'key': 'test-som-w', 'value': 'write'},
                                     {'key': 'test-som-r', 'value': 'read'},
                                     {'key': 'test-som-a', 'value': 'attribute'},
                                     {'key': 'test-som-x', 'value': 'execute'},
                                     {'key': 'test-som-c', 'value': 'command'}
                                     ],
...
@patch('som.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH)
...
@pytest.mark.parametrize("search_text, complementary_search, search_in_fields, paths, expected_result", [
    ...
    ("test-som-w", False, "items", NAME_FILES, RESULT_GET_LIST_FILE_1),
    ("test-ossec-w", False, "items", NAME_FILES, RESULT_GET_LIST_FILE_2),
    ("test-som-w", False, "items", [NAME_FILE_2], []),
    ...
    ("test-som-w", True, "items", NAME_FILES, RESULT_GET_LIST_FILE_2),
    ("test-ossec-w", True, "items", NAME_FILES, RESULT_GET_LIST_FILE_1),
    ("test-som-w", True, "items", [NAME_FILE_2], RESULT_GET_LIST_FILE_2),
])
@patch('som.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH)
...
def test_get_lists_sort():
    ...
    assert isinstance(result_a, AffectedItemsSomResult)
    assert isinstance(result_b, AffectedItemsSomResult)
...
@patch('som.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH)
@patch('som.cdb_list.iterate_lists', side_effect=lists_path_mock)
...
def test_get_list_file(filename, raw, expected_result, total_failed_items):
    ...
    """
    with patch('som.cdb_list.get_filenames_paths', return_value=[os.path.join(DATA_PATH, 'test_lists_2')]):
        result = get_list_file([filename], raw)
        if raw:
            assert result == expected_result
        else:
            isinstance(result, AffectedItemsSomResult)
...
@patch('som.cdb_list.safe_move')
@patch('som.cdb_list.delete_file_with_backup')
@patch('som.cdb_list.upload_file')
@patch('som.cdb_list.delete_list_file')
@patch('som.cdb_list.remove')
@patch('som.cdb_list.exists', return_value=True)
...
@patch('som.cdb_list.common.USER_LISTS_PATH', return_value='/test/path')
@patch('som.cdb_list.remove')
def test_upload_list_file_ko(mock_remove, mock_lists_path):
    ...
    with patch('som.cdb_list.safe_move') as mock_safe_move:
        with patch('som.cdb_list.exists', return_value=True):
            ...
            with patch('som.cdb_list.get_filenames_paths', return_value=['/test']):
        ...
        with patch('builtins.open'):
            with patch('som.cdb_list.exists', return_value=False):
                with patch('som.core.configuration.tempfile.mkstemp', return_value=['mock_handle', 'mock_tmp_file']):
                    with pytest.raises(SomInternalError, match=r'\b1005\b'):
...
def test_delete_list_file(mock_delete_file):
    ...
    try:
        ...
        with patch('som.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH):
            result = delete_list_file(['test_file'])
            assert result.render()['data']['affected_items'][0] ==\
                   'framework/som/tests/data/test_cdb_list/test_file'
    finally:
        try:
            os.remove(test_file)
        except Exception:

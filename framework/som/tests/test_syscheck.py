...
with patch('som.core.common.som_uid'):
    with patch('som.core.common.som_gid'):
        ...
        from som.syscheck import run, clear, last_scan, files
        from som.syscheck import AffectedItemsSomResult
        from som import SomError, SomInternalError
...
@patch('som.core.common.CLIENT_KEYS', new=os.path.join(test_agent_data_path, 'client.keys'))
@patch('som.syscheck.SomDBQueryAgents.__exit__')
@patch('som.syscheck.SomDBQueryAgents.__init__', return_value=None)
@patch('som.syscheck.SomQueue._connect')
@patch('som.syscheck.SomQueue.send_msg_to_agent', side_effect=set_callable_list)
@patch('som.syscheck.SomQueue.close')
def test_syscheck_run(close_mock, send_mock, connect_mock, agent_init_mock, agent_exit_mock,
    ...
    """
    with patch('som.syscheck.SomDBQueryAgents.run', return_value=failed_items[0]):
...
@patch('som.core.wdb.SomDBConnection.__init__', return_value=None)
@patch('som.core.wdb.SomDBConnection.execute', return_value=None)
@patch('som.core.wdb.SomDBConnection.close')
def test_syscheck_clear(wdb_close_mock, wdb_execute_mock, wdb_init_mock, agent_list, expected_result, agent_info_list,
    ...
    """
    with patch('som.syscheck.get_agents_info', return_value=set(agent_info_list)), \
            patch('som.syscheck.SomDBQueryAgents') as mock_wdbqa:
...
@patch('som.core.wdb.SomDBConnection.__init__', return_value=None)
@patch('som.core.wdb.SomDBConnection.execute', side_effect=SomError(1000))
@patch('som.core.wdb.SomDBConnection.close')
...
@pytest.mark.parametrize('agent_id, som_version', [
    (['001'], {'version': 'Som v3.6.0'}),
    (['002'], {'version': 'Som v3.8.3'}),
    (['005'], {'version': 'Som v3.5.3'}),
    (['006'], {'version': 'Som v3.9.4'}),
    (['004'], {}),
])
...
@patch("som.syscheck.SomDBConnection.execute", return_value=[{'end': '', 'start': ''}])
@patch('socket.socket.connect')
def test_syscheck_last_scan(socket_mock, wdb_conn_mock, db_mock, exists_mock, agent_id, som_version):
    ...
    agent_id : list
        Agent ID.
    som_version : dict
        Dict with the Som version to be applied.
    """
    with patch('som.syscheck.Agent.get_basic_information', return_value=som_version):
...
@patch('som.core.utils.path.exists', return_value=True)
@patch('socket.socket.connect')
@patch('som.core.common.WDB_PATH', new=test_data_path)

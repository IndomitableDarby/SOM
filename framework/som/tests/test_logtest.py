...
with patch('som.common.som_uid'):
    with patch('som.common.som_gid'):
        ...
        from som.logtest import run_logtest, end_logtest_session


def send_logtest_msg_mock(**kwargs):
    socket_response = create_som_socket_message(command=kwargs['command'], parameters=kwargs['parameters'])
...
def test_get_logtest_output(logtest_param_values):
    ...
    with patch('som.logtest.send_logtest_msg') as send_mock:

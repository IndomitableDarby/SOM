...
with patch('som.core.common.som_uid'):
    with patch('som.core.common.som_gid'):
        ...
        from som.manager import *
        from som.core.manager import LoggingFormat
        from som.core.tests.test_manager import get_logs
        from som import SomInternalError
...
manager_status = {'som-agentlessd': 'running', 'som-analysisd': 'running', 'som-authd': 'running',
 'som-csyslogd': 'running', 'som-dbd': 'running', 'som-monitord': 'running',
 'som-execd': 'running', 'som-integratord': 'running', 'som-logcollector': 'running',
 'som-maild': 'running', 'som-remoted': 'running', 'som-reportd': 'running',
 'som-syscheckd': 'running', 'som-clusterd': 'running', 'som-modulesd': 'running',
 'som-db': 'running', 'som-apid': 'running'}


@patch('som.core.manager.status', return_value=manager_status)
...
@pytest.mark.parametrize('tag, level, total_items, sort_by, sort_ascending', [
    ...
    ('som-modulesd:database', None, 2, None, None),
    ('som-modulesd:syscollector', None, 2, None, None),
    ('som-modulesd:syscollector', None, 2, None, None),
    ('som-modulesd:aws-s3', None, 5, None, None),
    ('som-execd', None, 1, None, None),
    ('som-csyslogd', None, 2, None, None),
...
def test_ossec_log(mock_exists, mock_active_logging_format, tag, level, total_items, sort_by, sort_ascending):
    ...
    tag : str
        Filters by log category (i.e. som-remoted).
    total_items : int
    ...
    """
    with patch('som.core.manager.tail') as tail_patch:
        ...
        if tag is not None and level != 'som-modulesd:syscollector':
...
def test_ossec_log_summary(mock_exists, mock_active_logging_format):
    ...
    expected_result = {
        'som-csyslogd': {'all': 2, 'info': 2, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0},
        'som-execd': {'all': 1, 'info': 0, 'error': 1, 'critical': 0, 'warning': 0, 'debug': 0},
        'som-modulesd:aws-s3': {'all': 5, 'info': 2, 'error': 1, 'critical': 0, 'warning': 2, 'debug': 0},
        'som-modulesd:database': {'all': 2, 'info': 0, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 2},
        'som-modulesd:syscollector': {'all': 2, 'info': 2, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0},
        'som-rootcheck': {'all': 1, 'info': 1, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0}
    }
...
@patch('socket.socket')
@patch('som.core.cluster.utils.fcntl')
@patch('som.core.cluster.utils.open')
...
def test_restart_ko_socket(mock_exists, mock_fcntl, mock_open):
    ...
    with patch("os.path.exists", return_value=True):
        with patch('socket.socket', side_effect=socket.error):
            with pytest.raises(SomInternalError, match='.* 1902 .*'):
                restart()
...
def test_validation(mock_exists, error_flag, error_msg):
    ...
    """
    with patch('som.core.manager.SomSocket') as sock:
        # Mock sock response
...
@patch('som.core.configuration.get_active_configuration')
...
@patch('som.manager.validate_ossec_conf', return_value={'status': 'OK'})
@patch('som.manager.write_ossec_conf')
@patch('som.manager.validate_som_xml')
@patch('som.manager.full_copy')
@patch('som.manager.exists', return_value=True)
@patch('som.manager.remove')
@patch('som.manager.safe_move')

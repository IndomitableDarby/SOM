...
with patch('som.core.common.som_uid'):
    with patch('som.core.common.som_gid'):
        ...
        import som.stats as stats
        from som.core.results import AffectedItemsSomResult
        from api.util import remove_nones_to_dict
        from som.core.tests.test_agent import InitAgent

SOCKET_PATH_DAEMONS_MAPPING = {'/var/ossec/queue/sockets/remote': 'som-remoted',
                               '/var/ossec/queue/sockets/analysis': 'som-analysisd'}
DAEMON_SOCKET_PATHS_MAPPING = {'som-remoted': '/var/ossec/queue/sockets/remote',
                               'som-analysisd': '/var/ossec/queue/sockets/analysis'}
...
def test_totals():
    ...
    with patch('som.stats.totals_', return_value=({})):
        response = stats.totals(date(2019, 8, 13))
        assert response.total_affected_items == len(response.affected_items)
        assert isinstance(response, AffectedItemsSomResult), 'The result is not SomResult type'
...
@patch('som.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/remote')
@patch('som.core.common.ANALYSISD_SOCKET', '/var/ossec/queue/sockets/analysis')
@patch('som.core.common.WDB_SOCKET', '/var/ossec/queue/db/wdb')
@patch('som.stats.get_daemons_stats_socket')
def test_get_daemons_stats(mock_get_daemons_stats_socket):
    """Makes sure get_daemons_stats() fit with the expected."""
    response = stats.get_daemons_stats(['som-remoted', 'som-analysisd', 'som-db'])
    ...
    assert isinstance(response, AffectedItemsSomResult), 'The result is not AffectedItemsSomResult type'
    assert response.total_affected_items == len(response.affected_items)


@patch('som.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/wrong_socket_name')
def test_get_daemons_stats_ko():
    """Makes sure get_daemons_stats() fit with the expected."""
    response = stats.get_daemons_stats(['som-remoted'])
...
@pytest.mark.parametrize('daemons_list, expected_daemons_list', [
    ([], ['som-remoted', 'som-analysisd']),
    (['som-remoted'], ['som-remoted']),
    (['som-remoted', 'som-analysisd'], ['som-remoted', 'som-analysisd'])
])
@patch('som.core.wdb.SomDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
@patch('som.stats.get_agents_info', return_value={'000', '001', '002', '003', '004', '005'})
@patch('som.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/remote')
@patch('som.core.common.ANALYSISD_SOCKET', '/var/ossec/queue/sockets/analysis')
@patch('som.stats.get_daemons_stats_socket', side_effect=side_effect_test_get_daemons_stats)
def test_get_daemons_stats_agents(mock_get_daemons_stats_socket, mock_get_agents_info, mock_socket_connect,
    ...
    assert isinstance(result, AffectedItemsSomResult), 'The result is not an AffectedItemsSomResult object'
...
@patch('som.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/remote')
@patch('som.core.common.ANALYSISD_SOCKET', '/var/ossec/queue/sockets/analysis')
@patch('som.stats.get_daemons_stats_socket', side_effect=side_effect_test_get_daemons_stats_all)
...
@patch('som.stats.get_daemons_stats_', return_value=[{"events_decoded": 1.0}])
...
@patch('som.core.agent.Agent.get_stats')
@patch('som.stats.get_agents_info', return_value=['000', '001'])

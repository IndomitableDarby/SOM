...
with patch('som.core.common.som_uid'):
    with patch('som.core.common.som_gid'):
        ...
        from som.agent import add_agent, assign_agents_to_group, create_group, delete_agents, delete_groups, \
        ...
        from som.core.exception import SomResourceNotFound
        from som.core.wdb_http import AgentsSummary
...
@pytest.mark.parametrize('fields, expected_items', [
    ...
    (
            ['version'],
            [{'version': 'Som v3.9.0', 'count': 1}, {'version': 'Som v3.8.2', 'count': 2},
             {'version': 'Som v3.6.2', 'count': 1}, {'version': 'N/A', 'count': 2}]
    ),
    (
...
@patch('som.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
...
def test_agent_get_distinct_agents(socket_mock, send_mock, fields, expected_items):
    ...
    assert isinstance(distinct, AffectedItemsSomResult), 'The returned object is not an "AffectedItemsSomResult".'
...
def test_agent_sort_order(socket_mock, send_mock, fields, order, expected_items):
    ...
    assert isinstance(sorted_agents, AffectedItemsSomResult), 'The returned object is not an ' \
...
def test_agent_get_agents_summary_status(socket_mock, send_mock):
    ...
    assert isinstance(summary, SomResult), 'The returned object is not an "SomResult" instance.'
...
def test_agent_get_agents_summary_os(connect_mock, send_mock):
    ...
    assert isinstance(summary, AffectedItemsSomResult), 'The returned object is not an "SomResult" instance.'
...
@patch('som.core.agent.Agent.reconnect')
@patch('som.agent.get_agents_info', return_value=short_agent_list)
@patch('som.core.wdb.SomDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect_agents(socket_mock, send_mock, agents_info_mock, reconnect_mock, agent_list, expected_items,
    ...
    assert isinstance(result, AffectedItemsSomResult), 'The returned object is not an "AffectedItemsSomResult".'
...
@patch('som.agent.send_restart_command')
@patch('som.agent.get_agents_info', return_value=set(short_agent_list))
@patch('som.core.wdb_http.SomDBHTTPClient._post', side_effect=send_msg_to_wdb_http_post_restartinfo)
@patch('socket.socket.connect')
...
@patch('som.agent.get_agents')
@patch('som.agent.get_groups')
@patch('som.core.wdb.SomDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_agents_in_group(socket_mock, send_mock, mock_get_groups, mock_get_agents, group, group_exists,
    ...
    if group_exists:
        ...
        with pytest.raises(SomResourceNotFound, match='.* 1710 .*'):
            get_agents_in_group(group_list=[group])


@pytest.mark.parametrize('group, q, expected_q', [
    ('default', '(name~som,status~active)', 'group=default;((name~som,status~active))'),
    ('default', 'name~som,status~active', 'group=default;(name~som,status~active)')
])
@patch('som.agent.get_agents')
@patch('som.agent.get_groups')
...
@pytest.mark.parametrize('agent_list, filters, q, error_code, expected_items', [
    ...
    (full_agent_list[1:], {'status': 'all', 'older_than': '1s', 'version': 'Som v3.6.2'}, None, 1731, ['002']),
    ...
    (['001', '002'], {'status': 'all', 'older_than': '1s'}, None, SomError(1726), None),
])
@patch('som.agent.Agent.remove')
@patch('som.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('som.core.wdb.SomDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_delete_agents(socket_mock, send_mock, mock_remove, agent_list, filters, q, error_code, expected_items):
    ...
    """
    if not isinstance(error_code, SomException):
...
@patch('som.core.agent.SomSocketJSON')
@patch('som.core.agent.get_manager_status', return_value={'som-authd': 'running'})
...
@patch('som.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('som.core.common.SHARED_PATH', new=test_shared_path)
@patch('som.core.wdb.SomDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
def test_agent_get_agent_groups_exceptions(socket_mock, send_mock, mock_get_groups, db_global, system_groups,
    ...
    with patch('som.core.common.DATABASE_PATH_GLOBAL', new=db_global):
        try:
...
@patch('som.core.common.DATABASE_PATH_GLOBAL', new=test_global_bd_path)
@patch('som.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('som.core.common.SHARED_PATH', new=test_shared_path)
...
@pytest.mark.parametrize('shared_path, group_list, group_exists, side_effect, expected_exception', [
    (test_shared_path, ['none'], False, None, SomResourceNotFound(1710)),
    ('invalid-path', ['default'], True, None, SomError(1006)),
    (test_shared_path, ['default'], True, SomError(1405), SomError(1405)),
    (test_shared_path, ['default'], True, SomException(1400), SomInternalError(1727))
])
@patch('som.agent.process_array')
@patch('som.core.agent.Agent.group_exists')
def test_agent_get_group_files_exceptions(mock_group_exists, mock_process_array, shared_path, group_list, group_exists,
    ...
    """
    with patch('som.core.common.SHARED_PATH', new=shared_path):
        ...
        except (SomError, SomInternalError) as e:
...
@patch('som.core.common.SHARED_PATH', new=test_shared_path)
@patch('som.core.common.som_gid', return_value=getgrnam('root'))
@patch('som.core.common.som_uid', return_value=getpwnam('root'))
@patch('som.agent.chown_r')
def test_create_group(chown_mock, uid_mock, gid_mock, group_id):
    ...
    try:
        ...
        assert isinstance(result, SomResult), 'The returned object is not an "SomResult" instance.'
...
@pytest.mark.parametrize('group_id, exception, exception_code', [
    ('default', SomError, 1711),
    ('group-1', SomError, 1711),
    ('invalid!', SomError, 1722),
    ('delete-me', SomInternalError, 1005),
    ('ar.conf', SomError, 1713),
    ('agent-template.conf', SomError, 1713)
])
@patch('som.core.common.SHARED_PATH', new=test_shared_path)
def test_create_group_exceptions(group_id, exception, exception_code):
    ...
    exception_code : int
        Expected error code for the Som Exception object raised by `get_group_files` with the given parameters.
    """
    try:
...
@patch('som.agent.get_groups')
@patch('som.agent.Agent.delete_single_group')
...
@pytest.mark.parametrize('group_list, expected_errors', [
    (['none-1'], [SomResourceNotFound(1710)]),
    (['default'], [SomError(1712)]),
    (['none-1', 'none-2'], [SomResourceNotFound(1710)]),
    (['default', 'none-1'], [SomError(1712), SomResourceNotFound(1710)]),
])
@patch('som.agent.get_groups')
def test_agent_delete_groups_other_exceptions(mock_get_groups, group_list, expected_errors):
    ...
    group_list : List of str
        List of groups to be deleted.
    expected_errors : list of SomError
        List of expected SomError to be raised by delete_groups if a group is not valid. An exception will be returned
...
@patch('som.agent.Agent.add_group_to_agent')
@patch('som.core.wdb.SomDBConnection._send', side_effect=send_msg_to_wdb)
@patch('som.core.agent.Agent.group_exists', return_value=True)
@patch('socket.socket.connect')
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['none-1'], ['001'], SomResourceNotFound(1710), True),
    (['group-1'], ['100'], SomResourceNotFound(1701), False),
    (['default'], ['000'], SomError(1703), False)
])
@patch('som.agent.Agent.group_exists')
@patch('som.agent.Agent.add_group_to_agent')
@patch('som.core.wdb.SomDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
async def test_agent_assign_agents_to_group_exceptions(socket_mock, send_mock, mock_add_group, mock_group_exists, group_list,
    ...
    catch_exception : bool
        ...
        `AffectedItemsSomResult` containing the exceptions in its 'failed_items'.
    """
...
@patch('som.core.common.DATABASE_PATH_GLOBAL', new=test_global_bd_path)
@patch('som.core.agent.Agent.unset_single_group_agent')
@patch('som.agent.get_groups')
@patch('som.agent.get_agents_info')
...
@pytest.mark.parametrize('group_id, agent_id, expected_error', [
    ('any-group', '100', SomResourceNotFound(1701)),
    ('any-group', '000', SomError(1703)),
    ('group-1', '005', SomResourceNotFound(1710)),
])
@patch('som.agent.get_agents_info', return_value=short_agent_list)
@patch('som.agent.get_groups', side_effect={'default'})
async def test_agent_remove_agent_from_group_exceptions(group_mock, agents_info_mock, group_id, agent_id, expected_error):
    ...
    expected_error : SomError
        The SomError object expected to be raised by remove_agent_from_group with the given parameters.
    """
    try:
    ...
    except (SomError, SomResourceNotFound) as error:
...
@patch('som.core.agent.Agent.unset_single_group_agent')
@patch('som.agent.get_agents_info', return_value=short_agent_list)
@patch('som.agent.get_groups', return_value={'group-1'})
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['any-group'], ['100'], SomResourceNotFound(1701), True),
    (['any-group'], ['000'], SomError(1703), True),
    (['any-group'], ['005'], SomResourceNotFound(1710), False),
])
@patch('som.core.agent.Agent.unset_single_group_agent')
@patch('som.agent.get_agents_info', return_value=short_agent_list)
@patch('som.agent.get_groups', return_value={'group-1'})
async def test_agent_remove_agent_from_groups_exceptions(mock_get_groups, mock_get_agents, mock_unset, group_list, agent_list,
    ...
    try:
        ...
        assert not catch_exception, \
            'An "SomError" exception was expected but was not raised.'
        # Check Typing
        assert isinstance(result, AffectedItemsSomResult), 'The returned object is not an "AffectedItemsSomResult".'
        assert isinstance(result.failed_items, dict), \
        ...
        assert catch_exception, \
            'No exception should be raised at this point. An AffectedItemsSomResult object with at least one ' \
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['non-group'], ['000'], SomResourceNotFound(1710), True),
    (['group-1'], ['000'], SomError(1703), False),
    (['group-1'], ['100'], SomResourceNotFound(1701), False),
])
@patch('som.agent.get_agents_info', return_value=short_agent_list)
@patch('som.agent.get_groups', return_value={'group-1'})
...
@pytest.mark.parametrize('agent_set, expected_errors_and_items, result_from_socket, filters, raise_error', [
    ...
    (
            ...
            {'error': 1,
             'data': [{'error': 18,
                       'message': 'Error from socket indicating SomInternalError',
                       'agent': 1}
                      ],
             'message': 'Error'},
            None,
...
@patch('som.agent.get_agents_info', return_value=set(full_agent_list))
@patch('som.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('som.core.wdb.SomDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
        ...
        if raise_error:
            ...
            for error in expected_errors_and_items.keys():
                if int(error) in ERROR_CODES_UPGRADE_SOCKET_BAD_REQUEST:
                    ...
                    with pytest.raises(SomInternalError, match=f".* {error} .*"):
                        upgrade_agents(agent_list=list(agent_set), filters=filters)
        else:
...
def test_agent_get_upgrade_result(mock_socket, mock_wdb, mock_client_keys, agent_set, expected_errors_and_items,
    ...
    with patch('som.core.agent.core_upgrade_agents') as core_upgrade_agents_mock:
        ...
        if raise_error:
            # Get upgrade result expecting a Som Exception
            for error in expected_errors_and_items.keys():
                with pytest.raises(SomInternalError, match=f".* {error} .*"):
...
@patch('som.core.som_socket.SomSocket')
@patch('som.core.wdb.SomDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
@patch('os.path.exists')
def test_agent_get_agent_config(mock_exists, socket_mock, send_mock, som_socket_mock, agent_list, component, configuration):
    ...
    """
    som_socket_mock.return_value.receive.return_value = b'ok {"test": "conf"}'
...
@patch('som.core.common.SHARED_PATH', new=test_shared_path)
@patch('som.core.common.MULTI_GROUPS_PATH', new=test_multigroup_path)
@patch('som.agent.get_agents_info', return_value=full_agent_list)
@patch('som.core.wdb.SomDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
def test_agent_get_agents_sync_group_exceptions(socket_mock, send_mock, agent_list, expected_error):
    ...
    agent_list : List of str
        List of agent ID's.
    expected_error : SomError
        Expected SomError to be returned (not raised) by the function.
    """
    result = get_agents_sync_group(agent_list=agent_list)
    assert isinstance(result, AffectedItemsSomResult), 'The returned object is not an "AffectedItemsSomResult".'
    # Check failed items
...
@patch('som.core.common.SHARED_PATH', new=test_shared_path)
@patch('som.core.configuration.upload_group_configuration')
...
@patch('som.core.common.SHARED_PATH', new=test_shared_path)
@patch('som.agent.get_distinct_agents')
@patch('som.agent.get_agent_groups')
@patch('som.agent.get_agents_summary_status')
@patch('som.agent.get_agents')
@patch('som.core.wdb.SomDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@patch('som.agent.get_agents_info', return_value=['test', 'test2'])
@patch('som.core.wdb.SomDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_get_agents_big_env(mock_conn, mock_send, mock_get_agents, insert_agents_db, agent_list, params, expected_ids):
    ...
    with patch('som.agent.get_agents_info', return_value=set(agent_ids_format(range(100000)))):
...
@patch('som.core.agent.Agent.get_agent_groups', new_callable=AsyncMock)
@patch('som.core.agent.Agent.set_agent_group_relationship')
...
@patch('som.core.agent.Agent.get_agent_groups', new_callable=AsyncMock)
@patch('som.core.agent.Agent.group_exists', return_value=False)
...
async def test_unset_single_group_agent_ko(agent_basic_mock, group_exists_mock, get_groups_mock, agent_id, group_id,
    ...
    expected_exc: int
        Expected SomException code error.
    """
    get_groups_mock.return_value = ['default']
    with pytest.raises(SomException, match=f".* {expected_exc} .*"):
...
def test_check_uninstall_permission():
    ...
    expected = SomResult({'message': 'User has permission to uninstall agents'})

...
import pytest
from som.core.common import USER_DECODERS_PATH


with patch('som.core.common.getgrnam'):
    with patch('som.core.common.getpwnam'):
...
def mock_som_paths():
    """Mock som paths."""
    with patch('som.core.common.SOM_PATH', new=test_data_path):
        with patch('som.core.configuration.get_ossec_conf', return_value=decoder_ossec_conf):
            with patch('som.core.common.DECODERS_PATH',
                       new=os.path.join(test_data_path, "tests", "data", "decoders")):
                with patch('som.core.common.USER_DECODERS_PATH',
                           new=os.path.join(test_data_path, "tests", "data", "etc", "decoders")):
                    with patch('som.decoder.to_relative_path',
                               side_effect=lambda x: os.path.relpath(x, som.core.common.SOM_PATH)):
                        yield
...
@pytest.mark.parametrize('names, status, filename, relative_dirname, parents, expected_names, expected_total_failed', [
    (None, None, None, None, False, {'agent-buffer', 'json', 'agent-upgrade', 'som', 'agent-restart'}, 0),
    (['agent-buffer'], None, None, None, False, {'agent-buffer'}, 0),
    (['agent-buffer', 'non_existing'], None, None, None, False, {'agent-buffer'}, 1),
    (None, 'enabled', None, None, False, {'agent-buffer', 'agent-upgrade', 'som', 'agent-restart'}, 0),
    (None, 'disabled', None, None, False, {'json'}, 0),
    (['agent-upgrade', 'non_existing', 'json'], 'enabled', None, None, False, {'agent-upgrade'}, 1),
    (None, None, 'test1_decoders.xml', None, False, {'agent-buffer', 'agent-upgrade', 'som', 'agent-restart'}, 0),
    (None, None, 'test2_decoders.xml', 'tests/data/decoders', False, {'json'}, 0),
    (None, 'all', None, 'tests/data/decoders', True, {'som', 'json'}, 0),
...
@pytest.mark.parametrize('conf, exception', [
    ...
    ({'ruleset': None}, SomInternalError(1500))
])
def test_get_decoders_files(conf, exception):
    """Test get_decoders_files function"""
    with patch('som.core.configuration.get_ossec_conf', return_value=conf):
        try:
            # UUT call
        ...
        except SomInternalError as exc:
...
def test_get_decoder_file_path(filename, relative_dirname, result, mock_som_paths):
    """Test get_decoder_file_path function."""
    res = decoder.get_decoder_file_path(filename=filename,
                                         relative_dirname=relative_dirname)
    assert res == os.path.join(som.core.common.SOM_PATH, result) if result else not res and isinstance(res, str)
...
def test_get_decoder_file(filename, raw, relative_dirname, contains, mock_som_paths):
    ...
    if raw:
        ...
        # Assert the result is an AffectedItemsSomResult
        assert isinstance(result, AffectedItemsSomResult)
...
@patch('som.decoder.delete_decoder_file')
@patch('som.decoder.full_copy')
@patch('som.decoder.validate_som_xml')
@patch('som.decoder.upload_file')
@patch('som.decoder.remove')
@patch('som.decoder.safe_move')
@patch('som.decoder.validate_dummy_logtest')
def test_upload_file(mock_logtest, mock_safe_move, mock_remove, mock_upload_file,
                     mock_xml, mock_full_copy, mock_delete, mock_som_paths,
    ...
    with patch('som.decoder.validate_upload_delete_dir', return_value=ret_validation):
        with patch('som.decoder.exists', return_value=overwrite):
            with patch('som.decoder.to_relative_path',
                       side_effect=lambda x: os.path.relpath(x, som.core.common.SOM_PATH)):
            ...
            assert isinstance(result, AffectedItemsSomResult), 'No expected result type'

            mock_xml.assert_called_once_with(content)
            mock_upload_file.assert_called_once_with(content, decoder_path)
            if overwrite:
                full_path = os.path.join(som.common.SOM_PATH, decoder_path)
...
@patch('som.decoder.delete_decoder_file', side_effect=SomError(1019))
@patch('som.decoder.upload_file')
@patch('som.decoder.safe_move')
@patch('som.core.utils.check_remote_commands')
def test_upload_file_ko(*_):
    ...
    # clean backup files
    search_pattern = os.path.join(som.core.common.SOM_PATH, "**", "*.backup")
...
def test_delete_decoder_file(filename, relative_dirname):
    ...
    with patch('som.decoder.exists', return_value=True):
        # Assert returned type is AffectedItemsSomResult when everything is correct
        with patch('som.decoder.remove'):
...
def test_delete_decoder_file_ko():
    ...
    with patch('som.decoder.exists', return_value=True):
        with patch('som.decoder.remove', side_effect=IOError()):

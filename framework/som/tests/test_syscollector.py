...
with patch('som.core.common.som_uid'):
    with patch('som.core.common.som_gid'):
        ...
        from som import syscollector
        from som.core.results import AffectedItemsSomResult
        from som.core.syscollector import Type, get_valid_fields
...
@patch('som.syscollector.get_agents_info', return_value=['000', '001'])
...
def test_get_item_agent(mock_agent_attr, mock_basic_info, mock_agents_info, mock_exists, select, search):
    ...
    with patch('som.core.utils.SomDBConnection') as mock_wdb:
        ...
        assert isinstance(results, AffectedItemsSomResult)

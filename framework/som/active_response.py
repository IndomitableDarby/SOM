...
from som.core import active_response, common
from som.core.agent import get_agents_info, get_rbac_filters, SomDBQueryAgents
from som.core.exception import SomException, SomError, SomResourceNotFound
...
def run_command(agent_list: list = None, command: str = '', arguments: list = None,
                alert: dict = None) -> AffectedItemsSomResult:
    ...
    AffectedItemsSomResult
        Affected items.
    """
    result = AffectedItemsSomResult(all_msg='AR command was sent to all agents',
    ...
    if agent_list:
        ...
        for agent_id in set(agent_list) - system_agents:
            result.add_failed_item(id_=agent_id, error=SomResourceNotFound(1701))

        rbac_filters = get_rbac_filters(system_resources=system_agents, permitted_resources=agent_list)

        with SomDBQueryAgents(select=['id', 'status', 'version'], query="id!=000", **rbac_filters) as db_query:

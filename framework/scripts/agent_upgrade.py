...
# Set framework path
path.append(dirname(argv[0]) + '/../framework')  # It is necessary to import Som package

# Import framework
try:
    import som.agent
    from som.agent import upgrade_agents, get_upgrade_result, get_agents
    from som.core import common
    from som.core.exception import SomError
    from som.core.cluster import utils as cluster_utils
except Exception as e:
    print("Error importing 'som' package.\n\n{0}\n".format(e))
    exit()
...
def get_script_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("-v", "--version", type=str, help="Version to upgrade. [Default: latest Som version]")
...
def list_outdated():
    ...
    agents = som.agent.get_outdated_agents()
...
async def main():
    ...
    except SomError as som_err:
        print(f"Error {som_err.code}: {som_err.message}")
        if args.debug:
            raise

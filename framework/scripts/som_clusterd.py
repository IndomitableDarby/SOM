...
from som.core.utils import clean_pid_files
...
def set_logging(foreground_mode=False, debug_mode=0) -> SomLogger:
    """Set cluster logger.
...
def print_version():
    """Print Som metadata."""
    from som.core.cluster import __author__, __licence__, __version__, __som_name__
    print(f"\n{__som_name__} {__version__} - {__author__}\n\n{__licence__}")
...
def exit_handler(signum, frame):
    ...
    pyDaemonModule.delete_child_pids('som-clusterd', cluster_pid, main_logger)

    # Remove cluster's pidfile
    pyDaemonModule.delete_pid('som-clusterd', cluster_pid)
...
# Master main
#
async def master_main(args: argparse.Namespace, cluster_config: dict, cluster_items: dict, logger: SomLogger):
    ...
    logger : SomLogger
        Cluster logger.
    """
    from som.core.cluster import local_server, master
    from som.core.cluster.hap_helper.hap_helper import HAPHelper
...
# Worker main
#
async def worker_main(args: argparse.Namespace, cluster_config: dict, cluster_items: dict, logger: SomLogger):
    ...
    from som.core.cluster import local_server, worker
...
def main():
    """Main function of the som-clusterd script in charge of starting the cluster process."""
    import som.core.cluster.cluster

    # Set correct permissions on cluster.log file
    if os.path.exists(f'{common.SOM_PATH}/logs/cluster.log'):
        os.chown(f'{common.SOM_PATH}/logs/cluster.log', common.som_uid(), common.som_gid())
        os.chmod(f'{common.SOM_PATH}/logs/cluster.log', 0o660)
    ...
    if cluster_configuration['disabled']:
        sys.exit(0)
    try:
        som.core.cluster.cluster.check_cluster_config(cluster_configuration)
    except Exception as e:
        main_logger.error(e)
        sys.exit(1)
    ...
    som.core.cluster.cluster.clean_up()

    # Check for unused PID files
    clean_pid_files('som-clusterd')

    # Foreground/Daemon
    if not args.foreground:
        pyDaemonModule.pyDaemon()

    # Drop privileges to som
    if not args.root:
        os.setgid(common.som_gid())
        os.setuid(common.som_uid())

    pid = os.getpid()
    pyDaemonModule.create_pid('som-clusterd', pid)
    if args.foreground:
    ...
    try:
        ...
        pyDaemonModule.delete_child_pids('som-clusterd', pid, main_logger)
        pyDaemonModule.delete_pid('som-clusterd', pid)


if __name__ == '__main__':
    import som.core.cluster.utils as cluster_utils
    from som.core import common, configuration, pyDaemonModule
    ...
    # Set logger
    try:
        debug_mode_ = configuration.get_internal_options_value('som_clusterd', 'debug', 2, 0) or args.debug_level
    except Exception:
        debug_mode_ = 0

...
import scripts.som_logtest as som_logtest
...
@patch('scripts.som_logtest.init_logger')
@patch('scripts.som_logtest.SomLogtest')
@patch('scripts.som_logtest.init_argparse')
@patch('builtins.input', return_value='mock')
def test_main(input_mock, argparse_mock, som_logtest_class_mock, init_logger_mock, register_mock, logger_warning_mock,
    ...
    som_logtest_class_mock.return_value = SomLogtestMock()
    ...
    register_mock.assert_called_once_with(som_logtest_class_mock.return_value.remove_last_session)
    sys_exit_mock.assert_has_calls([call(0), call(1)])

    logger_info_mock.assert_has_calls([call('%s', 'Som ERROR - Som Inc.'),
                                       call('%s', '\nThis program is free software; you can redistribute it and/or '
                                                  'modify\nit under the terms of the GNU General Public License '
                                                  '(version 2) as\npublished by the Free Software Foundation. For more '
                                                  'details, go to\nhttps://www.gnu.org/licenses/gpl.html\n'),
                                       call('Starting som-logtest %s', 'ERROR'),
                                       call('Type one log per line')])
    logger_error_mock.assert_called_once_with('Unit test configuration wrong syntax: %s',
                                              som_logtest_class_mock.return_value.get_last_ut())
    logger_warning_mock.assert_has_calls([call('** Som-Logtest: %s', 'WARNING'), call('')])

    assert som_logtest_class_mock.return_value.show_last_ut_result_called is True
    assert som_logtest_class_mock.return_value.remove_last_session_called is False
    assert som_logtest_class_mock.return_value.get_last_ut_called is True
    assert som_logtest_class_mock.return_value.process_log_called is True
    ...
    som_logtest_class_mock.return_value = SomLogtestMock(True)
    ...
    som_logtest_class_mock.return_value = SomLogtestMock(process_log_exception='ValueError')
    ...
    logger_error_mock.assert_called_once_with('** Som-logtest error ')

    # Test the third exception
    logger_error_mock.reset_mock()
    som_logtest_class_mock.return_value = SomLogtestMock(process_log_exception='ConnectionError')
    logger_error_mock.side_effect = Exception()
    ...
    logger_error_mock.assert_called_once_with('** Som-logtest error when connecting with som-analysisd')


# Test SomDaemonProtocol class methods

def create_som_daemon_protocol_class():
    """Create new SomDaemonProtocol class."""
    return som_logtest.SomDeamonProtocol()


def test_wdp_init():
    """Test the init method, checking the initial status of its attributes."""
    wdp = create_som_daemon_protocol_class()

    assert isinstance(wdp.protocol, dict)
    assert wdp.protocol['version'] == 1
    assert isinstance(wdp.protocol['origin'], dict)
    assert wdp.protocol['origin']['name'] == 'som-logtest'
    assert wdp.protocol['origin']['module'] == 'som-logtest'


@patch('json.dumps', return_value='')
def test_wdp_wrap(json_dumps_mock):
    """Test if the data is being properly wrapped with som daemon protocol information."""
    wdp = create_som_daemon_protocol_class()
    assert wdp.wrap(command='command', parameters={'parameters': 'parameters'}) == json_dumps_mock.return_value
    json_dumps_mock.assert_called_once_with({'version': 1, 'origin': {'name': 'som-logtest', 'module':
        'som-logtest'}, 'command': 'command', 'parameters':
                                                 {'parameters': 'parameters'}})


@patch('json.loads', return_value={'error': 'error', 'message': 'message'})
def test_wdp_unwrap(json_loads_mock):
    """Test if the data is being properly unwrapped from the som daemon protocol."""
    wdp = create_som_daemon_protocol_class()
    msg = {}
...
# Test SomSocket methods

def create_som_socket_class(file):
    """Create a new SomSocket class."""
    return som_logtest.SomSocket(file=file)


def test_ws_init():
    """Test the correct start of the SomSocket class."""
    file = ''
    assert create_som_socket_class(file).file == file
...
def test_ws_sent(unpack_mock, pack_mock, msg_waitall_mock, stream_mock, unix_mock, socket_socket_mock):
    ...
    ws = create_som_socket_class(file=file)
...
# Test SomLogtest class methods

@patch('scripts.som_logtest.SomSocket', return_value=SomSocketMock())
@patch('scripts.som_logtest.LOGTEST_SOCKET')
@patch('scripts.som_logtest.SomDeamonProtocol', return_value=SomDeamonProtocolMock())
def create_som_logtest_class(som_deamon_mock, logtest_socket_mock, som_socket_mock):
    """Create new SomLogtest class."""
    return som_logtest.SomLogtest()


def test_wl_init():
    """Test the correct initialization of SomLogtest class."""
    wl = create_som_logtest_class()

    assert isinstance(wl.protocol, SomDeamonProtocolMock)
    assert isinstance(wl.socket, SomSocketMock)
...
@patch('scripts.som_logtest.SomLogtest.remove_session')
...
@patch('logging.debug')
@patch('json.dumps', return_value='')
@patch('scripts.som_logtest.SomLogtest.show_ossec_logtest_like')
...
@patch('logging.info')
@patch('scripts.som_logtest.SomLogtest.show_phase_info')
def test_wl_show_ossec_logtest_like(show_phase_info_mock, info_mock):
    """Test if som-logtest output is being shown as ossec-logtest output."""
    ...
    # Test the third 'if'
    som_logtest.SomLogtest.show_ossec_logtest_like(output)
...
def test_wl_show_phase_info(info_mock):
    """Check if som-logtest is processing phase information."""
    phase_data = {'key': 'value', 'key2': 'value2', 'key3': {'1': '2'}}
    show_first = ['key']

    som_logtest.SomLogtest.show_phase_info(phase_data=phase_data, show_first=show_first)
    info_mock.assert_has_calls([call("\t%s: '%s'", 'key', 'value'), call("\t%s: '%s'", 'key2', 'value2')])


@patch('logging.info')
@patch('scripts.som_logtest.SomLogtest.get_last_ut', return_value='mock')
...
def create_som_class():
    """Auxiliary function to create a Som class."""
    return som_logtest.Som


@patch('scripts.som_logtest.common.find_som_path', return_value='')
def test_som_get_install_path(find_som_path_mock):
    """Test is we can get the installation path correctly."""
    assert create_som_class().get_install_path() == find_som_path_mock.return_value


@patch('subprocess.PIPE')
@patch('subprocess.Popen')
@patch('os.path.join', return_value='')
@patch('scripts.som_logtest.Som.get_install_path', return_value='')
def test_som_get_info(get_install_mock, join_mock, popen_mock, pipe_mock):
    """Check if we can properly obtain information from som-control."""
    ...
    som = create_som_class()
    popen_mock.return_value = ProcMock()
    field = 'key'

    # Test the 'try'
    assert som.get_info(field=field) == field
    get_install_mock.assert_called_once_with()
    join_mock.assert_called_once_with(get_install_mock.return_value, 'bin', 'som-control')
    ...
    assert som.get_info(field=field) == 'ERROR'


@patch('scripts.som_logtest.Som.get_info', return_value='')
def test_som_get_version_str(get_info_mock):
    """Test if the version is being properly retrieved."""
    assert create_som_class().get_version_str() == get_info_mock.return_value
    get_info_mock.assert_called_once_with('SOM_VERSION')


@patch('scripts.som_logtest.Som.get_version_str', return_value='')
def test_som_get_description(get_version_mock):
    """Test if the description is being properly retrieved."""
    assert create_som_class().get_description() == f"Som {get_version_mock.return_value} - Som Inc."
    get_version_mock.assert_called_once_with()


@patch('textwrap.dedent', return_value='')
def test_som_get_license(dedent_mock):
    """Test that the license was not changed."""
    assert create_som_class().get_license() == dedent_mock.return_value
...
def test_init_logger(logging_mock):
    ...
    som_logtest.init_logger(ArgsMock())

...
@patch('scripts.cluster_control.local_client.LocalClient', return_value='LocalClient return value')
@patch('scripts.cluster_control.control.get_nodes', return_value={'items': [{'name': 'som_worker'}]})
...
async def test_print_health(get_health_mock, get_nodes_mock, local_client_mock, get_utc_strptime_mock, print_mock):
    ...
    more = True
    worker_status = get_health_mock.return_value['nodes']['som_worker2']['status']
    worker_info = get_health_mock.return_value['nodes']['som_worker2']['info']
    get_utc_strptime_mock.side_effect = seconds_mock

    # Test cases 1 and 2
    for filter_node in ['som_worker', None]:
        # Reset mocks
        print_mock.reset_mock()
    ...
    print_mock.assert_called_once_with(f"Cluster name: {config['name']}\n\nLast completed synchronization for connected"
                                       ...
                                       f"som_worker2 "
                                       f"({get_health_mock.return_value['nodes']['som_worker2']['info']['ip']}): "
...
@patch('som.core.cluster.cluster.check_cluster_config')
@patch('som.core.cluster.utils.read_config', return_value='')
@patch('som.core.cluster.utils.get_cluster_status', return_value={'enabled': 'no', 'running': 'yes'})

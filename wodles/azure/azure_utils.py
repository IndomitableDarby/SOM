...
CREDENTIALS_URL = 'https://documentation.som.com/current/azure/activity-services/prerequisites/credentials.html'
DEPRECATED_MESSAGE = (
...
def send_message(message: str):
    ...
    try:
        ...
        if e.errno == 111:
            logging.error('Som must be running.')
            sys.exit(1)
        elif e.errno == 90:
            logging.error(
                'Message too long to send to Som.  Skipping message...'
            )
        else:
            logging.error(f'Error sending message to som: {e}')
            sys.exit(1)
    finally:
        s.close()

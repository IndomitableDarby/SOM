...
def test_DockerListener__init__():
    ...
    for attribute in ['som_path', 'som_queue', 'msg_header', 'client', 'thread1', 'thread2']:
...
def test_DockerListener_process(mock_send):
    """Test process function sends the decoded events to the Som socket."""
    event = b"test"
    dl = docker_listener.DockerListener()
    dl.process(event)
...
def test_DockerListener_send_msg(mock_socket):
    """Test send_msg sends the messages with the expected contents to the Som socket."""
    msg = '{"test": "value"}'
    ...
    m.connect.assert_called_with(dl.som_queue)

...
@patch('som_integration.SomIntegration.get_client')
@patch('som_integration.utils.find_som_path', return_value=utils.TEST_SOM_PATH)
@patch('som_integration.utils.get_som_version')
def test_som_integration_initializes_properly(mock_version, mock_path, mock_client):
    """Test if the instances of SomIntegration are created properly."""

    args = utils.get_som_integration_parameters()
    integration = som_integration.SomIntegration(**args)
    mock_path.assert_called_once()
    mock_version.assert_called_once()
    assert integration.som_path == utils.TEST_SOM_PATH
    assert integration.som_queue == os.path.join(integration.som_path, utils.QUEUE_PATH)
    assert integration.som_wodle == os.path.join(integration.som_path, utils.WODLE_PATH)
...
def test_default_config(mock_boto_config, file_exists, options, retry_attempts, retry_mode):
    """Test if `default_config` function returns the Som default Retry configuration if there is no user-defined
    configuration.
    ...
    with patch('som_integration.path.exists', return_value=file_exists):
        if file_exists:
            with patch('aws_tools.get_aws_config_params') as mock_get_config, \
                ...
                config = som_integration.SomIntegration.default_config(profile=utils.TEST_AWS_PROFILE)
            ...
            if aws_tools.RETRY_ATTEMPTS_KEY in profile_config or aws_tools.RETRY_MODE_CONFIG_KEY in profile_config:
                ...
                retries = som_integration.SOM_DEFAULT_RETRY_CONFIGURATION

            assert config['config'].retries == retries
        else:
            config = som_integration.SomIntegration.default_config(profile=utils.TEST_AWS_PROFILE)

            mock_boto_config.Config.assert_called_with(retries=som_integration.SOM_DEFAULT_RETRY_CONFIGURATION)
...
@pytest.mark.parametrize('region', list(som_integration.DEFAULT_GOV_REGIONS) + ['us-east-1', None])
@pytest.mark.parametrize('service_name', list(som_integration.SERVICES_REQUIRING_REGION) + ['other'])
def test_som_integration_get_client_authentication(access_key, secret_key, profile, region, service_name):
    ...
    service_name : str
        Name of the service.
    """
    kwargs = utils.get_som_integration_parameters(access_key=access_key, secret_key=secret_key, profile=profile,
    ...
    if region and service_name in som_integration.SERVICES_REQUIRING_REGION:
        expected_conn_args['region_name'] = region
    else:
        expected_conn_args['region_name'] = region if region in som_integration.DEFAULT_GOV_REGIONS else None

    with patch('som_integration.utils.find_som_path', return_value=utils.TEST_SOM_PATH), \
            patch('som_integration.utils.get_som_version', return_value=utils.SOM_VERSION), \
            patch('som_integration.boto3.Session') as mock_boto:
        som_integration.SomIntegration(**kwargs)
...
def test_som_integration_get_client(iam_role_arn, service_name, external_id):
    ...
    """
    kwargs = utils.get_som_integration_parameters(access_key=None, secret_key=None, profile=None,
    ...
    sts_kwargs = {'aws_access_key_id': None, 'aws_secret_access_key': None, 'aws_session_token': utils.TEST_TOKEN,
                  'region_name': None}
    assume_role_kwargs = {'RoleArn': iam_role_arn, 'RoleSessionName': 'SomLogParsing',
    ...
    with patch('som_integration.utils.find_som_path', return_value=utils.TEST_SOM_PATH), \
            patch('som_integration.utils.get_som_version', return_value=utils.SOM_VERSION), \
            patch('som_integration.boto3.Session', side_effect=[mock_boto_session, mock_sts_session]) as mock_session:
        instance = som_integration.SomIntegration(**kwargs)
...
def test_som_integration_get_client_handles_exceptions_on_botocore_error():
    """Test `get_client` function handles exceptions as expected."""
    mock_boto_session = MagicMock()
    mock_boto_session.client.side_effect = som_integration.exceptions.ClientError({'Error': {'Code': 1}},
                                                                                             'operation')

    with patch('som_integration.utils.find_som_path', return_value=utils.TEST_SOM_PATH), \
            patch('som_integration.utils.get_som_version', return_value=utils.SOM_VERSION), \
            patch('som_integration.boto3.Session', return_value=mock_boto_session):
        with pytest.raises(SystemExit) as e:
            som_integration.SomIntegration(**utils.get_som_integration_parameters())
...
def test_som_integration_get_sts_client(access_key, secret_key, profile):
    ...
    profile : str
        AWS profile name.
    """
    instance = utils.get_mocked_som_integration(access_key=access_key, secret_key=secret_key, profile=profile)
    expected_conn_args = {}
    if access_key and secret_key:
        expected_conn_args['aws_access_key_id'] = access_key
        expected_conn_args['aws_secret_access_key'] = secret_key
    ...
    with patch('som_integration.boto3.Session', return_value=mock_session) as mock_boto:
...
def test_som_integration_get_sts_client_handles_exceptions_when_invalid_creds_provided():
    """Test `get_sts_client` function handles invalid credentials exception as expected."""
    mock_boto_session = MagicMock()
    mock_boto_session.client.side_effect = som_integration.exceptions.ClientError({'Error': {'Code': 1}},
                                                                                             'operation')

    instance = utils.get_mocked_som_integration(access_key=None, secret_key=None, profile=None)

    with patch('som_integration.boto3.Session', return_value=mock_boto_session):
        with pytest.raises(SystemExit) as e:
...
def test_som_integration_send_msg(dump_json):
    ...
    """
    instance = utils.get_mocked_som_integration()
    msg = dumps(utils.TEST_MESSAGE) if dump_json else utils.TEST_MESSAGE
    with patch('som_integration.socket.socket') as mock_socket:
        m = MagicMock()
        mock_socket.return_value = m
        instance.send_msg(utils.TEST_MESSAGE, dump_json=dump_json)
        mock_socket.assert_called_once()
        m.send.assert_called_with(f"{som_integration.MESSAGE_HEADER}{msg}".encode())
...
def test_som_integration_send_msg_socket_error(error_code, expected_exit_code):
...
@patch('io.BytesIO')
def test_som_integration_decompress_file(mock_io):
    """Test 'decompress_file' method calls the expected function for a determined file type."""
    integration = utils.get_mocked_som_integration()
    integration.client = MagicMock()
    # Instance that inherits from SomIntegration sets the attribute bucket in its constructor
...
@patch('io.BytesIO')
def test_aws_som_integration_decompress_file_handles_exceptions_when_decompress_fails(mock_io):
...
def test_som_integration_send_msg_handles_exceptions():
...
@patch('som_integration.utils.get_som_version')
@patch('som_integration.SomIntegration.get_client')
@patch('som_integration.SomAWSDatabase.check_metadata_version')
@patch('som_integration.sqlite3.connect')
def test_som_aws_database_initializes_properly(mock_connect, mock_metadata, mock_client, mock_version, mock_path):
    """Test if the instances of SomAWSDatabase are created properly."""
    mock_connect.return_value = MagicMock()
    args = utils.get_som_aws_database_parameters()
    som_aws_db = som_integration.SomAWSDatabase(**args)

    assert som_aws_db.db_path == os.path.join(som_aws_db.som_wodle, f"{utils.TEST_DATABASE}.db")
    mock_connect.assert_called_once()
    som_aws_db.db_connector.cursor.assert_called_once()
    mock_metadata.assert_called_once()


def test_som_aws_database_create_table():
    """Test `create_table` function creates the table using the expected SQL."""
    instance = utils.get_mocked_som_aws_database()
...
def test_som_aws_database_create_table_handles_exceptions_when_table_not_created():
...
@patch('som_integration.SomAWSDatabase.create_table')
def test_som_aws_database_db_initialization(mock_create_table, table_list):
...
def test_som_aws_database_db_initialization_handles_exceptions():
...
def test_som_aws_database_close_db():
...
def test_som_aws_database_check_metadata_version_existing_table(custom_database):
    ...
    instance = utils.get_mocked_som_aws_database(db_name=utils.TEST_DATABASE)
    ...
    assert old_metadata_value != utils.SOM_VERSION
    ...
    assert new_metadata_value == utils.SOM_VERSION


def test_som_aws_database_check_metadata_version_no_table(custom_database):
...
def test_som_aws_database_check_metadata_version_handles_exceptions(custom_database, table_exists):
...
def test_som_aws_database_delete_deprecated_tables(custom_database):
    ...
    for table in som_integration.DEPRECATED_TABLES:

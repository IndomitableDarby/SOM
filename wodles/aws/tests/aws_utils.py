...
def get_som_integration_parameters(service_name: str = TEST_SERVICE_NAME, profile: str = TEST_AWS_PROFILE,
    ...
    """Return a dict containing every parameter supported by SomIntegration. Used to simulate different ossec.conf
    configurations.
...
def get_som_aws_database_parameters(service_name: str = TEST_SERVICE_NAME, profile: str = TEST_AWS_PROFILE,
                                      db_name: str = TEST_DATABASE, access_key: str = None, secret_key: str = None,
                                      iam_role_arn: str = None, region: str = None, discard_field: str = None,
                                      discard_regex: str = None, sts_endpoint: str = None, service_endpoint: str = None,
                                      iam_role_duration: str = None, external_id: str = None):
    """Return a dict containing every parameter supported by SomAWSDatabase.
...
def get_mocked_som_integration(**kwargs):
    with patch('som_integration.SomIntegration.get_client'), \
            patch('som_integration.sqlite3.connect'), \
            patch('som_integration.utils.find_som_path', return_value=TEST_SOM_PATH), \
            patch('som_integration.utils.get_som_version', return_value=SOM_VERSION):
        return som_integration.SomIntegration(**get_som_integration_parameters(**kwargs))


def get_mocked_som_aws_database(**kwargs):
    with patch('som_integration.SomAWSDatabase.check_metadata_version'), \
            patch('som_integration.SomIntegration.get_client'), \
            patch('som_integration.sqlite3.connect'), \
            patch('som_integration.utils.find_som_path', return_value=TEST_SOM_PATH), \
            patch('som_integration.utils.get_som_version', return_value=SOM_VERSION):
        return som_integration.SomAWSDatabase(**get_som_aws_database_parameters(**kwargs))

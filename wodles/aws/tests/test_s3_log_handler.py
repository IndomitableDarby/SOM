...
def test_aws_sl_subscriber_bucket_initializes_properly(mock_som_integration, mock_client, mock_sts_client):
    ...
    mock_som_integration.assert_called_with(integration, access_key=None, secret_key=None,
                                              profile=None,
                                              service_name='s3',
...
def test_aws_sl_subscriber_bucket_obtain_logs(mock_som_integration, mock_sts_client):
...
@patch('som_integration.SomIntegration.get_sts_client')
@patch('som_integration.SomIntegration.__init__', side_effect=som_integration.SomIntegration.__init__)
def test_aws_sl_subscriber_bucket_obtain_logs_with_datetime(mock_som_integration, mock_sts_client):
...
@patch('som_integration.SomIntegration.get_sts_client')
@patch('som_integration.SomIntegration.__init__', side_effect=som_integration.SomIntegration.__init__)
def test_aws_sl_subscriber_bucket_obtain_logs_handles_exception(mock_som_integration,
                                                                                    mock_sts_client):
...
@patch('som_integration.SomIntegration.send_msg')
@patch('som_integration.SomIntegration.get_sts_client')
@patch('som_integration.SomIntegration.__init__', side_effect=som_integration.SomIntegration.__init__)
def test_aws_sl_subscriber_bucket_process_file(mock_som_integration, mock_sts_client, mock_send, mock_obtain):
...
def test_obtain_logs_processes_different_data_types(bucket_name, log_path, content, expected_logs):
    ...
    with patch('s3_log_handler.som_integration.SomIntegration.__init__'):

...
cp -rf /tmp_volume/config/* /var/ossec/ && chown -R som:som /var/ossec/api
...
if [ $4 == "standalone" ]; then
  ...
  sed -i "s:validate_responses=False:validate_responses=True:g" /var/ossec/api/scripts/som_apid.py
fi
...
chown root:som /var/ossec/etc/client.keys
chown -R som:som /var/ossec/queue/db
chown -R som:som /var/ossec/etc/shared
chmod --reference=/var/ossec/etc/shared/default /var/ossec/etc/shared/group*
cd /var/ossec/etc/shared && find -name merged.mg -exec chown som:som {} \; && cd /
chown root:som /var/ossec/etc/shared/ar.conf
...
/var/ossec/bin/som-control start

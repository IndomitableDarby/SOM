...
def assign_som_ownership(filepath: str):
    ...
    if not os.path.isfile(filepath):
        f = open(filepath, "w")
        f.close()
    if os.stat(filepath).st_gid != common.som_gid() or \
        os.stat(filepath).st_uid != common.som_uid():
        os.chown(filepath, common.som_uid(), common.som_gid())
...
def configure_ssl(params):
    ...
    try:
        ...
        if not os.path.exists(api_conf['https']['key']) \
            ...
            logger.info(
                f"Generated private key file in SOM_PATH/{to_relative_path(api_conf['https']['key'])}")
            generate_self_signed_certificate(private_key, api_conf['https']['cert'])
            logger.info(
                f"Generated certificate file in SOM_PATH/{to_relative_path(api_conf['https']['cert'])}")
        ...
        # Check and assign ownership to som user for server.key and server.crt files
        assign_som_ownership(api_conf['https']['key'])
        assign_som_ownership(api_conf['https']['cert'])
        ...
        if exc.errno == 22:
            ...
            msg = f'Som API SSL ERROR. Please, ensure ' \
                    f'if path to certificates is correct in the configuration ' \
                    f'file SOM_PATH/{to_relative_path(CONFIG_FILE_PATH)}'
            print(msg)
            logger.error(msg)
            raise exc from exc
...
def start(params: dict):
    ...
    If another Som API is running, this function will fail because uvicorn server will
    ...
    app.add_middleware(SomAccessLoggerMiddleware, MiddlewarePosition.BEFORE_EXCEPTION)
...
def print_version():
    ...
    print('\n{} {} - {}\n\n{}'.format(__som_name__, __version__, __author__, __licence__))
...
def add_debug2_log_level_and_error():
    """Add a new debug level used by som api and framework."""
...
if __name__ == '__main__':
    ...
    from som.core import common, pyDaemonModule, utils
    from som.rbac.orm import check_database_integrity
    ...
    try:
        ...
        print(f"Error when trying to start the Som API. {e}")
        sys.exit(1)
    ...
    try:
        ...
        print(f"Error when trying to start the Som API. {api_log_error}")
        sys.exit(1)

    # set permission on log files
    for handler in uvicorn_params['log_config']['handlers'].values():
        if 'filename' in handler:
            assign_som_ownership(handler['filename'])
            os.chmod(handler['filename'], 0o660)

    # Configure and create the som-api logger
    ...
    if 'use_only_authd' in api_conf:
        ...
        logger.warning(
            "'use_only_authd' option was deprecated on v4.3.0. Som Authd will always be used")
    ...
    try:
        ...
        print(f'Internal error when trying to start the Som API. {e}')
        sys.exit(1)
    finally:

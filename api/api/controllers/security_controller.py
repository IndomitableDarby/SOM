...
from som import security, __version__
from som.core.cluster.control import get_system_nodes_or_none
...
from som.core.security import revoke_tokens
...
async def deprecated_login_user(user: str, raw: bool = False) -> ConnexionResponse:
    ...
    return ConnexionResponse(body=token,
           ...
           ConnexionResponse(body=dumps(SomResult({'data': TokenResponseModel(token=token)})),
...
async def get_user_me_policies(pretty: bool = False, wait_for_complete: bool = False) -> ConnexionResponse:
    ...
    """
    data = SomResult({'data': request.context['token_info']['rbac_policies'],
...
async def revoke_all_tokens(pretty: bool = False) -> ConnexionResponse:
    ...
    data = raise_if_exc(await dapi.distribute_function())
    if type(data) == AffectedItemsSomResult and len(data.affected_items) == 0:
        raise_if_exc(SomPermissionError(4000, data.message))

...
def append_som_prefixes(dictionary: Dict, path_fields: Dict[Any, List[Tuple[str, str]]]) -> None:
    """Append Som prefix to all path fields in a dictionary.
    Parameters
    ----------
    dictionary : dict
...
def generate_self_signed_certificate(private_key: rsa.RSAPrivateKey, certificate_path: str):
    ...
    subject = issuer = x509.Name([
        ...
        x509.NameAttribute(NameOID.ORGANIZATION_NAME, u"Som"),
        x509.NameAttribute(NameOID.COMMON_NAME, u"som.com"),
    ])
...
def read_yaml_config(config_file: str = CONFIG_FILE_PATH, default_conf: dict = None) -> Dict:
    ...
    # Append Som prefixes to all relative paths in configuration
    append_som_prefixes(configuration, {API_SSL_PATH: [('https', 'key'), ('https', 'cert'), ('https', 'ca')]})

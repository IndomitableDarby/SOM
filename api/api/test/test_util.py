...
from som.core.exception import SomError, SomPermissionError, SomResourceNotFound, \
    SomInternalError
...
@pytest.mark.parametrize('exception_type, code, extra_fields, returned_code, returned_exception', [
    ...
    (SomError, 1000, ['remediation', 'code'], 400, ProblemException),
    (SomPermissionError, 4000, ['remediation', 'code'], 403, ProblemException),
    (SomResourceNotFound, 1710, ['remediation', 'code'], 404, ProblemException),
    (SomInternalError, 1000, ['remediation', 'code'], 500, ProblemException)
])
...
@pytest.mark.parametrize('obj, code', [
    ((SomError(6001), ['value0', 'value1']), 429),
    ((SomInternalError(1000), ['value0', 'value1']), None),
    ((SomPermissionError(4000), ['value0', 'value1']), None),
    ((SomResourceNotFound(1710), ['value0', 'value1']), None)
])
...
@pytest.mark.parametrize('link', [
    '',
    'https://documentation.som.com/current/user-manual/api/reference.html'
])
@pytest.mark.asyncio
...
async def test_only_master_endpoint(mock_exc):
    ...
    with patch('api.util.running_in_master_node', return_value=False):
        await func_()
        mock_exc.assert_called_once_with(SomResourceNotFound(902))

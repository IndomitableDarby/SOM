...
_som_key = re.compile(r'[a-zA-Z0-9]+$')
_som_version = re.compile(r'^(?:som |)v?\d+\.\d+\.\d+$', re.IGNORECASE)
...
SOM_COMPONENT_CONFIGURATION_MAPPING = MappingProxyType(
    {
...
def is_safe_path(path: str, basedir: str = common.SOM_PATH, relative: bool = True) -> bool:
...
def check_component_configuration_pair(component: str, configuration: str) -> SomError:
    """
    ...
    -------
    SomError
        It can either return a `SomError` or `None`, depending on the given component and configuration. The exception
        is returned and not raised because we use the object to create a problem on API level.
    """
    if configuration not in SOM_COMPONENT_CONFIGURATION_MAPPING[component]:
        return SomError(1128, extra_message=f"Valid configuration values for '{component}': "
                                              f"{SOM_COMPONENT_CONFIGURATION_MAPPING[component]}")
...
@Draft4Validator.FORMAT_CHECKER.checks("som_key")
def format_som_key(value):
    return check_exp(value, _som_key)


@Draft4Validator.FORMAT_CHECKER.checks("som_version")
def format_som_version(value):
    return check_exp(value, _som_version)

...
class SomJsonFormatter(jsonlogger.JsonFormatter):
    """
...
def set_logging(log_filepath, log_level='INFO', foreground_mode=False) -> dict:
    ...
    This function creates a logging configuration dictionary, configure the som-api logger
    ...
    log_config_dict = {
        ...
        "formatters": {
            ...
            "json" : {
                '()': 'api.alogging.SomJsonFormatter',
                'style': '%',
                'datefmt': "%Y/%m/%d %H:%M:%S"
            }
        },
        "filters": {
            'plain-filter': {'()': 'som.core.wlogging.CustomFilter',
                             'log_type': 'log' },
            'json-filter': {'()': 'som.core.wlogging.CustomFilter',
                             'log_type': 'json' }
        },
        "handlers": {
            "default": {
        ...
        "loggers": {
            "som-api": {"handlers": hdls, "level": log_level, "propagate": False},
    ...
    for handler, d in handlers.items():
        if d and 'filename' in d:
            if api_conf['logs']['max_size']['enabled']:
                ...
                d.update({
                    'class':'som.core.wlogging.SizeBasedFileRotatingHandler',
                    'maxBytes': max_size,
                    'backupCount': 1
                })
            else:
                d.update({
                    'class': 'som.core.wlogging.TimeBasedFileRotatingHandler',
                    'when': 'midnight'
                })

...
import som.core.utils as core_utils
import som.rbac.utils as rbac_utils
from api.constants import SECURITY_CONFIG_PATH
from api.constants import SECURITY_PATH
from api.util import raise_if_exc
from som import SomInternalError
from som.core.cluster.dapi.dapi import DistributedAPI
from som.core.cluster.utils import read_config
from som.core.common import som_uid, som_gid
from som.rbac.orm import AuthenticationManager, TokenManager, UserRolesManager
from som.rbac.preprocessor import optimize_resources
...
def generate_keypair():
    ...
    try:
        if not os.path.exists(_private_key_path) or not os.path.exists(_public_key_path):
            ...
            try:
                os.chown(_private_key_path, som_uid(), som_gid())
                os.chown(_public_key_path, som_uid(), som_gid())
...
def decode_token(token: str) -> dict:
    ...
    try:
        ...
        payload = jwt.decode(token, generate_keypair()[1], algorithms=[JWT_ALGORITHM], audience='Som API REST')

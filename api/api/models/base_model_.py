...
from som.core.exception import SomError, SomNotAcceptable
...
class Body(Model):
    ...
    def decode_body(cls, body, unicode_error=None, attribute_error=None):
        try:
            ...
            raise_if_exc(SomError(unicode_error))
        except AttributeError:
            raise_if_exc(SomError(attribute_error))
        return decoded_body

    @classmethod
    def validate_content_type(cls, request, expected_content_type):
        if request.mimetype != expected_content_type:
            raise_if_exc(SomNotAcceptable(6002))
